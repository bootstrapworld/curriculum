...
The option `--slides lessonA` additionally generates the slide deck
...
* A Unix-like environment.
  ...
  - If you're on Windows, you'll want to install [WSL2](https://docs.microsoft.com/en-us/windows/wsl/install) to give yourself a Linux environment first (we recommend Ubuntu).
  - If you're on macOS, your environment is close but missing some tools. You'll want to install [Homebrew](https://brew.sh/) first, then run the following commands to add them:
+
...
* Racket, to do preprocessing and other bookkeeping. Any version should do. We're not using any bleeding-edge features of Racket. You'll want to [download and install DrRacket](https://download.racket-lang.org/) and make sure the `racket` binary is in your `$PATH`.
...
Emphasized text is set within underscores: `+_..._+`.

Bold text is set within stars: `+*...*+`.

In-text code fragments are set within backticks: ``++`...`++``.
...
- a string specifying glossed item (e.g., `"variable"`),  or a list of strings for
  grammatically related glossed items, (e.g., `("apply", "applying",
  "applied")`)
- a string specifying the definition for the glossed item

The reason for associating a list of grammatically related glossed items
to the same definition is to allow the source document to use whatever
variant most fits the prose. The item in the generated glossary will
always be the first item (presumably the singular, non-declined variant) in the list.

As a convenience, it is not necessary to specify
"naive plurals" in this list, viz., the ones that simply add "s" to the
singular. Thus `("coordinate" "coordinates")` can be reduced to
`("coordinate")`, which can be further reduced to `"coordinate"`.
