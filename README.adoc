...
+
  brew install gnu-sed curl make coreutils gnu-tar lua asciidoctor pkg-config cairo pango libpng jpeg giflib librsvg pixman nvm jq racket findutils mkcert
...
* If you want to live-test features that require being logged into BootstrapWorld.org (such as quiz link generation), run `mkcert --install` and follow the instructions in your terminal
...
== Authoring using the hot-reload server

The most common modality for authors is to launch the authoring server (`./author`), then simply edit the curriculum files using Sublime, VSCode, or your favorite text editor. Edits will automatically trigger a rebuild of the affected files, and your browser will automatically reload to any files that have changed.
