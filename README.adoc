...
* `jq` - you _should_ already have this installed. Type `jq` at the command line and hit Enter. If you get `command not found`, WSL/Unix users can install via `sudo apt install jq`.
...
* Lua: `sudo apt-get install lua5.4`

* NVM: follow the instructions on https://github.com/nvm-sh/nvm/blob/master/README.md#installing-and-updating
    to install the Node Version Manager. (Use the `curl` as you've
    already installed `curl`.)

=== Last step (for everyone!)
