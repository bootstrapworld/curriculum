...
- `--no-pdf` (aka `--nopdf`) avoids PDF generation to speed up the debugging
process.
...
`data-science`, add the pathway by name as a second argument:
  ...
  deploy-to-brown jrandomuser data-science-pathway
...
*Note:* The Algebra pathway is an anomaly, since it exists in both `-wescheme` and `-pyret` variants. To deploy this pathway, you will need to _separately_ type 
