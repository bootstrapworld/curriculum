...
If no `--proglang` option is supplied, a proglang is assumed after
reading the `proglang.txt` associated with the course:
If a course is defined only for
a single proglang, that proglang is assumed. If a course is defined for
multiple proglangs, the first of them is assumed. If no proglangs are
explicitly defined for a course, the proglang is assumed to be `pyret`.
The possible values for this option are
`pyret`, `wecheme`, `codap`, and `spreadsheets`.
The distribution directory for a pathway built with
a non-``pyret`` proglang typically tacks on the proglang as suffix. No
suffix is tacked on if the course only admits of one proglang.

If the option `--everything` is supplied, every pathway is built for all
possible proglangs.
