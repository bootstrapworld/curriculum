...
* There are two packages inside the `lib/sublime-settings/` folder in the repository. Both need to be copied into your Sublime User folder following the instructions below:

** *On MacOS*, type `open ~/Library` in the Terminal to open the hidden Library folder in the Finder. Then navigate to Application Support -> Sublime Text -> Packages -> User. Copy both files from the `sublime-settings` folder in the repo, into your `User` folder.

** *On Windows*, enter `explorer %APPDATA\Sublime Text\Packages\User` into your Windows terminal. This will open the folder in Explorer. Copy both files from the `sublime-settings` folder in the repo, into your `User` folder.
...
used to specify it. But this conversion is more often than not
...
hella confusing to read unless your list is very small.
