...
- Racket, to do preprocessing and other bookkeeping. Any version should do. I’m not using any
bleeding-edge features of Racket.

- HTML -> PDF generator `puppeteer`, as well as `puppeteer-cluster` to build in parallel.  `sudo npm install puppeteer` to install the first, `sudo npm install puppeteer-cluster` for the second.

- PDF collection and page numbering is done using `pdf-lib`. This can be installed via `sudo npm install pdf-lib`.
