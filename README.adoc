...
`@fitb`, `@fitbruby` and `@blanklines` all draw writing lines whose spacing is controlled by CSS custom properties, rather than by per-element padding or margins. Each writing line is `line-height` tall, so changing one number changes the height of the line _and_ the space it reserves.
...
Because custom properties inherit, set one on the widest element that should be affected, in the page's `<style>` block. `--blank-height` is usually all you need:
