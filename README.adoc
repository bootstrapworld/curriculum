...
its metadata. Here is an example:
...
Ensure that all four fields have values, otherwise the build will issue
warnings when using any such underdefined image. (You will get warnings
even if the value is an empty string. However, for the moment,
empty-string captions are ignored.)
