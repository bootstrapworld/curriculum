...
the `--natlang` option chosen during build. The default is
...
`make` can take the variable settings on the command line:
...
`make clean` scrubs any existing `distribution`.
...
This requires that the environment variables `HOSTINGER_IPADDR`,
