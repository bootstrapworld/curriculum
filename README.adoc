...
- `--pdf` (or `--book`, for legacy reasons) generates PDFs for every page and
  solution-page, and merges them into `workbook.pdf` (normal student workbook),
  `long-workbook.pdf`, (solutions for student workbook), `workbook-long.pdf`
  (workbook w/ optional exercises included) and `pd-workbook.pdf` (workbook w/ lesson plans prepended to student workbook.
...
- A Unix-like environment. If you're already on Linux, you're there.
    -- If you're on Windows, you'll want to install [WSL2](https://www.windowscentral.com/how-install-wsl2-windows-10) to give yourself a Linux environment first.
    -- If you're on macOS, your environment will need some additional unix tools. You'll want to install [Homebrew](https://brew.sh/) first.

- Bash. Keeps all the scripts humming. (This is already available on Linux, macOS, and WSL)

- Racket, to do preprocessing and other bookkeeping. Any version should do. We're not using any bleeding-edge features of Racket. You'll want to [download DrRacket](https://download.racket-lang.org/) and make sure the `racket` binary is in your `$PATH`.

- Asciidoctor, a Ruby program, to generate HTML from AsciiDoc. (The format is AsciiDoc, the program that converts it is Asciidoctor. I wish we had this neat nomenclatural separation for other programs too.) To install it,
+
...
- GNU `sed`. This is pre-installed on Linux/WSL, but may need to be
explicitly installed on macOS, using `brew install gnu-sed`.

- A current version of `curl`. This is pre-installed on Linux/WSL, but may need to be
explicitly updated on macOS, using `brew install curl`.

- Several `npm` packages, *all of which* are installed by running `npm install`:
...
occassionally there are search terms a user might employ which are
NOT reflected in any part of the lesson (i.e. - "PEMDAS"). These 
keywords can be added anywhere in the lesson with the `keywords` 
directive: `@keywords{larry, curly, moe}`. 
