...
repo anywhere on the machine hosting the Bootstrap website, and
you have a valid `distribution` subdirectory, then calling the
script `deploy-from-here` updates the documents displayed on the
site.

If you don't have a valid `distribution` subdirectory, calling
...
https://www.bootstrapworld.org/materials/spring2020/courses.
