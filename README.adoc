...
`standards-csta-dictionary.rkt` for a list of standards defined
...
- `GRAPH=yes` -- updates the lesson dependency graph in
  `distribution/en-us/dependency-graph.js`. (By default, this file is
  created only if doesn't exist already)
...
- `--deploy`: deploys to website. If dir `distribution/` not found,
  makes it first; otherwise uses already-built distribution
- `--force` (aka `--superforce`, `--super-force`, `-f`, `-F`):
  builds from scratch, scrubbing any previous `distribution/`
- `--graph` (aka `-g`): updates the lesson dependency graph
...
NOTE: There was an older script called `build-pathway` that our authors have
...
1.  a symbol specifying the natural language (e.g., `en-us`, `es-mx`)
2. the glossed item. This can be:
...
generated glossary
3. a string specifying the definition for the glossed item
