...
+
  brew install gnu-sed curl make coreutils gnu-tar lua
...
* Node. The standard Node offered view your OS's package manager may not
  ...
  - Do `nvm install 19` followed by `nvm use 19` to use version 19,
    which is OK for now.
  - Note: the node `canvas` package is extremely finicky with versions of node. If you are getting installation errors about `canvas` or `node-gyp`, use node v19!
