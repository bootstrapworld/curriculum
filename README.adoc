...
If no `--proglang` option is supplied, the programming language
used is `wescheme`. To specify another language, say `pyret`, use
`--language pyret`.

   distribution/
     pathwayX-wescheme-en-us/index.html
...
link:distribution/pathwayP-wescheme-en-us/lessons/lessonA/index.html[Sample
lesson plan]
