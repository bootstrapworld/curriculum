...
https://bootstrapworld.site/materials/spring2023/en-us
...
the value of the environment variable `$NATLANG` during build.
The default is
...
Captions are optional and will print under the image on the webpage and workbook page. They can be left out if no caption is desired.
...
`make` can take environment-variable settings on the command line:

- `BOOK=yes` -- creates the PDF versions of the HTML files, as also the
...
- `LINKCHECK=yes` -- checks the various internal and external links in
  the documents to ensure they are valid. By default, this check isn't
  done to save time

- `GRAPH=yes` -- creates the lesson dependency graph in
  `distribution/en-us/dependency-graph.js`

- `NATLANG=LL` -- builds for the natural language `LL`. The default is
  `en-us` (AmEng)

- `SEMESTER=season` -- can be set to anything, typically: `fall`,
  `spring`. The default is `fall`

- `YEAR=yyyy` -- can be set to anything, typically the four-digit year
  of the Common Era. The default is the current year followed by
  `-BETA`

[IMPORTANT]
--
These environment variables may also be set separately (i.e., not as a
`make` argument) on your machine, either on the command line or in your
shell profile. In such cases, they should be `export`ed so that the make
can see them, e.g.,
...
The `make` target `clean` scrubs any existing `distribution`.
...
We
currently deploy to:
https://www.bootstrapworld.site

A typical `make deploy` goes to something like
https://www.bootstrapworld.site/materials/fall2023, assuming that
...
command line or your shell profile, or on the `make` command line, e.g.,

    make SEASON=fall YEAR=2023 deploy

IMPORTANT: If `SEASON` and `YEAR` are not set externally and are not
...
overwriting
a currently active deployment. Always explicitly set `SEASON` and
`YEAR` when deploying in earnest.

Deployment requires that you have enabled SSH access to the web site machine,
...
[NOTE]
--
`make deploy` actually calls the script `deploy-to-hostinger`, which may also be
called on its own, e.g.,

    deploy-to-hostinger
    SKIPLIB=true deploy-to-hostinger
--

=== The build wrapper

A Bash script `build` is provided as a convenient wrapper for `make`:
...
(There was an older script called `build-pathway` that our authors have
gotten used to. It is provided as a trivial identify wrapper for
`build`.)
...
various standards. For now, these are:
  ...
  standards-cc-ela-dictionary.rkt
...
dictionary file `standards-cc-math-dictionary.rkt`
...
+
  # use modern unix versions of commands, installed via brew
  PATH="$(brew --prefix)/opt/coreutils/libexec/gnubin:/usr/local/opt/gnu-sed/libexec/gnubin:/usr/local/opt/make/libexec/gnubin:$PATH"
