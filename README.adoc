...
[IMPORTANT]
--
The variables `SEMESTER` and `YEAR`
may also be set separately, as true environment variables (i.e., not as a
`make` argument), on your machine. This can be done either on your shell command line or in your
shell profile. In such cases, they should be ``export``ed so that the make
can see them, e.g.,

    export SEMESTER=spring YEAR=2023

You can also supply these prefix environment-variable settings:

    SEMESTER=fall YEAR=2023 make ...

The other variables (`NATLANG`, `COURSE`) can only be supplied as `make` arguments.
--
...
and manually delete the lessons you want rebuilt. You can also manually
