...
* `hunspell` used for spellchecking changes to .adoc files
+
On MacOS, `brew install hunspell`. In addition, install the `en_US`
dictionaries by following the instructions in
https://github.com/hunspell/hunspell#usage, and putting the files
`en_US.aff` and `en_US.dic` in one of the directories specified by
`hunspell -D`. For macOS, a good location is `~/Library/Spelling`.
