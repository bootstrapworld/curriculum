...
* There are two packages inside the `lib/sublime-settings/` folder in the repository, which need to be copied into your Sublime User folder:

** *On MacOS*, open the Terminal and make sure you're in the `curriculum` repository folder. Type `cp lib/sublime-settings/* ~/Library/Application\ Support/Sublime\ Text/Packages/User/`.

** *On Windows*, make sure your curriculum folder is open on the screen, and navigate to `lib/sublime-settings`. Enter `explorer %APPDATA\Sublime Text\Packages\User` into your Windows terminal to open _another_ window in Explorer. Copy both files from the `curriculum/lib/sublime-settings` folder into your `Sublime Text\Packages\User` folder.
