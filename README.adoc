...
- @`fitbruby{6em}{salmon}{fish species}` - displays a blank line 3em wide, with the word "salmon" inside, and the ruby "fish species" underneath
...
`@blanklines` can be any number of lines *tall*, whereas `@fitb` and `@fitbruby` are always the fixed height but have variable *width* (3em, 6em, etc.). Leaving the width property blank makes the element stretch all the way to the end of the containing block.
...
Custom properties cannot be defined on an element itself - they can only be inherited. To tweak the value for every fill-in-the-blank element on a page, you would declare the property on the _whole page_:

*The (old) Property Way:* `div#body .fitb { padding-top: 2.2em; }` +
*The Custom Property Way:* `div#body { --blank-height: 2.2em; }`

Here's an example of a page that uses all three to one height for `@fitb`s on the page, another for all `@fitbruby`s on the page, and another for blanks inside a table cell, and .
...
td { --blank-height: calc(0.2em + 1em); }        /* only blanks inside table cells */
.fitbruby { --fitb-height: calc(0.3em + 1em); }  /* only fitbruby blanks */
</style>
++++
----
...
* *Set the property on an ancestor, not on the blank.* `.fitb`, `.fitbruby` and `.blanklines` read it; scoping a rule to a container (`#content`, `td`, a class you add with `@span` or a block attribute) is how a whole region is tightened together.
...
`@fitb{width}{answer}` takes an optional width (e.g. `@fitb{8em}{}`); with no width, the blank stretches to the end of the line. Prefer `em` for widths too, for the same screen/print reason as above.
