...
CAUTION: For backward compatibility, the filename
         `lesson-index.rkt` is allowed in lieu of
         `workbook-pages.rkt` but this use is deprecated.
...
CAUTION: For backward compatibility, the filename
         `workbook-index.rkt` is allowed in lieu of
         `lesson-order.rkt` but this use is deprecated.
