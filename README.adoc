...
`sample-lesson/langs/en-us/index.adoc`. While the file basename doesn't
...
options `--debug`, `--help`, `--language`, `--nopdf`,
`--proglang`, `--verify-link`, and `--version`.

If `--help` or `--version`, the script displays a help message
...
process. - `--verify-links` reports invalid links in the doctree
...
Each lesson plan is strongly advised to have a

  @lesson-description{A brief paragraph describing the lesson.}

This is displayed in the lesson itself, but is also part of the
autogenerated thumbnails used by the pathway narrative for each
of its lessons.
