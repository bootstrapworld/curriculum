...
function drawBlankLines() {
  document.querySelectorAll('.blanklines').forEach(n => {
    ...
    // via --blank-height (see core.less). Use getComputedStyle instead of
    // getBoundingClientRect to account for browser zoom
    const rowHeight = parseFloat(getComputedStyle(n).lineHeight);

    // How many lines? The @blanklines macro records the
    // count in data-lines; fall back to deriving it from the reserved
    // height, which the macro sets to n * rowHeight.
    const requested = parseInt(n.dataset.lines, 10);
    const nLines = requested > 0 ? requested
      : Math.max(1, Math.round(n.offsetHeight / rowHeight));

    // Draw each rule as a real border-bottom (same "solid thin black" that
    // .fitb uses) rather than an SVG background: in PDF output an SVG
    // background is rasterized and comes out lighter/thinner than the
    // vector border on a neighboring @fitb, and on screen (1.25x zoom) a
    // fixed 1px SVG stroke is heavier than the snapped border.
    n.querySelectorAll(':scope > .blankrule').forEach(r => r.remove());
    const probe = document.createElement('span');
    probe.className = 'fitb';
    n.appendChild(probe);
    const bw = parseFloat(getComputedStyle(probe).borderBottomWidth) || 1;
    probe.remove();
    for (let i = 1; i <= nLines; i++) {
      const rule = document.createElement('div');
      rule.className = 'blankrule';
      rule.style.cssText = `position:absolute; left:0; right:0; height:0; pointer-events:none;` +
        ` top:${(i * rowHeight - bw).toFixed(2)}px; border-bottom:solid thin black;`;
      n.appendChild(rule);
    }
    ...
    n.style.height = `${nLines * rowHeight}px`;
