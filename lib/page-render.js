...
function drawBlankLines() {
  document.querySelectorAll('.blanklines').forEach(n => {
    // .blanklines and .fitb/.fitbruby all get their row height from the
    // same --blank-pitch custom property (core.less), so .blanklines' own
    // line-height already IS the pitch a stack of fitb lines would use --
    // no hidden-rig measurement needed. getComputedStyle returns layout
    // px regardless of the workbookpage's CSS zoom, so no zoom conversion
    // is needed either (unlike getBoundingClientRect, which is zoom-scaled).
    const pitch = parseFloat(getComputedStyle(n).lineHeight);

    // How many lines were requested? The @blanklines macro records the
    ...
    // Draw one rule per writing line. A fitb's border-bottom sits at the
    // bottom of its own pitch-tall box, so the first rule is one pitch
    // down from the top, the second two pitches down, and so on. An SVG
    // <line> strokes centered on its coordinate (half above, half below),
    // unlike a CSS border-bottom which fills flush inward from the edge --
    // drawn at the exact row boundary, the last rule's bottom half would
    // fall outside the container and get clipped, rendering it visibly
    // thinner than the rest. Nudge the stroke up by half its width so it
    // fills flush against the boundary instead, matching how the border
    // actually renders (and incidentally fixing the last-row clipping,
    // since every rule now sits entirely inside its own row).
    ...
    // or as a run of @fitb lines. line-height is already the pitch, so
    // solution-page answer text already advances at the right rate --
