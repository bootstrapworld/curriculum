...
function drawBlankLines() {
  document.querySelectorAll('.blanklines').forEach(n => {
    ...
    //
    // The LAST rule sits flush against the background image's own bottom
    // edge (nLines * rowHeight), which is also where the CSS background
    // gets clipped to this element's box. Chrome's headless print/PDF
    // rasterizer (unlike its screen compositor) can clip a hair into that
    // 0.5px margin, rendering just that one rule visibly thinner -- give
    // it 1px of clearance instead to keep it out of the clipped margin.
    let dataUrl = 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg">';
    for (let i = 1; i <= nLines; i++) {
      const offset = (i === nLines) ? 1 : 0.5;
      const y = (i * rowHeight - offset).toFixed(1);
