...
async function fetchUserHash() {
    console.log('fetching user hash from admin-ajax.php')
  const formData = new FormData();
  formData.append('action', 'get_user_hash');

  return await fetch('https://beta.bootstrapworld.org/wp-admin/admin-ajax.php', {
    method: 'POST',
    body: formData
  })
...
async function fetchQuizId(teacher_hash, quiz_path) {
  const formData = new FormData();
  formData.append('teacher_hash', teacher_hash);
  formData.append('quiz_path', quiz_path);

  return await fetch('https://beta.bootstrapworld.org/data/GenerateAssessmentLink.php', {
    method: 'POST',
    body: formData
  })
  .then(response => response.json())
...
async function makeQuizLink(quizButtonLink, teacherId, quiz_path) {
  ...
  async function handleKeyDown(event) {
    if (event.key === "Enter") {
      ...
      const quizId = await fetchQuizId(teacherId, quiz_path);
      linkList.innerHTML += `<li><a href="${quiz_path}?groupId=${userText}&teacherId=${teacherId}&assessmentId=${quizId}">${userText}</a>`;
      reset();
