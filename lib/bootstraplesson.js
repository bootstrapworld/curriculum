...
async function populateQuizLinks() {
  ...
  quizLinks.forEach( link => {
    ...
    quizBox.innerHTML += `${link.innerHTML}
      ...
      <input type="text" id="textInput" placeholder="Name your class here and press Enter">
    <ul></ul>`
    link.replaceWith(quizBox);
  })
}

function makeQuizLink(quizButtonLink, teacherId, quizLink) {
  ...
  function reset() {
    ...
    textBox.value = null;
    textBox.removeEventListener(handleKeyDown);
  }
  ...
  function handleKeyDown(event) {
    if (event.key === "Enter") {
      ...
      if (!userText) reset();
      linkList.innerHTML += `<li><a href="${quizLink}?groupId=${userText}&teacherId=${teacherId}">${userText}</a>`;
      reset();
