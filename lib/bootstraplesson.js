...
function populateLesson() {
  ...
  function pdfDownloadLinks(lesson, pathway, priorData) {
    let pageLinks = [...document.querySelectorAll(`a[href*="${lesson}/pages"]`)];
    ...
    function makeListAndAddButton(optPages) {
      ...
      const inGraph = (optPages? graph[lesson].optPages : graph[lesson].pages)
        .map(p => p.replace('adoc', ''));
      pageLinks = pageLinks.filter(l => 
        inGraph.some(p => l.href.includes(p)));
      console.log('from graph', inGraph, 'matching links', pageLinks)

      if (pageLinks.length == 0) return; // no pages? bail
      ...
      pageLinks.forEach(link => {
        ...
        if(!uniqueLinks.has(idx)) { uniqueLinks.set(idx, link); }
      });
      const sortedLinks = [...uniqueLinks.entries()].sort((a,b) => a[0]<b[0]? -1 : 1);
      console.log(JSON.stringify(sortedLinks))
      sortedLinks.forEach(([idx,link]) =>
        sublist.innerHTML += `<li><a href="${link.href}">${link.innerHTML}</a></li>`);
