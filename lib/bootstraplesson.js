...
function populateLesson() {
  ...
  function buildLessonSidebar(lesson, pathway, priorData) {
    let lessonList = (pathway && pathwayTocs[pathway] && getPathwayLessons(pathway)) || lessons;
    if(!lessonList) {
      lessonList = priorData.titles.reverse();
    }
    const sidebarSection = document.getElementsByClassName("sidebarlessons")[0];
    const lessonDOMList = sidebarSection.getElementsByTagName("ul")[0];
    lessonList.forEach((l, i) => {
      if(!graph[l]) { return; } // if it's not in the graph, skip
      const li = document.createElement('li');
      const linkHTML = `<a href='../${l}/index.shtml`
