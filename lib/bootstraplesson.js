 ...
 function isNewest() {
   ...
   if (!path.includes("latest")) {
     ...
     try {
       ...
       console.log("Could not retrieve a url", e);
     }
...
function getParams() {
  ...
  var lesson = path.includes("pages")? false: path.slice(-2)[0];
...
function populateLessonFromGraph() {
  ...
  if(!pathway || !pathwayTocs[pathway] || !pathwayTocs[pathway].includes(normalize(lesson))) {
    console.warn('This lesson is not a part of a recognized pathway')
    pathway = false;
  }
