...
if(typeof document !== 'undefined') {
  document.addEventListener('DOMContentLoaded', function () {
    const container = document.getElementById('body'); // this is the div#body elt
    console.log(container);
    fixStretchTables();
    attachCodeMirror();
    if(!container.classList.contains("workbookpage")) {
      configureGraph(graph);
      //isNewest();
      attachCollapsible();
    }
    if(container.classList.contains("TeacherResources")) { populateTeacherResources(); }
    if(container.classList.contains("LessonPlan"))       { populateLesson();           }
  });
}
