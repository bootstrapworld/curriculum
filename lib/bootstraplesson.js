...
function normalizeLesson(l){ return l.split("-").slice(0,-1).join("-"); }
...
function computeTransitiveData(lesson, priorLessons) {
  return priorLessons.reduceRight(
    (acc, l) => ({
      ...
      prerequisites: acc.prerequisites.includes(l)? 
        graph[l].prerequisites.concat(acc.prerequisites).filter(onlyUnique)
        : acc.prerequisites.filter(onlyUnique)
    }),
    { pages, primitives, prerequisites} = graph[lesson]
  );
}
...
function populateLessonFromGraph() {
  ...
  var normalizedPriors = graph[normalizedLesson].prerequisites.concat([normalizedLesson]);
  ...
  if(!pathwayTocs || !pathway || !pathwayTocs[pathway]) {
    ...
    if(normalize) { normalizedPriors = normalizedPriors.map(normalizeLesson); }
  }
  ...
  var priorData = computeTransitiveData(normalizedLesson, normalizedPriors);

  addPageNums(priorData, normalize);
  buildLessonSidebar(lesson, normalizedLesson, pathway);
  makeLangTable(priorData);  
}

function addPageNums(priorData, normalize) {
...
function buildLessonSidebar(lesson, normalizedLesson, pathway) {
  var prerequisites = graph[normalizedLesson].prerequisites;
  ...
  pathwayTocs[pathway].forEach((l, i) => {
