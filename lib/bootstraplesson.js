...
function populateTeacherResources() {
  ...
  const container = document.querySelectorAll('.lesson-info-table')[0];
  ...
  lessons.forEach(l => {
    ...
    if(notes) {
      rowStr += `<br><a href="../../../l/lessons/+${l}/pages/${notes}">Notes</a>`
    }
    rowStr += `</td><td>`
    rowStr += starters
      .filter(name => !["editor", "program-list"].includes(name))
      .map(name => {
        console.log(name, starterFiles, starterFiles[name])
        return `<a href="${starterFiles[name][proglang].url}">${starterFiles[name].title}</a>`
      })
      .join(', ');
    rowStr += `</td></tr>`;
