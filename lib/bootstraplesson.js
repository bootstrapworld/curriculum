...
function populateLessonFromGraph() {
  ...
  */
  ...
  try {
    addAllStateStandardsEntries();
    populateStandardsDictionaries();
    showStandardsAlignment();
  } catch (e) { console.log('adding Standards', e); }
  try {
...
function addAllStateStandardsEntries() {
  const dropdown = document.getElementsByClassName('standardsAlignmentSelect')[0];
  const nationalGroup = document.createElement('optgroup');
  nationalGroup.label = "By Standard";
  Object.keys(dictionaries.standards).forEach(dict => {
    const opt = document.createElement('option');
    opt.value = 'standards-'+dict;
    opt.appendChild(document.createTextNode(dict));
    nationalGroup.append(opt)
  });
  dropdown.appendChild(nationalGroup);

  const stateGroup = document.createElement('optgroup');
  stateGroup.label = "By State";
  [...stateDict.entries()].forEach(entry => {
    const opt = document.createElement('option');
...
function populateStandardsDictionaries() {
  // get the standard keys for this lesson
  const {lesson, pathway} = getParams();
  const dictParent = document.querySelector('table.sidebarstandards tr:last-child .content')
    || document.querySelector('#content');

  // Return the dictionary list that matches the name, 
  // creating a new one if necessary
  function getDictList(dictName) {
    const dictList = dictParent.querySelector('.standards-'+dictName+' dl');
    if (dictList) { return dictList; }
    else {
      const div = document.createElement('div');
      div.classList.add('dlist', 'alignedStandards', 'standards-'+dictName);
      div.innerHTML = '<div class="title"></div><dl></dl>';
      dictParent.append(div);
      return div.querySelector('dl');
    }
  }

  // Add dictionaries and alignment info for every standard
  // this lesson addresses
  for( const [dict, stds] of Object.entries(dictionaries.standards) ) {
    for( const [id, std] of Object.entries(stds) ) {
      if((!lesson && std.lessons.length) || (lesson && std.lessons.includes(lesson))) {
        const list = getDictList(dict);
        const dt = document.createElement('dt');
        dt.className = "hdlist1";
        dt.innerHTML = id;
        const dd = document.createElement('dd');
        dd.innerHTML = "<p>" + std.description;
        // if it's a a pathway alignment page, list and link all lessons
        if(!lesson && std.lessons.filter(l => graph[l]).length > 0) {
          dd.innerHTML += "[See: ";
          dd.innerHTML += std.lessons.map(l => {
            return !graph[l]? "" : 
...
function showStandardsAlignment() {
  ...
  [...stdTables].forEach(t => t.style.display = 'none');
  const selection = selectTag.value;
  const stds = document.querySelector('table.sidebarstandards tr:last-child .content')
    || document.querySelector('#content'); 

  var stdTable = document.getElementsByClassName(selection);

  // If we don't have an alignment div for this, make one and add it
  if(!stdTable.length) {
    const newStdTable = document.createElement('div');
    newStdTable.classList.add('dlist', 'alignedStandards', selection);
    newStdTable.innerHTML = '<div class="title"></div><dl></dl>';
    stds.appendChild(newStdTable);
  }

  // Get the relevant table, and add copycat standards if it has them
  if(stdTable.length > 1) throw "ERROR: two standards tables have the same name";
  if(stdTable.length == 0) throw "ERROR: no standards table exists for " + selectTag.value;

  // Add any copycat standards to this table's dictionary list
  stdTable = stdTable[0];
  stdEntries = stdTable.querySelector('dl');
  if(!stdTable.classList.contains('finshed')) {
    if(ccStates.includes(selectTag.value.slice(10))) {
      copyStandards('.standards-CCSS-Math', stdEntries);
      copyStandards('.standards-CCSS-ELA', stdEntries);
    }
    if(cstaStates.includes(selectTag.value.slice(10))) {
      copyStandards('.standards-CSTA', stdEntries);
    }
    stdTable.classList.add('finshed'); // mark as completed
  }
...
window.addEventListener("load", function () {
  ...
  attachCollapsible();
  sidebarToggleEvent();
  if(document.body.classList.contains("LessonPlan")) { populateLessonFromGraph(); }
  if(location.href.includes('pathway-alignments')) { populatePathwayAlignment(); }
  fixStretchTables();
});
