...
function populateLesson() {
  ...
  function addPageNums(priorData) {
    ...
    [...document.querySelectorAll(printables)].forEach((span) => {
      ...
      const index = priorData.pages.indexOf(path.join("/")) + 1;
      if (index < 1) console.error(`link ${link} does not match a workbook page`);
      else if (!lowIndex  || index <= lowIndex) lowIndex  = index;
      else if (!highIndex || index > highIndex) highIndex = index;
    ...
    if(pathway) {
      ...
      pagesLink.innerHTML += ` ${lowIndex}-${highIndex}`;
    }
...
const knownLongPages = [
  ...
  'why-not-spreadsheets.pdf',        // from data-science resources
  'datasets-overview.pdf'            // from data-science resources
];
