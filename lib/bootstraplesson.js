...
function populateAlignmentDictionaries(alignedTo) {
  ...
  switch(alignedTo) {
    case "lesson" : alignedLessons = [lesson].map(normalize); break;
    case "pathway": alignedLessons = lessons.map(normalize); break;
    default : alignedLessons = Object.keys(graph);
  }
  ...
  function addGroup(keyName, groupLabel) {
    ...
    for( const [dict, stds] of Object.entries(dictionaries[keyName]) ) {
      ...
      for( const [id, std] of Object.entries(stds) ) {
        ...
        if(std.lessons.filter(l => alignedLessons.includes(l)).length) {
          ...
          if(!lesson) {
            std.description += " [See: ";
            std.description += std.lessons.filter(l => alignedLessons.includes(l)).map(l =>
