...
function populateAlignmentDictionaries() {
  ...
  function addGroup(keyName, groupLabel) {
    ...
    for( const [dict, stds] of Object.entries(dictionaries[keyName]) ) {
      ...
      // OR if there's a lesson in the current pathway that aligns to it
      for( const [id, std] of Object.entries(stds) ) {
        if((!lesson && std.lessons.filter(l => graph[l]).length) 
          ...
          if(!lesson) {
            std.description += " [See: ";
            std.description += std.lessons.filter(l=>graph[l]).map(l =>
