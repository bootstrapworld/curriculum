...
function computePriors(lesson, pathway) {
  ...
  if(!pathwayTocs || !pathway || !pathwayTocs[pathway]) {
    console.warn("Not associated with pathway '" + pathway
      + "'. This lesson will be rendered in standalone mode");
    // compute transitive prereqs
    ...
    normalizedPriors = graph[lesson].prerequisites.concat([lesson]);
...
function buildLessonSidebar(lesson, suffix, pathway, priorData) {
  ...
  var prerequisites = graph[lesson].prerequisites;
...
// if a pathway is defined, add a link to the workbook at the top of the
// materials section
function pathwayWorkbookLink(pathway) {
  ...
  var materialsList = preamble.querySelectorAll('tr')[2].querySelector('ul');
  var wbPages = materialsList.querySelectorAll('.PrintableExercise:not(.Optional)');
  ...
  materialsList.insertBefore(LI, materialsList.children[0]);
}
...
function populateLessonFromGraph() {
  ...
  */
  ...
  if(pathway){
    addPathwayTitle(pathway);
    pathwayWorkbookLink(pathway);
  } 
  addPageNums(priorData);
  addPrereqs(priorData, suffix);
...
function addPrereqs(priorData, suffix) {
  var prereqs = priorData.prerequisites.filter(onlyUnique);
  if (prereqs.length == 0) return; // don't render an empty row
  ...
  prereqs.forEach(l => {
    var li = document.createElement('li');
    li.innerHTML = '<p><a href="../'+l+suffix+'">'+graph[l].title+'</a></p>';
    ul.appendChild(li);
  })
