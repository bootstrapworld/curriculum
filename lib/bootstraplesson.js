...
function addCopyCatStandards() {
  var dropdown = document.getElementsByClassName('standardsAlignmentSelect')[0];
  ccStates.forEach( state => {
    var opt = document.createElement('option');
    opt.value = state.short; opt.appendChild(document.createTextNode(state.long));
    dropdown.appendChild(opt);
  });
}

function showStandardsAlignment() {
  console.log('showStandardsAlignment');
  ...
  // use the tag value by default, but fall back to copycat standards
  var stdTable = selectTag.value;
  if(ccStates.includes(stdTable)) stdTable = "standards-CCSS-Math";
  stdTable = document.getElementsByClassName(stdTable);
