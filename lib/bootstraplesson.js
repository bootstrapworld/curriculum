...
function isNewest() {
  ...
  if(url.includes("curriculum/distribution/en-us")) return;
...
function configureGraph(graph) {
  ...
  const requiredFields = ["glossary", "prerequisites", "pages", "optPages", "starterFiles", "primitives"];
    ...
    function makeCollapsiblePageList(container, isOptional, text) {
      ...
      if(pageList.length > 0) {
        container.querySelector('ul').innerHTML = `
          <li><p><a href="javascript:downloadLessonPDFs(${isOptional})">${text}</a></p>
        ...
        pageList.forEach(({fileName:path, title:text}) => {
          ...
          if(priorData.pathway && !isOptional) {
            html += `<span class="pageNum">(Page ${index + 1})</span>`;
          }
          html += `</li>`;
        ...
        if(!isOptional && graph[lesson].pages.length) {
          ...
          else pagesLink.innerHTML += ` ${lowIndex + 1}-${highIndex + 1}`;
        }
        let studentWorkbookHTML;
        if(pathway) {
           studentWorkbookA = `<a href="../../courses/"+pathway+"/workbook/workbook.pdf">`;
      ...
      starterList.forEach(name => {
        ...
        starterBullets += `<li><p><a href="${file[proglang].url}">${file['title']}</a></p></li>`
      })
