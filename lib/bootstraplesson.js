...
function addCopyCatStandards() {
  const dropdown = document.getElementsByClassName('standardsAlignmentSelect')[0];
  const states = new Set(cstaStates.concat(ccStates));
  [...states.values()].sort().forEach( state => {
  const opt = document.querySelector('option[value="standards-'+state+'"]') 
    || document.createElement('option');
    opt.innerHTML = ''; // reset any existing opt names
    opt.value = 'standards-'+state; 
    opt.appendChild(document.createTextNode(stateDict.get(state)));
...
function showStandardsAlignment() {
  ...
  function copyStandards(source, dest) {
    ...
    [...stds.children].forEach(n => {
      dest.querySelector('dl').appendChild(n.cloneNode(true));
    });
  }
  ...
  // If we don't have a list for this, make one
  if(!document.getElementsByClassName(selection).length) {
  ...
  stdTable = stdTable[0];
  // Copy the CC standards to this table
  if(!stdTable.classList.contains('copied')) {
    if(ccStates.includes(selectTag.value.slice(10))) {
      copyStandards('.standards-CCSS-Math', stdTable);
      copyStandards('.standards-CCSS-ELA', stdTable);
    }
    // Copy the CSTA standards to this table
    if(cstaStates.includes(selectTag.value.slice(10))) {
      copyStandards('.standards-CSTA', stdTable);
    }
    stdTable.classList.add('copied'); // mark as completed
  }
