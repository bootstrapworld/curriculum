...
function populateLesson() {
  ...
  function populateMaterials(priorData) {
    ...
    function makeCollapsiblePageList(materialsList, isOptional) {
      const bullet = materialsList.children[0];
      const pagesLink   = bullet.querySelector('a');
      const pageSublist = bullet.querySelector('ul');
      const pageList = isOptional? graph[lesson].optPages : graph[lesson].pages;
      ...
      const starterList = isOptional? graph[lesson].starterFiles : graph[lesson].optStarterFiles;
      ...
      materialsList.innerHTML += otherBulletsHTML;
    }
