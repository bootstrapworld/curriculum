...
function showStandardsAlignment() {
  ...
  // If we don't have a list for this, make one and fill it with relevant copies
  if(!document.getElementsByClassName(selection).length) {
    ...
    if(ccStates.includes(selectTag.value.slice(10))) {
      copyStandards('.standards-CCSS-Math', newStdTable);
      copyStandards('.standards-CCSS-ELA', newStdTable);
    }
    // Copy the CSTA standards to this table
    if(cstaStates.includes(selectTag.value.slice(10))) {
      copyStandards('.standards-CSTA', newStdTable);
    }
    stds.appendChild(newStdTable);
  }
  stdTable = document.getElementsByClassName(selection);
  ...
  stdTable[0].style.display = 'block';
}
