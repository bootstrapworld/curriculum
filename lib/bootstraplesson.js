...
const getTransitivePrereqs = (l) => {
  ...
  return prereqs.concat(...prereqs.map(getTransitivePrereqs)).filter(onlyUnique).sort(sortByPrereqs);
}
...
function computeTransitiveData(lesson, pathway, lessons = false) {
  ...
  // If we're looking at a list of lessons
    ...
    priorLessons = getTransitivePrereqs(lesson).concat([lesson]);
  }
  console.log(priorLessons);
  ...
  function processLessonData(acc, l) {
    ...
    let lessonGlossary = graph[l].glossary || []; // some lessons (like HOC) don't have any glossary defined
    lessonGlossary.forEach(({vocab, description}) => accGlossary.set(vocab, description));
...
function populateLesson() {
  ...
  function buildLessonSidebar(lesson, pathway, priorData) {
    ...
    if(!lessonList) {
      lessonList = priorData.titles.sort(sortByPrereqs);
    }
  ...
  function checkPathwayOnlyContent(pathway) {
  ...
  function pdfDownloadLinks(lesson,pathway, priorData) {
    const pageLinks = [...document.querySelectorAll(`a[href*="${lesson}/pages"]`)];

    function makeListAndAddButton(optPages) {
      const pagesLink = document.querySelector(`a[href^="javascript:downloadLessonPDFs(${optPages})"]`);
      console.log("adding to", pagesLink)
      const container = pagesLink.parentNode;
      const uniqueLinks = new Map();
      let sublist, firstIndex, lastIndex;

      // filter by opt- or required-pages
      const pagesFromGraph = (optPages? graph[lesson].optPages : graph[lesson].pages)
        .map(p => p.replace('adoc', ''));
      const filtered = pageLinks.filter(l => 
        pagesFromGraph.some(p => l.href.includes(p)));
      console.log('from graph', pagesFromGraph, 'matching links', filtered)

      // If there's a pathway, add link to student workbook for required pages
      if((pageLinks.length > 0) && !optPages && pathway) {
        ...
        container.insertBefore(txt,  container.lastChild);
        container.insertBefore(link, container.lastChild);
        pagesLink.innerHTML += " " + firstIndex + "-" + lastIndex;
      }

      // Add sublist, page range
      if (pageLinks.length > 0) { 
        sublist = document.createElement("ul"); 
        container.appendChild(sublist); 
      }
      pageLinks.forEach(link => {
        const idx  = priorData.pages.indexOf(link.getAttribute("href"));
        if(idx > -1) { // add page numbers if it's in the book
          link.firstChild.data += ` (Page ${idx})`; // add pagenum
          firstIndex = (!firstIndex || (idx < firstIndex))? idx : firstIndex;
          lastIndex  = (!lastIndex  || (idx > lastIndex ))? idx : lastIndex;
        }        
        // if it's our first time processing this link, add it the sublist
        if(!uniqueLinks.has(idx)) {
          sublist.innerHTML += `<li>${link.outerHTML}</li>`;
          uniqueLinks.set(idx, link); // add to unique map
        }
      });
    }
    makeListAndAddButton(false); // required
    makeListAndAddButton(true);  // optional
  }
  ...
  */
  ...
  try {
    checkPathwayOnlyContent(pathway);    // Potentially hide any pathway-only content
    checkPathwaySpecificContent(pathway); // show/hide pathway-specific content
  } catch (e) { console.log('checkPathwayContent', e); }
  try { 
    pdfDownloadLinks(lesson, pathway, priorData);
  } catch (e) { console.log('pdfDownloadLinks', e); }
  try { 
...
function showPageLinks(optPages) {
  const pagesLink = document.querySelector(`a[href^="javascript:downloadLessonPDFs(${optPages})"]`);
  console.log(optPages, pagesLink)
...
function downloadLessonPDFs(optPages) {
  const { lesson, pathway, lessons } = getParams();
  makeCustomBook(kebabToTitleCase(lesson), [lesson], optPages);
}

async function makeCustomBook(title, lessons, optPages = false) {
    let { lesson, pathway } = getParams();
    ...
    var pageUrls = lessons
        .map(l => {
          const requiredPages = graph[l].pages.concat(graph[l].handouts);
    ...
    console.log("building custom book from these paths", pageUrls)
...
/*
    function downloadAndCleanup(file, bytes) {
      label.innerHTML = "Downloading..."
      download(bytes, title + " - Student Workbook.pdf", "application/pdf");
      status.style.display = "none";
    }
*/
    makeWorkbook(
      ...
      async (file, bytes) => await showPDFAndCleanup(file, bytes)
    );
}
  ...
  longworkbook = false) 
{
  ...
  for (const [idx, path] of paths.entries()) {
    ...
    // For each page-to-be-copied, rotate and paginate
    pages.forEach(page => {
      ...
      const pageNum = isBack? romanize(backPageCount) : String(mainPageCount);
      ...
      if (!isFront) { page.drawText(pageNum, {size: 8, ...opts}); }
