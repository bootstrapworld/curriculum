...
function populateLesson() {
  ...
  function pdfDownloadLinks(lesson, pathway, priorData) {
    ...
    let linkMap = new Map();
    // add page numbers to required pages, and build a->idx map
    pageLinks.forEach(a => {
      const idx  = priorData.pages.indexOf(a.getAttribute("href"));
      if(idx > -1) { // add page numbers if it's in the book
        a.firstChild.data += ` (Page ${idx})`; // add pagenum
      }        
      // Remove the link, re-adding with index as the key
      linkMap.set(a, idx)
    });
