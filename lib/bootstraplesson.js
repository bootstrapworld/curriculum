const { pathwayTocs }    = require('./pathway-tocs.js');
let graph;
...
async function attachCodeMirror() {
  const { CodeMirror } = await import("codemirror/addon/runmode/runmode.node.js");
...
async function configureGraph() {
  graph = await import('./dependency-graph.js');
...
function populateLesson() {
  ...
  async function populateMaterials(priorData) {
    const { pathway, lesson, proglang } = getParams();
    const { starterFiles } = await import('./starterFiles.js');
    ...
    function makeCollapsiblePageList(container, isOptional, text) {
      ...
      if(pageList.length > 0) {
        container.querySelector('ul').innerHTML = `
          <li class="noprint"><p><a onclick="Bootstrap.main.downloadLessonPDFs(${isOptional})">${text}</a></p>
...
async function populateLessonAlignment(lesson) {
  ...
  async function addGroup(keyName, groupLabel) {
    // add the dropdown optgroup
    const group = document.createElement('optgroup');
    group.label = groupLabel;

    // load the dictionaries
    const { dictionaries } = await import('./dictionaries.js');
  ...
  await addGroup("standards", "By Standard");
  await addGroup("practices", "By Practice");
  await addGroup("textbooks", "By Textbook");
 ...
 * PRINTING FUNCTIONS
 *******************************************/
async function downloadLessonPDFs(optPages) {  
  const { lesson } = getParams();
  const { makeCustomBook } = await import('./makeWorkbook.js');
...
if(typeof document !== 'undefined') {
  document.addEventListener('DOMContentLoaded', async function () {
    fixStretchTables();
    await attachCodeMirror();
    
    if(!document.body.classList.contains("workbookpage")) {
...
module.exports = { 
  ...
  downloadLessonPDFs : downloadLessonPDFs,
  getParams: getParams,
  graph: graph,
  computeTransitiveData: computeTransitiveData,
};