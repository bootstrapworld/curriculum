...
function getParams() {
  ...
  if(pathway && lessons && lessons.length > 0) { 
    console.log('custom pathway', lessons);
...
function populateLessonFromGraph() {
  ...
  // If there's a lessons list, find all the lessons that come before this one
  // Otherwise just use the transitive prereqs for this lesson
  function computePriors(lesson, lessons) {
    var normalizedPriors;
    if(lessons) {
      normalizedPriors = lessons.slice(0, lessons.indexOf(lesson) + 1);
    } else {
      console.warn("Could not find a list of valid lessons. This page will be rendered in standalone mode");
  ...
  function computeTransitiveData(lesson, pathway, lessons) {
    var priorLessons = computePriors(lesson, lessons.map(normalize));
  ...
  function buildLessonSidebar(lesson, suffix, pathway, lessons, priorData) {
    ...
    var lessonList = lessons? lessons.map(normalize) : priorData.titles.reverse();
    ...
    lessonList.forEach((l, i) => {
      ...
      var linkHTML = "<a href='../" + l + suffix + "/index.shtml" 
       + location.search + "'>" + graph[l].title + "</a>";
  ...
  function workbookPageLinks(pathway) {
    ...
    if(!pathway || !pathwayTocs[pathway] || !wbPages.length) {
  ...
  if (typeof graph == undefined || !graph) {
    console.warn("Dependency graph did not load! This lesson may not render correctly");
    return;
  }
  ...
  if(!lesson) {
    console.warn('Not a valid lesson - will skip populateLessonFromGraph()');
    return;
  }

  var suffix = lesson.replace(normalize(lesson), '');

  // compute prior lessons & aggregate page/prereq/primitive data from priors
  // if no pathway or lesson list is defined, just use prereqs for this lesson
  var priorData = computeTransitiveData(normalize(lesson), pathway, lessons);
  /*
  console.log(
    "pathway is", pathway, 
  ...
  */
  ...
  try { 
    if(pathway) { workbookPageLinks(pathway); }
  ...
  try { 
    buildLessonSidebar(normalize(lesson), suffix, pathway, lessons, priorData); 
  } catch (e) { console.log('buildLessonSidebar', e); }
  try {
    populateAlignmentDictionaries();
  ...
  function addPrereqs(lesson, suffix, pathway) {
    ...
    // if a lesson list is defined, sort the prereqs in that order
    if(lessons) {
      prereqs.sort((a,b) => (lessons.indexOf(a) < lessons.indexOf(b))? -1 : 1);
    }
    ...
    prereqs.forEach(l => {
      if (!graph[l]) return;
      var li = document.createElement('li');
      li.innerHTML = '<p><a href="../'+l+suffix+'/index.shtml'+location.search+'">'+graph[l].title+'</a></p>';
      ul.appendChild(li);
    })
...
function populateAlignmentDictionaries() {
  const {lesson, pathway} = getParams();
  const pathwayLessons = pathway? pathwayTocs[pathway].map(normalize) 
    : lesson? []
    : Object.keys(graph);
  ...
  function addGroup(keyName, groupLabel) {
    ...
    for( const [dict, stds] of Object.entries(dictionaries[keyName]) ) {
      ...
      for( const [id, std] of Object.entries(stds) ) {
        if((!lesson && std.lessons.filter(l => pathwayLessons.includes(l)).length)
          || (lesson && std.lessons.includes(lesson))) {
          ...
          if(!lesson) {
            std.description += " [See: ";
            std.description += std.lessons.filter(l => pathwayLessons.includes(l)).map(l =>
