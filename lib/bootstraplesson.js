...
function populateLesson() {
  ...
  function pdfDownloadLinks(pathway) {
    // Most lessons have a list of materials. If not, our work here is done
    const materialsList = document.querySelector('.ulist.materials-links .ulist');
    if(!materialsList) return;

    const materialsBullet = materialsList.parentNode;
    
    if(pathway) {
      const path = "../../courses/"+pathway+"/workbook/workbook.pdf";
      let LI = document.createElement("LI");
      LI.innerHTML = `<a href="${path}">Download the entire Student Workbook</a>`;
      materialsBullet.parentNode.insertBefore(LI, materialsBullet.nextSibling);
    }
  }
...
function downloadLessonPDFs(optPages) {
  const { lesson, pathway, lessons } = getParams();
  makeCustomBook(lesson, [lesson], optPages);
}
...
async function makeCustomBook(title, lessons, optPages = false) {
    ...
    var pageUrls = lessons
        .map(l => (optPages? graph[l].optPages : graph[l].pages)
            .filter(p => p !== "")
    ...
    function downloadAndCleanup(file, bytes) {
      ...
      download(bytes, pathway + "- Student Workbook.pdf", "application/pdf");
    ...
    makeWorkbook(
      ...
      async (file, bytes) => await downloadAndCleanup(file, bytes)
    );
}
  ...
  longworkbook = false) 
{
  ...
  for (const [idx, path] of paths.entries()) {
    ...
    if (longworkbook && (pages.length > 1) && !knownLongPages.includes(fileName)) {
      console.log(`WARNING: in ${course} ${outputPath.split('/').pop()}, 
        ${path} has ${pageCount} pages`);
    }
    // Unless it's whitelisted, truncate all PDFs to one page
    if(!knownMultiPages.includes(fileName)) { pages.splice(0, 1); }
    ...
    pages.forEach(page => {
      ...
      if (isBack)  { backPageCount++;  }
      if (!(isFront || isBack)) { mainPageCount++; }
      ...
      const portrait       = { x: width / 2, y: 26,         size: 8, rotate: degrees(  0)}
      const leftLandscape  = { x: 26,        y: height / 2, size: 8, rotate: degrees(-90)}
      const rightLandscape = { x: width-26,  y: height / 2, size: 8, rotate: degrees( 90)}
      
      // Add page number for main and back pages
      const opts = isLandscape? (isRightHand? rightLandscape : leftLandscape) : portrait;
      if (!isFront) { page.drawText(pageNum, opts); }
