 ...
 *******************************************/
const proglangs = process.env.ALL_PROGLANGS.split(new RegExp(' \+'));
...
function populateLessonFromGraph() {
  ...
  function computeTransitiveData(lesson, pathway, lessons = false) {
    var priorLessons;
    // Are we looking at a pathway, or standlone?
    if(lessons) {
  ...
  function buildLessonSidebar(lesson, priorData) {
    ...
    lessonList.forEach((l, i) => {
      ...
      var linkHTML = "<a href='../" + removeProglangSuffix(l) + "/index.shtml" 
...
function populateAlignmentDictionaries(alignedTo) {
  ...
  switch(alignedTo) {
    ...
    case "pathway": alignedLessons = lessons; break;
