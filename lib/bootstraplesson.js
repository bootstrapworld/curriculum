...
function configureGraph(graph) {
  // remove "phantom" lessons (empty directories left over from
  ...
  Object.keys(graph).forEach(k => {
    graph[k].glossary ??= []; // HOC lessons have null glossaries
...
function computeTransitiveData(lesson, pathway, lessons = false) {
  ...
  function processLessonData(acc, l) {
    ...
    graph[l].glossary.forEach(({vocab, description}) => acc.glossary.set(vocab, description));
...
function populateLesson() {
  ...
  if(!lesson) {
    console.warn('Not a lesson - will skip populateLessonFromGraph()');
    return;
  }
  ...
  if(document.body.classList.contains('pyret') || document.body.classList.contains('wescheme')){
    ...
    } catch (e) { console.error('makeLangTable', e); }
  }
  */
...
function downloadLessonPDFs(optPages) {
  ...
  const pageNums = pagePaths.map(p => optPages? "" : priorData.pages.indexOf(p) + 1);

  makeCustomBook(kebabToTitleCase(lesson), [lesson], optPages, pageNums);
}
  ...
  presetPageNums = false) 
{
  ...
  for (const [idx, path] of paths.entries()) {
    ...
    if((pages.length > 1) && !knownLongPages.includes(fileName)) {
      ...
      if (longworkbook) {
        console.warn(`WARNING: in ${course} ${outputPath.split('/').pop()}, 
