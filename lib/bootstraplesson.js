...
function attachCollapsible() {
  ...
  // all the sections
  const sections = document.querySelectorAll(".LessonPlan h2, .TeacherResources .sect1 h2");
  ...
  sections.forEach(function(section) {
    ...
    section.addEventListener("click", function(event) {
      const toToggle = event.shiftKey? sections : [section];
      toToggle.forEach(toggleCollapsible);
    });
  });
}
...
function downloadLessonPDFs() {
  const { lesson, pathway, lessons } = getParams();
  makeCustomBook(lesson, [lesson]);
}

async function makeCustomBook(title, lessons) {
  ...
  longworkbook = false) 
{
  ...
  // given a PDF doc and the raw bytes of a PDF page,
  // add page to the doc using the proper orientation
  async function renderPageBytesIntoBook(bytes, path) {
    // handle back matter and contract pages differently
    ...
    // Paginate non-front-matter pages, but use different numbering for back-matter
  ...
  for (const [idx, path] of paths.entries()) {
    ...
    if (longworkbook && (pageCount > 1) && !knownLongPages.includes(fileName)) {
      console.log(`WARNING: in ${course} ${outputPath.split('/').pop()}, 
        ${path} has ${pageCount} pages`);
    }

    if(knownMultiPages.includes(fileName)) {
      for (let i = 0; i < pageCount; i++) {
        const subDocument = await PDFDocument.create();
        const [copiedPage] = await subDocument.copyPages(doc, [i])
        subDocument.addPage(copiedPage);
        const bytes = await subDocument.save()
        await renderPageBytesIntoBook(bytes, path);
      }
    } else {
      await renderPageBytesIntoBook(bytes, path);
    }
