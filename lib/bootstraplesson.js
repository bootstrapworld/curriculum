...
function populateLesson() {
  ...
  function pdfDownloadLinks(lesson, pathway, priorData) {
    ...
    let linkMap = new Map();
    // add page numbers to required pages, and 
    // build an href->{a,idx} map (to remove duplicates)
    pageLinks.forEach(a => {
      const index  = priorData.pages.indexOf(a.getAttribute("href"));
      if(index > -1) { // add page numbers if it's in the book
        a.firstChild.data += ` (Page ${index})`; // add pagenum
      }        
      linkMap.set(a.href, {a:a, idx: index})
    });
    ...
    function makeListAndAddButton(optPages) {
      ...
      const entries = [...linkMap.entries()]
        .filter(([href, {a, idx}]) => optPages? idx == -1 : idx > 0)
        .sort((a,b) => a[1].idx<b[1].idx? -1 : 1);
      if (entries.length == 0) return; // no pages? bail

      // build the page links, in order
      const sublist = document.createElement("ul"); 
      entries.forEach(([href,{a, idx}]) =>
        sublist.innerHTML += `<li><a href="${href}">${a.innerHTML}</a></li>`);
      ...
      if(!optPages && pathway) {
        ...
        pagesLink.innerHTML += " " + entries[0][1] + "-" + entries.pop()[1];
      }
