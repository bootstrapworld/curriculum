...
const getTransitivePrereqs = (l) => {
    ...
    const prereqs = graph[l].prerequisites;
...
function configureGraph(graph) {
    ...
    const lessonsToIgnore = ['__sample-lesson','__sample-lesson-wescheme', 'zample-lesson', 'hoc-wescheme-intro', 'hoc-winter', 'hoc-winter-parley', '__sample-lesson-codap', 
        ...
        'project-exploration-paper-codap', 
...
function populateLesson() {
  ...
  function correctPathwayLinks(pathway) {
    [...document.querySelectorAll('.pathway-link a')].forEach(link => {
      const href = link.getAttribute('href');
      if(!pathway) { link.parentNode.remove(); }
      else { link.href = href.substring(0,13) + "/" + pathway + href.substring(13); }
    });
  }
  ...
  try {
    ...
    // if a pathway is defined, correct the link at runtime. If not, hide it.
    correctPathwayLinks(pathway);
...
function populateAlignmentDictionaries(alignedTo) {
  ...
  function addGroup(keyName, groupLabel) {
    ...
    for( const [dict, stds] of Object.entries(dictionaries[keyName]) ) {
      let {fullname,url} = stds['__metadata'];
