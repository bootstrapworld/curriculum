...
function populateAlignmentDictionaries() {
  ...
  // add a group of standards, showing alignment and menu entry
  function addGroup(keyName, groupLabel) {
    ...
    for( const [dict, stds] of Object.entries(dictionaries[keyName]) ) {
      ...
      // Walk over entries, adding each to the list
      for( const [id, std] of Object.entries(stds) ) {
        if((!lesson && std.lessons.length) || (lesson && std.lessons.includes(lesson))) {
          ...
          if(!lesson && std.lessons.filter(l => graph[l]).length > 0) {
            std.description += " [See: ";
            std.description += std.lessons.map(l => {
              return !graph[l]? "" : 
      ...
      // if we actually have alignments, add them to the page and optgroup
...
function showStandardsAlignment() {
  ...
  if(!stdTable) {
    ...
    stds.appendChild(stdTable);
  }

  // Get the relevant table, and add copycat standards if it has them
  if(!stdTable) throw "ERROR: no standards table exists for " + selectTag.value;
  ...
  if(!stdTable.classList.contains('finished')) {
    ...
    stdTable.classList.add('finished'); // mark as completed
  }
