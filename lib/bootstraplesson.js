...
/*******************************************
 * Extract pathway name, current lessons,
 * and lesson list from URL and dependency graph
 *******************************************/
...
function computeFromPathway() {
  ...
  if (!lesson) console.error("lesson is not defined");
  if (!pathwayLessons || !graph || !lesson) return;

  var idx = pathwayLessons.indexOf(lesson);
  ...
  langTableRow.innerHTML = '<td>Language Table</td>';
  langTableRow.appendChild(langTableCell);
  langTableCell.appendChild(makeLangTable(priorData.primitives));
  preamble.appendChild(langTableRow);
}
...
function makeLangTable(primitives) {
  ...
  types.forEach(type => {
    ...
    row.innerHTML = "<td><p>"+type+"</p></td><td>" + 
      prims[type].map(p => '<tt class="pyret">'+p+'</tt>').join() + 
      "</td><td>" + langTable[type].values.map(v => 
        '<tt class="pyret">'+v+'</tt>').join() +
      "</td>";
