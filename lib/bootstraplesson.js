...
function isNewest() {
  ...
  const url_year = parseInt(url.match(/(\d{4})\//)[1]);
  const release = url.slice(0, url.indexOf(url_year) + 4);
  const current_date = new Date();
  const current_month = current_date.getMonth();
  const current_year = current_date.getFullYear();

  const next_release = (release.match(/\/spring/i))? release.replace("/spring", "/fall")
    : release.replace(/\/fall\d{4}/, `/spring${(url_year+1)}`);
  const next_year = parseInt(next_release.match(/(\d{4})/)[1]);

  // is the next release in the fall of this year?
  const fall_year = next_release.match(/fall(\d{4})/);
  const this_fall = fall_year && (fall_year[1] == current_year)
  ...
  nodes.forEach( n => {
    // Get the computed style object for the element
      ...
      if (lineHeight === 'normal') {
        ...
        lineHeight = tempDiv.offsetHeight + 'px'; // Get the measured height
      ...
      let yOffset = 0, y=Math.round(baselineOffset);
      while (yOffset < nodeHeight) {
        const line = `<line x1="0" y1="${y + yOffset}" x2="1000" y2="${y + yOffset}" style="stroke:black;stroke-width:1"/>`
        dataUrl += line;
        yOffset = Math.round(yOffset + lineHeight);
      }
      dataUrl += '</svg>'
      const st =
      n.style.height = `round(up, ${n.style.height} - ${marginTop} + 3px, 1px)`;
...
function getStarterFileUrl(name, proglang) {
  ...
  const prefix = ((proglang == "pyret") && (file[proglang].prefix == undefined))
    ? "https://code.pyret.org/editor#shareurl=" : "";
  return prefix + file[proglang].url;
}
...
function populateLesson() {
  ...
  function populateMaterials(priorData) {
    ...
    function makeCollapsiblePageList(container, isOptional, text) {
      ...
      const pageList = isOptional
        ? graph[lesson].optPages
        : [...graph[lesson].pages, ...graph[lesson].handouts];
      const materialsList = container.querySelector('ul');
      ...
      if(pageList.length > 0) {
        ...
        pageList.forEach(({fileName:path, title:text}) => {
          ...
          if (isWorkbookPage && index < 1) {
            console.error(`link ${path} doesn't match a workbook page`);
          }

          let itemHTML = `<li><span class="PrintableExercise ${isOptional? "Optional" : ""}">
          ...
          if(isWorkbookPage) {
            itemHTML += `<span class="pageNum">(Page ${index})</span>`;
            if (!lowIndex  || index <= lowIndex) lowIndex  = index;
            if (!highIndex || index > highIndex) highIndex = index;
          } else if(isHandout) {
            itemHTML += `<span class="pageNum">(handout)</span>`;
          }
          itemHTML += `</li>`;

          pageSublist.innerHTML += itemHTML;
        });

        // If there's a pathway, add link to student workbook for required pages
        if(!isOptional && graph[lesson].pages.length) {
          console.log(lowIndex === highIndex ? ` ${lowIndex}` : ` ${lowIndex}-${highIndex}`)
          pagesLink.innerHTML += lowIndex === highIndex ? ` ${lowIndex}` : ` ${lowIndex}-${highIndex}`;
        }
        let studentWorkbookHTML;
        if(pathway && !isOptional) {
          const workbookLink = `../../courses/${pathway}/workbook/workbook.pdf`;
          let workbookAnchor = `<a href="${workbookLink}" target="_blank">`;
          if(!pathwayTocs[pathway] && lessons) {
             workbookAnchor = `<a href="javascript:void(0)" onclick="makeBook()">`;
          }
          bullet.firstChild.innerHTML += ` from the ${workbookAnchor} Student Workbook</a>`;
        }
      ...
      starterList.forEach(name => {
        if (!["program-list", "editor"].includes(name)) {
          materialsList.innerHTML += `<li><p>
    ...
    makeCollapsiblePageList(requiredContainer, false,
      "PDF of all Handouts and Page"+(graph[lesson].pages.length > 1? "s" : ""));
    const requiredMaterials = requiredContainer.querySelector('ul');

    if (graph[lesson].glossary.length) {
      requiredMaterials.innerHTML += `<li><p><a href="../../Glossary.shtml?lesson=${lesson}" target="_blank">Lesson Glossary</a></p></li>`;
    }
    if (graph[lesson].slides) {
      requiredMaterials.innerHTML += `<li><p><a href="${graph[lesson].slides}" target="_blank">Lesson Slides</a></p></li>`;
    }
    requiredMaterials.innerHTML += `<li><p><a href="index.pdf" target="_blank">Printable Lesson Plan</a> (a PDF of this web page)</p></li>`;
    ...
    if (["pyret", "wescheme"].includes(proglang)) {
      optMaterials.innerHTML += `<li><p><a href="../../Contracts.shtml?pathway=${pathway}">Contracts Reference</a></p></li>`;
    }
  }
  ...
  function buildLessonSidebar(lesson, pathway, priorData) {
    ...
    const toolbar = document.querySelector('.lesson-content').nextElementSibling;
    ...
    // attach to existing sidebar entries, and inherit WP classes
    toolbar.lastElementChild.appendChild(sidebar);
    sidebar.classList.add("m17zxgn2tjtr-1b", "m17zxgn2tjtr-12", "m17zxgn2tjtr-11", "m17zxgn2tjtr-1c", "x-text");
    sidebarstandards.classList.add("m17zxgn2tjtr-m", "x-text");

    var sidebarSection = document.getElementsByClassName("sidebarlessons")[0];
    var lessonDOMList = sidebarSection.querySelector("ul");
    lessonDOMList.classList.add("m17zxgn2tjtr-m", "x-text");
    lessonList.forEach((l, i) => {
      if(!graph[l]) { return; } // if it's not in the graph, skip
      var li = document.createElement('li');
      var linkHTML = "<a href='../" + l + "/"
    ...
    if(!title) { console.error('EXCEPTION: no title was found'); }
      ...
      title.innerHTML = `
        ...
        ${kebabToTitleCase(pathway)} Lessons</h4>`;
    }
    title.innerHTML += `<p/>`;
  }
  ...
  function addPrereqs(prereqs) {
    ...
    if(pathway && pathwayTocs[pathway]) {
      const pathwayLessons = getPathwayLessons(pathway);
    }
    ...
    const materialsRow = document.querySelectorAll('table.preamble tr')[2];
