...
const getTransitivePrereqs = (l, callingLesson = l) => {
    if(!graph[l]) {
        throw "getTransitivePrereqs: " + callingLesson + " requested an unknown lesson " + l;
    }
...
function populateLesson() {
  ...
  function pdfDownloadLinks(pathway, priorData) {
    // If there's a pathway, add page range and workbook link
    if(pathway) {
      ...
      pageLinks.forEach(link => {
        ...
        if(idx == -1) return; // skip pages that are not in the book
      ...
      const workbookPath = "../../courses/"+pathway+"/workbook/workbook.pdf";
      container.innerHTML += " from the <a href='"+workbookPath+"'>Student Workbook</a>";
    }
  }
  ...
  */
  ...
  try { 
    pdfDownloadLinks(pathway, priorData);
...
const knownLongPages = [
  'stress-project.pdf',              // from box-plots
  'rubric-stress-or-chill.pdf',      // from box-plots
  'datasets-and-starter-files.pdf',  // from choosing-your-dataset
  'for-teachers-about-datasets.pdf', // from choosing-your-dataset
  'answer-key.pdf',                  // from debugging
  'infographic.pdf',                 // from bar-and-pie-charts
  'flags-of-the-world-resource.pdf', // from flags
  'SavingImagesGuide.pdf',           // from making-game-images
  'infographics.pdf',                // from project-infographics
  'animation-worksheet.pdf',         // from multiple reactive lessons
  'pythag-practice.pdf',             // from distance-formula
  'dr-telephone.pdf',                // from functions-dr
  'Contracts.pdf',                   // from multiple course resources
];
  ...
  longworkbook = false) 
{
  ...
  for (const [idx, path] of paths.entries()) {
    ...
    // Warnings will already have been thrown during PDF generation (see html2pdf.js)
