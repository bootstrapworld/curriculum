...
function downloadLessonPDFs(optPages) {
  ...
  makeCustomBook(graph[lesson].title, [lesson], true, optPages, pageNums);
}

function makeBook() {
    var {pathway, lessons, lesson, proglang} = getParams();
    // for each lesson, walk through the pages
    const pageCount = lessons.reduce((acc, l) => acc + graph[l].pages.length, 1)
    const pageNums = Array.from({length: pageCount}, (_, i) => i + 1)
    makeCustomBook(pathway, lessons, true, false, pageNums)
}

async function makeCustomBook(title, lessons, reqPages=false, optPages=false, solns=false, pageNums) {
  const url = window.location.href.toLowerCase();
  const baseURL = url.match(/.*\/materials\/(fall|spring)\d{4}\/[a-z]{2}\-[a-z]{2}\//)[0];
  ...
  var pageUrls = lessons
    .map(l => {
      ...
      const requiredPages = reqPages? graph[l].pages : [];
      const optionalPages = optPages? handouts.concat(graph[l].optPages) : [];
      return [...requiredPages, ...optionalPages].filter(p => p.fileName !== "")
        .map(p => baseURL + `lessons/${l}/${solns? "solution-pages" : "pages"}/${p.fileName}`)
      })
    .flat()
  ...
  async function makeTOC() {
    ...
    lessons.forEach((l, i) => {
      const title = graph[l].title;
      const pages = graph[l].pages.length + (optPages? graph[l].optPages.length : 0);
      TOC.drawText(title, {
        x: 75,
        y: height - (150 + (i * 30)),
        size: 14,
  ...
  warnings.forEach(({path, length}) => 
    console.log(`WARNING: in ${title}, ${path} has ${length} pages`));
  ...
  ) 
{
  ...
  // Ready the TOC
  if(TOCbytes) {
    let doc   = await PDFDocument.load(TOCbytes);
    const [TOCpage] = await workbook.copyPages(doc, [0]);
  }
  ...
  for (const [idx, path] of paths.entries()) {
    ...
    if((pages.length > 1) && !knownLongPages.includes(fileName)) {
      ...
      if (longworkbook) {
        ...
        warnings.push({path: path.split('/').slice(-3).join('/'), length: pages.length});
      }
    ...
    pages.forEach((page, multiPageIdx) => {
      ...
      if(mainPageCount == 1 && TOCbytes) {
        // Load the table of contents before the first page
  ...
  return {path: outputPath, bytes: pdfBytes, warnings: warnings};
}
/*******************************************
 * EVENT HANDLERS
 *******************************************/
