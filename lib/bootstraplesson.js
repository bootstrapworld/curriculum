...
function computePriors(lesson, pathway) {
  ...
  if(!pathwayTocs || !pathway || !pathwayTocs[pathway]) {
    ...
    priorLessons = graph[normalizeLesson(lesson)].prerequisites.concat([normalizeLesson(lesson)]);
  } else {
    var pathwayLessons = pathwayTocs[pathway];
    var idx = pathwayLessons.indexOf(normalizeLesson(lesson)) + 1;
    priorLessons = pathwayLessons.slice(0, idx).map(normalizeLesson);
  }
  return priorLessons;
}
...
function processLessonData(acc, l) {
  ...
  return ({
      ...
      prerequisites: 
        (acc.prerequisites.length == 0 || acc.prerequisites.includes(l)? 
          graph[l].prerequisites.concat(acc.prerequisites) : acc.prerequisites)
        .filter(onlyUnique)
    });
}
...
function computeTransitiveData(lesson, pathway) {
  ...
  return priorLessons.reduceRight(
    processLessonData,
    { titles: [], pages: [], primitives:[], prerequisites:[] });
}
...
function buildLessonSidebar(lesson, pathway, priorData) {
  var lessonList = pathwayTocs[pathway] || priorData.titles;
  var prerequisites = graph[normalizeLesson(lesson)].prerequisites;
