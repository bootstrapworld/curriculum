...
function populateLesson() {
  ...
  function addPageNums(priorData) {
    ...
    [...document.querySelectorAll(printables)].forEach((span) => {
      ...
      const pageNum = ((path.split('/')[3] == lesson)? `Page ` : `Prior lesson - Page `) + index;
      span.parentNode.innerHTML += `<span class="pageNum">(${pageNum})</span>`;
      // When downloading PDFs from this lesson, skip references to other pages
      if(path.split('/')[3] == lesson) {
...
async function makeCustomBook(title, lessons, optPages=false, presetPageNums=false) {
  ...
  const priorPages = computeTransitiveData(lesson, pathway, lessons)
    .pages.map(p => p.replace(".html", ".pdf"))

  console.log(
    "building custom book from these paths", pageUrls, priorPages,
    "pageNums are", pageUrls.map(url => '../../lessons/'+lesson+'/'+url))
  ...
  presetPageNums = false) 
{
  ...
  for (const [idx, path] of paths.entries()) {
    ...
    const pageNums = presetPageNums || pages.map(page => {
      ...
      return isBack? romanize(backPageCount) : String(mainPageCount);
    });
    ...
    pages.forEach((page, i) => {
      ...
      if (!path.includes("front-matter")) { 
        page.drawText(pageNums[i], {size: 8, ...opts}); 
      }
