...
function downloadLessonPDFs() {
  const { lesson, pathway, lessons } = getParams();
  makeCustomBook(lesson, [lesson], true);
}
...
async function makeCustomBook(title, lessons, optPages = false) {
    ...
    var pageUrls = lessons
        .map(l => graph[l].pages.concat(optPages? graph[l].optPages : [])
            .filter(p => p !== "")
  ...
  longworkbook = false) 
{
  ...
  for (const [idx, path] of paths.entries()) {
    ...
    let copiedPages = await workbook.copyPages(doc, doc.getPageIndices());
    const fileName = path.match(/[ \w-]+\.[\w]+/)[0];
    if (longworkbook && (copiedPages.length > 1) && !knownLongPages.includes(fileName)) {
    ...
    if(knownMultiPages.includes(fileName)) { copiedPages.slice(0, 1); }

    // For each page-to-be-copied, rotate correctly and paginate
    copiedPages.forEach(page => {
      // Compute page number (arabic or roman)
      const isBackMatter = path.includes("back-matter") || path.includes("Contracts.pdf");
      const isFrontMatter = path.includes("front-matter");
      if (isBackMatter)   { backPageNo++; }
      if (!isFrontMatter) { pageNo++; }
      const pageNum = isBackMatter? romanize(backPageNo) : String(pageNo);
      ...
      const portraitOpts      = { x: width / 2, y: 26,         size: 8, rotate: degrees(0)  }
      const leftLandscapeOpts = { x: 26,        y: height / 2, size: 8, rotate: degrees(-90)}
      const rightLandscapeOpts= { x: width-26,  y: height / 2, size: 8, rotate: degrees(90) }
      
      // Add page number and merge into the workbook
      const opts = isLandscape? (isRightHand? rightLandscapeOpts : leftLandscapeOpts) : portraitOpts;
      if (!isFrontMatter) { page.drawText(pageNum, opts); }
