...
function populateLessonFromGraph() {
  ...
  */
  ...
  } catch (e) { console.log('addCopyCatStandards', e); }
  try {
...
function addAllStateStandardsEntries() {
  ...
  [...stateDict.entries()].forEach(entry => {
    const opt = document.querySelector('option[value="standards-'+entry[0]+'"]') 
      || document.createElement('option');
    opt.innerHTML = ''; // reset any existing opt names
...
function populateStandardsDictionaries() {
  ...
  const lesson = graph[getParams().lesson];
  const dictParent = document.querySelector('table.sidebarstandards tr:last-child .content');

  // DELETEME
  console.log('deleting hardcoded standard nodes');
  [...dictParent.children].forEach(n => n.remove());
  ...
  lesson.standards.forEach(aligned => {
    Object.keys(dictionaries.standards).forEach(std => {
      if(dictionaries.standards[std][aligned]) {
        const list = getDictList(std);
        const dt = document.createElement('dt');
        dt.className = "hdlist1";
        dt.innerHTML = aligned;
        const dd = document.createElement('dd');
        dd.innerHTML = "<p>" + dictionaries.standards[std][aligned] + "</p>";
        list.appendChild(dt);
        list.appendChild(dd);
      }
    });
...
function showStandardsAlignment() {
  ...
  // If we don't have an alignment table for this, make one and add it
  if(!stdTable.length) {
