...
/* This space left blank to keep line number consistent */
...
function fixStretchTables() {
  console.log('calling fixStretchTables');
...
function formatPageURL(page, lesson) {
  return ("../" + lesson + "/" + page).replace(".adoc", "");
}
...
function getParams() {
  ...
  var season = url.pathname.split('/')[2];
...
console.log('populateLesson');
  function populateMaterials(priorData) {
    const { pathway, lesson, proglang, season } = getParams();
    ...
    if (graph[lesson].glossary.length !== 0) {
      otherBulletsHTML += `
      <li><p><a href="/wp-content/themes/pro-child/static-resources/${season}/en-us/Glossary.shtml?lesson=${lesson}" target="_blank">Lesson Glossary</a></p></li>`;
    }
    ...
    if(["pyret", "wescheme"].includes(proglang)) {
      optMaterials.innerHTML += `
        <li><p><a href="/wp-content/themes/pro-child/static-resources/${season}/en-us/Contracts.shtml?pathway=${pathway}">Contracts Reference</p></li>`
    }
  }
  ...
  function buildLessonSidebar(lesson, pathway, priorData) {
    ...
    const rightCol = document.getElementById('raw_code_container').parentNode.nextSibling;
    rightCol.children[1].insertBefore(sidebar, rightCol.children[1].firstChild);
    ...
    toolbar.firstChild.appendChild(sidebar);
  ...
  // if a #userInfo node exists, extract the userid
  // and add it to any quiz links
  function populateQuizLinks(lesson) {
    const userInfo = document.getElementById('userInfo');
    if(!userInfo) return;
    [...document.querySelectorAll('a.quiz')].forEach(a =>
      a.href += `?cookie=${userInfo.getAttribute('userid')}`);
  }
  ...
  if(!pathway || !pathwayTocs[pathway]) {
    if(!getPathwayLessons(pathway).includes(lesson)) pathway = false;
  }
  ...
  console.log(
    "pathway is", pathway,
    "lesson is", lesson,
    "pathwayLessons are", getPathwayLessons(pathway),
  ...
  try {
    populateQuizLinks(lesson);
  } catch (e) { console.log('populating quiz links', e); }
  ...
  function addPrereqs(prereqs) {
    ...
    prereqs.forEach(l => {
      if (!graph[l]) return;
      var li = document.createElement('li');
      li.innerHTML = '<p><a href="../'+l+'/?pathway='+pathway+'">'+graph[l].title+'</a></p>';
      ul.appendChild(li);
    })
  ...
  function addGroup(keyName, groupLabel) {
    ...
    for( const [dict, stds] of Object.entries(dictionaries[keyName]) ) {
      ...
      div.innerHTML = `<div class="title">
        <h4><a href="${stdUrl}">${fullname}</a></h4>
      </div><dl></dl>`;
...
if(typeof document !== 'undefined') {
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const bodyDivClasses = bodyDiv? [...bodyDiv.classList] : [];
    fixStretchTables();
    attachCodeMirror();
    if(!bodyDivClasses.includes("workbookpage")) {
      configureGraph(graph);
      //isNewest();
      attachCollapsible();
    }
    if(bodyDivClasses.includes("TeacherResources")) { populateTeacherResources(); }
    if(bodyDivClasses.includes("LessonPlan"))       { populateLesson();           }
  });
}
