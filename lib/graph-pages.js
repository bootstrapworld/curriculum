...
async function fetchUserHash() {
  ...
  return await postJSON('https://bootstrapworld.org/wp-admin/admin-ajax.php',formData)
...
function quizLinkRowHTML(quiz_path, teacherId, class_name, link_id) {
  return `<a href="${quiz_path}?groupId=${class_name}&teacherId=${teacherId}&assessmentId=${link_id}">${class_name}</a>`;
}
...
async function fetchNewQuizId(teacher_hash, quiz_path, class_name) {
  ...
  return await postJSON('https://bootstrapworld.org/data/GenerateAssessmentLink.php', formData)
  .then(data => {
    if (data.link_id) {
      return data.link_id
    } else {
...
async function makeQuizLink(quizButtonLink, teacherId, quiz_path) {
  ...
  composerRow.innerHTML = `<input type="text" maxLength="40" placeholder="Name your class here and press Enter">`;
    ...
    function generateStarterFilesHTML(isOptional) {
      ...
      return list
        ...
        .map(name => {
          ...
          return `<li><p><a href="${getStarterFileUrl(name, proglang)}" target="_blank">${title}</a></p></li>`;
        })
        .join('');
    }
