...
async function populateQuizLinks() {
  ...
  quizLinks.forEach( link => {
    ...
    restoreQuizLinks(quizBox.querySelector('ul'), teacherId, quizLink);
  })
}

// Cache of in-flight/completed fetchExistingQuizLinks() calls, keyed by
// teacherId+quiz_path. The same quiz_path legitimately appears more than
// once on a lesson page (e.g. once in the materials list, once inline in a
// teacher note), and populateQuizLinks() fires a restoreQuizLinks() call for
// each occurrence without awaiting the others -- so we cache the PROMISE,
// not just the resolved value, to dedupe callers that race each other
// before the first request has even completed.
const quizLinksCache = new Map();
...
function fetchExistingQuizLinks(teacherId, quiz_path) {
  const cacheKey = `${teacherId}::${quiz_path}`;
  if (quizLinksCache.has(cacheKey)) return quizLinksCache.get(cacheKey);
  ...
  quizLinksCache.set(cacheKey, request);
  return request;
}

// Fetch + render existing links into `linkList`, oldest (valid_from) first.
async function restoreQuizLinks(linkList, teacherId, quiz_path) {
  const links = await fetchExistingQuizLinks(teacherId, quiz_path);
  links
...
// Append one class's quiz link to a quizBox's <ul>. Shared by newly-created
// links (makeQuizLink) and links restored on page load (restoreQuizLinks).
function appendQuizLinkRow(linkList, quiz_path, teacherId, class_name, link_id) {
  linkList.innerHTML += `<li><a href="${quiz_path}?groupId=${class_name}&teacherId=${teacherId}&assessmentId=${link_id}">${class_name}</a>`;
}
...
async function makeQuizLink(quizButtonLink, teacherId, quiz_path) {
  ...
  async function handleKeyDown(event) {
    if (event.key === "Enter") {
      ...
      appendQuizLinkRow(linkList, quiz_path, teacherId, class_name, quizId);
      reset();
