...
async function populateQuizLinks() {
  ...
  // Disable every quiz link immediately, before any async work -- otherwise
  // there's a window (however brief) where the raw, unguarded quiz link is
  // live and clickable while we're still checking login status. The
  // .quiz-pending CSS class backs this up with pointer-events:none in case
  // anything below throws before it gets to the real disabled/quizBox state.
  quizLinks.forEach(link => link.classList.add('quiz-pending'));
  ...
  quizLinks.forEach( link => {
    ...
    if(!teacherId) {
        link.classList.remove('quiz-pending');
