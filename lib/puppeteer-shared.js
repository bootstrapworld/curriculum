...
// Timeout Tuning
// pageTimeout -- worst-case for a single request on a single page (mathjax)
// navigationTimeout -- how long to wait for a page to finish loading before giving up
//                      and retrying it. Kept shorter than pageTimeout, since navigation
//                      hangs tend to be transient (network/resource stalls) rather than
//                      a sign the page genuinely needs more time -- so failing faster and
//                      retrying sooner is a net win, without putting slower (but legitimate)
//                      operations like PDF generation at risk of false failures.
// pagesPerChunk -- number of pages to a sign a single cluster task (e.g. - one browser page)
// chunkTimeout -- (worst-case for a single page) * (size of chunks), before entire task is aborted
// retryLimit - max number of times to redo the entire chunk (even if just one page fails). <=1
// retryDelay - time between retries (<=15ms)
// pageRetryLimit -- max number of times to retry a single page within its chunk before reporting it as failed
// pageRetryDelay -- time to wait between per-page retries
// protocolTimeout -- time we'll allow Chrome to freeze
const pageTimeout     = 20 * sec;
const navigationTimeout = 10 * sec;
const pagesPerChunk   = 10;
const chunkTimeout    = pageTimeout * pagesPerChunk * 1.5; // worst-case for every page, plus 50% headroom
const retryLimit      = 0;
const retryDelay      = 10 * sec;
const pageRetryLimit  = 10;
const pageRetryDelay  = 5 * sec;
const protocolTimeout = 3 * 60 * sec; // 3m!
...
module.exports = {
  ...
  navigationTimeout, pageRetryLimit, pageRetryDelay,
