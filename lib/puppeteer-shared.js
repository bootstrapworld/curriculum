...
const navigationTimeout = 25 * sec;
...
const blocked_domains = [
  ...
  'wp-admin/admin-ajax.php', // login-status check (fetchUserHash); pages loaded locally are never logged in, and admin-ajax.php is slow
];
...
const thingsToCache = [
  ...
  'https://www.bootstrapworld.org/fonts/rubik/rubik-variable-wght.woff2',
  'https://www.bootstrapworld.org/fonts/rubik/rubik-italic-variable-wght.woff2',
  ...
  async function addUrlToCache(url) {
    ...
    try {
      ...
      const reason = err.name === 'AbortError'
        ...
        : err.message;
      // Warn and move on: a page that requests this URL will just fetch it live
      // (see setupRequestInterception's fallback), so one bad URL shouldn't abort
      // the whole run.
      console.warn(`Warning: failed to cache URL "${url}": ${reason}`);
    } finally {
