...
function runBuild(natlang, changedPaths) {
  return new Promise((resolve) => {
    const env = { ...process.env, NATLANG: natlang };
    // Tell dir-hoist-timestamp.sh exactly which files our chokidar watcher
    // saw change, so it can skip re-stating every file under lessons/ and
    // pathways/ just to rediscover what we already know. Unset (e.g. the
    // very first build, called from main() before any watch events exist)
    // falls back to its normal full scan.
    if (changedPaths && changedPaths.size > 0) {
      env.AUTHOR_CHANGED_PATHS = [...changedPaths].join('\n');
    }
...
function makeBuildQueue(natlang, { onStart, onSuccess, onFailure }) {
  ...
  async function runNow() {
    ...
    onStart();
    const ok = await runBuild(natlang, currentBuildPaths);
    building = false;
