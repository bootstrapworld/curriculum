...
function makeBuildQueue(natlang, onSuccess) {
  let building = false;
  let queued = false;
  let timer = null;
  let cooldownUntil = 0;
...
// ---------------------------------------------------------------------------
// Live reload: a trivial SSE channel. The injected client script reconnects
// on its own (that's just how EventSource behaves), so a server restart is
// enough to pick back up once it's back.
// ---------------------------------------------------------------------------
...
async function main() {
  ...
  const buildQueue = makeBuildQueue(opts.natlang, broadcastReload);
