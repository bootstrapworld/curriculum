= Draft of merge commit message for resolution of curriculum issue #467

This merge addresses issue #467, replacing the original
`build-pathway` Bash shell script with a `Makefile`.
...
`massage-distribution-lesson` and `massage-course`. Both these
...
1. The primitives are then consolidated per lesson, using the
   external script `collect-primitives.sh`.
...
4. An external script `make-image-list.sh` collects all the image
   information from the lessons into a global image glossary at
   `en-us/images.js`.

5. An external script `make-pathways-tocs.sh` collects the ToC
...
`make-dependency-graph.sh` that gathers the generated info about
