...
async function processLessons() {
  ...
  await cluster.task(async ({ page, data }) => {
    ...
    for await (const url of urls) {
      ...
      const SCALE = 2;
      await page.setViewport({ width: 1000, height: 25000, deviceScaleFactor: SCALE });
      ...
      for (const {selector, prefix} of configs) {
        ...
        await Promise.all(elts.map(async (elt, i) => {
          ...
          const right  = Math.min(imgWidth,  Math.round((box.x + box.width)  * SCALE));
          const bottom = Math.min(imgHeight, Math.round((box.y + box.height) * SCALE));
