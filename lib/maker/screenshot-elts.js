...
async function processLessons(tabsPerCore=1) {
  ...
  let CACHE = {};
  async function addResponseToCache(response) {
  	const url = response.url();
  	console.warn('adding '+url)
	  try {
	    const buffer = await response.buffer(); // This must be awaited immediately
	    CACHE[url] = {
	    ...
	    delete CACHE[url]; // Remove 'PENDING' if buffering fails
	  }
	}
    ...
    await page.setRequestInterception(true);
		page.on('request', request => {
		  ...
		  if (blocked_domains.some(domain => url.includes(domain))) {
		    request.abort();
/*		  } else if (CACHE[url] && (CACHE[url] !== 'PENDING')) {
...
/*      const url = response.url();
      // don't cache images or HTML
			if(['document', 'image'].includes(response.request().resourceType())){ return; }      //console.log('updating cache with response from', url);
      if (!CACHE[url] && response.ok()) {
      	//console.log('caching', response.request().resourceType())
      	CACHE[url] = 'PENDING'; // Placeholder to stop other tabs from double-downloading
		    await addResponseToCache(response);
		  }
*/    });
