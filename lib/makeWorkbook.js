...
const prefix = process.env["TOPDIR"] + "/distribution/" + process.env["NATLANG"] +
...
async function makeLuluCover(frontCoverPath, backCoverPath) {
  const [coverWidth, coverHeight] = [17.652, 11.25];     // in inches, from lulu.com
  ...
  // read the files
  const frontCover = fs.readFileSync(prefix + "/" + frontCoverPath);
  const backCover  = fs.readFileSync(prefix + "/" + backCoverPath);
...
async function makeWorkbook(fileSpecList, pdfFile, longworkbook = false) {
  ...
  if(pdfFile == prefix+"/workbook/workbook.pdf") {
  ...
  for (fileSpec of fileSpecList) {
    ...
    if (longworkbook) {
      ...
      if(pageCount > 1 && !knownLongPages.includes(fileName)) {
        console.log(`WARNING: in ${process.env["TGTPATHWAY"]} ${pdfFile.split('/').pop()}, ${fileSpec.file} has ${pageCount} pages`);
      }
    }
    ...
    const landscape = {
      ...
      y: isRightHand ? 0 : page.getHeight(), // seems to give us a 0.5" top margin
