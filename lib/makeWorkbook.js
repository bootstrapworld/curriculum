...
async function makeWorkbook(fileSpecList, pdfFile, longworkbook = false) {
  ...
  async function paginate(page, fileSpec, isLandscape) {
  ...
  for (fileSpec of fileSpecList) {
    ...
    const pages = await workbook.copyPages(doc, doc.getPageIndices());
      pages.forEach(page => {
        workbook.addPage(page);
        paginate(page, fileSpec);
      });
    } else {
      ...
      paginate(page, fileSpec, isLandscape);
    }
  }
