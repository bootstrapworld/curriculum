...
async function makeWorkbookFromSpecList(specList, outputPath, longworkbook = false) {
  ...
  let coverSuffix = outputPath.match(/.*workbook(-.*.pdf)/);
  coverSuffix = coverSuffix && coverSuffix[1];
  if(coverSuffix && (pageCount > 2)) {
    ...
    // write the page
    await fs.writeFile(prefix + "/workbook/cover"+coverSuffix, coverBytes);
  }
  // write the book
