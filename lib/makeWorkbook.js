...
async function makeWorkbook(fileSpecList, pdfFile, longworkbook = false) {
  ...
  for (fileSpec of fileSpecList) {
    ...
    const isContracts  = fileSpec.file.includes("Contracts.pdf");
    const isBackMatter = fileSpec.file.includes("back-matter") || isContracts;
    ...
    if (longworkbook && (pageCount > 1) && !knownLongPages.includes(fileName)) {
      console.log(`WARNING: in ${process.env["COURSE_DIR"]} ${pdfFile.split('/').pop()}, 
        ${fileSpec.file} has ${pageCount} pages`);
    }

    async function renderPageBytesIntoBook(bytes) {
    ...
    if(knownMultiPages.includes(fileName)) {
      for (let i = 0; i < pageCount; i++) {
        ...
        await renderPageBytesIntoBook(bytes);
      }
    } else {
      await renderPageBytesIntoBook(bytes);
    }
...
async function makeAllWorkbooks() {
  await makeWorkbook(bmContractsFiles, prefix + "/resources/pages/bm-contracts.pdf");
  await makeWorkbook(bmContractsSolsFiles, prefix + "/resources/solution-pages/bm-contracts.pdf");
