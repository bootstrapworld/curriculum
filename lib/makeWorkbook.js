...
async function makeBook(courseJSON, opts, outputPath, readFn, writeFn) {
  ...
  for (const [idx, {path, kind}] of pageList.entries()) {
    ...
    pdfPages.forEach(async page => {
      ...
      if (kind == "front-matter") {
        ++frontPageCount;
        pageNum = "";
        pageNumStyle = { S: 'a' };  // alpha
      } else if (kind == "back-matter" ) {
        pageNum = romanize(++backPageCount);
        pageNumStyle = { S: 'r' };  // roman
      } else if (kind == "required") {
        pageNum = ++mainPageCount;
        pageNumStyle = { S: 'D' };  // decimal
      } else if (kind == "handouts") {
        pageNum = `(handout)`;
        pageNumStyle = { S: 'r', P: PDFHexString.fromText('handouts ')  };  // roman
      } else if (kind == "optional") {
        pageNum = `(optional page)`;
        pageNumStyle = { S: 'r', P: PDFHexString.fromText('optional ') };  // roman
      }
      paginationStyles[paginationStyles.length] = pageNumStyle;
  ...
  let requiredCount = 0, sectionCount = 0;
  let pagination = [0, { S: 'a' }];
//console.log(paginationStyles)
  for(idx in paginationStyles) {
    const style = paginationStyles[idx];

    // Keep a running count of required pages ("D" for "decimal"),
    // since they have to pick up where they left off
    if(style.S == "D") { requiredCount++; }

    // if it's more of the same, increment the section count
    if(style.S == pagination[pagination.length-1].S) {
      sectionCount++;
      //console.log('saw another', style.S, 'section count is now', sectionCount);
    // if it's a new style, add that style to the pagination
    } else {
      //console.log('saw a new style', style.S, 'starting new section after', sectionCount,'pages');

      // If the new style is a required page ("D" for "decimal"),
      // start counting from where we left off. Otherwise start from 1
      style.St = (style.S == "D")? requiredCount : 1

      // add the new style to our array
      //console.log('adding', [idx, style])
      pagination = pagination.concat([Number(idx), style])
      sectionCount = 1;
    }
  }
