...
async function makeCustomBook(pathwayName, lessonList, opts) {
  const config = {
    ...
    "lessons": lessonList,
    "back-matter": [
       // `Contracts.shtml?pathway=${pathwayName}&lessons=${lessonList}`,
...
function makePageList(config, options) {
  ...
  const defaultCourseJSON = {
    ...
    "lessons": [],
    "back-matter": [
      "Contracts.shtml?lessons=" + config.lessons.join(),
...
async function makeCover(page, title, font) {
  ...
  // compute subtitle
  const subtitle = `Student Workbook, ${capitalizeFirst(semester)} ${year}`;
...
async function makeBook(courseJSON, opts, outputPath, readFn, writeFn) {
  ...
  const customCover = pageList[0].path.includes('default-front-cover');
  ...
  for (const [idx, {path, kind}] of pageList.entries()) {
    ...
    pdfPages.forEach(async page => {
      ...
      // If it's the first page and we need to make the cover, 
      // add the course title and proglang
      if((idx == 0) && makeCover) {
        await makeCover(page, courseJSON.courseName, lato);
      }
