...
const latoPath = "lib/fonts/lato.medium.ttf";
...
async function makeTOC(lessons, pdfDoc, lato) {
  // make the page
...
async function makeBook(pageList, outputPath, onProcessPage, onFinish) {
  ...
  const latoBytes = await reader(latoPath);
  ...
  for (const [idx, {path, kind}] of pageList.entries()) {
    ...
    const bytes = await onProcessPage(path, idx);
...
async function reader(path, _) { return await fs.readFile(path); }
