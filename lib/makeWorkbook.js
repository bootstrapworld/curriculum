...
// unused for now - will be set if we want to include lesson plans
const makeMasterWorkbook = process.env["MASTERWORKBOOK"];

const { PDFDocument, PDFName, degrees, PageSizes } = require("pdf-lib");
const fontkit = require('@pdf-lib/fontkit');
const latoPath = "https://www.1001fonts.com/download/font/lato.medium.ttf";
...
function makePageList(config, reqPages, optPages, solns) {
  let pages         = [];
  courseJSON = { ...defaultCourseJSON, ...config };
  ...
  courseJSON.lessons.forEach(l => {
    ...
    lessonPages.flat().forEach(obj => 
      obj.path = `distribution/en-us/lessons/${l}/${solns? "solution-" : ""}pages/${obj.fileName}`);
...
async function generateLuluCover(pageList, pageCount, outputPath) {
  ...
  if(coverNeeded && (pageCount > 2)) {
    ...
    const frontCover = await fs.readFile(pageList[0].path);
    const backCover  = await fs.readFile(pageList[pageList.length-1].path);
...
async function makeTOC(lessons, pdfDoc, lato, noto) {
  // make the page
  ...
  lessons.forEach((l, i) => {
    const title = emojiStrip(graph[l].title).trim(); // trim emoji and leading spaces
...
async function makeBook(pageList, outputPath, onProcessPage, onFinish) {
  ...
  const latoBytes = await fetch(latoPath).then(async res => res.arrayBuffer());
  const lato = await tocDoc.embedFont(latoBytes, {subset: true});
  await workbook.embedFont(latoBytes, {subset: true});
  //const notoBytes = await fetch(notoPath).then(async res => res.arrayBuffer());
  //const noto = await tocDoc.embedFont(notoBytes, {subset: true});
  //await workbook.embedFont(notoBytes, {subset: true});
  ...
  for (const [idx, {path, kind}] of pageList.entries()) {
    ...
    pdfPages.forEach(async page => {
      ...
      if (kind == "front-matter") { 
        ++frontPageCount; // increment the counter, 
        pageNum = "";     // but don't display any pageNum string
      } else if (kind == "back-matter") { 
        pageNum = romanize(++backPageCount);
      } else if (kind == "required") { 
        pageNum = ++mainPageCount;
      } else if (kind == "handout"){
        pageNum = `(handout)`;
      } else if (kind == "optional") {
        pageNum = `(optional page)`;
      }
  ...
  // generate the cover file for Lulu
  await generateLuluCover(pageList, workbook.getPageCount(), outputPath);

  // Write the bytes
  onFinish(outputPath, pdfBytes);

  // Return the warnings array
  return warnings;
}

async function reader(_, path) { return await fs.readFile(path); }
async function writer(path, bytes){ return await fs.writeFile(path, bytes); }

// passes flags to makeBook: reqPages, optPages, solns, handouts, outputPath, onProcessPage
async function makeAllWorkbooks() {
  console.log('@@@@@@@@@@@@ Making books for ', prefix)
  let warnings = [];
  // required pages ONLY, with and without solutions - no handouts
  warnings.push(await makeBook(makePageList(config, true, false, false), prefix + "/workbook/workbook.pdf", reader, writer));
  warnings.push(await makeBook(makePageList(config, true, false, true),  prefix + "/resources/protected/workbook-sols.pdf", reader, writer));
  // optional pages ONLY, with and without solutions - no handouts
  warnings.push(await makeBook(makePageList(config, false, true, false), prefix + "/workbook/opt-exercises.pdf", reader, writer));
  warnings.push(await makeBook(makePageList(config, false, true, true),  prefix + "/resources/protected/opt-exercises-sols.pdf", reader, writer));
  // all pages, with and without solutions - no handouts
  warnings.push(await makeBook(makePageList(config, true, true, false),  prefix + "/workbook/workbook-long.pdf", reader, writer));
  warnings.push(await makeBook(makePageList(config, true, true, true),   prefix + "/resources/protected/workbook-long-sols.pdf", reader, writer));
