...
function makePageList(config, options) {
  ...
  courseJSON.lessons.forEach(l => {
    ...
    // check to see if we're at the root level or inside a lesson
    const loadedLesson = window && window.location.pathname.includes('lessons')
    // customize the path, pointing to pages or solution-pages
    lessonPages.flat().forEach(obj => 
      obj.path = `./${loadedLesson? "" :`lessons/${l}`}/${solns? "solution-" : ""}pages/${obj.fileName}`);
