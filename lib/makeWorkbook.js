...
async function makeWorkbookFromSpecList(specList, outputPath, longworkbook = false) {
  ...
  // If it's a standard (non-empty) workbook, generate the Lulu cover page
  if((outputPath == prefix+"/workbook/workbook.pdf") && (pageCount > 0)) {
    ...
    const backSettings  = { x: bleed * 72,                 y: bleed * 72 };
    const frontSettings = { x: (bleed + 8.5 + spine) * 72, y: bleed * 72 };
      
    // Read the cover files: front is first path, back is last path
    ...
    const dimensions = embeddedFront.scale(1.0); // front and back are the same dimensions
    ...
    fs.writeFile(prefix + "/workbook/cover.pdf", coverBytes);  
  }

  await fs.writeFile(path, bytes); // save file
}
