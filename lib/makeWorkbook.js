...
if(NODE_PROCESS) {
  ...
  graph   = require(topdir + "/distribution/en-us/lib/dependency-graph.js");
  PDFLib  = require("pdf-lib");
  fontkit = require('@pdf-lib/fontkit')
} else {
  prefix = "../../";
  //prefix = "/wp-content/themes/pro-child/static-resources/fall-2025/en-us/"
}
...
function computeLabels(paginationStyles) {
  const { PDFHexString } = PDFLib;
  let requiredCount = 0, pagination = [0, { S: 'a' }];
  for(idx in paginationStyles) {
    const pageNumStyle = paginationStyles[idx];

    // Keep a running count of required pages ("D" for "decimal"),
    // since they have to pick up where they left off
    if(pageNumStyle.S == "D") { requiredCount++; }

    // if it's a new style, add that style to the pagination
    if(pageNumStyle.S !== pagination[pagination.length-1].S) {

      // If the new style is a required page ("D" for "decimal"),
      // start counting from where we left off. Otherwise start from 1
      pageNumStyle.St = (pageNumStyle.S == "D")? requiredCount : 1

      // If a string prefix exists, covert to hex
      if(pageNumStyle.P) pageNumStyle.P = PDFHexString.fromText(pageNumStyle.P)

      // add the new style to our array
      pagination = pagination.concat([Number(idx)-1, pageNumStyle])
    }
  }
  return pagination
}
...
function makePageList(config, options) {
  const { reqPages, optPages, solns, handouts } = options;
  let pages         = [];

  // defaults for custom workbooks, or books without their own cover
  const defaultCourseJSON = {
    "front-matter": [
      (loadedLesson? prefix : "") + "lib/default-front-cover.pdf",
      (loadedLesson? prefix : "") + "lib/default-attributes.pdf",
    ],
    "lessons": [],
    "back-matter": [
      (loadedLesson? prefix : "") + "lib/default-back-cover.pdf",
    ],
    "outputPath" : ""
  };
  ...
  courseJSON.lessons.forEach(l => {
    ...
    if(handouts) {
      lessonPages.push(graph[l].handouts.map(f => ({...f, kind:"handout"})));
    }
    if(optPages) {
...
async function makeTOC(lessons, pdfDoc, font) {
  ...
  // since projects don't have wbpages, filter them out
  lessons = lessons.filter(l => !l.startsWith('project-'))
...
async function makeBook(courseJSON, opts, outputPath, readFn, writeFn) {
  const { PDFDocument, PDFName, degrees, PageSizes, PDFHexString } = PDFLib;
  const { addTOC } = opts;

  // set font paths
  const latoPath = (loadedLesson? prefix : "") + "lib/fonts/lato/lato.medium.ttf";
  const notoPath = "https://www.1001fonts.com/download/font/noto-emoji.regular.ttf";
  const centuryPath = (loadedLesson? prefix : "") + "lib/fonts/centurygothic.ttf";
  const centuryBPath = (loadedLesson? prefix : "") + "lib/fonts/centurygothic_bold.ttf";
  ...
  // Build up state for digital pagination, tracking the style for each page
  // start page numbering with lowercase letters
  let pageNumStyle, paginationStyles = [{S: 'a'}];
  ...
  for (const [idx, {path, kind}] of pageList.entries()) {
    ...
    pdfPages.forEach(async page => {
      ...
      // compute section counts, pageNum string, and pagination style
      if (kind == "front-matter") {
        ++frontPageCount;
        pageNum = "";
        paginationStyles.push({ S: 'a' });  // Lowercase alpha
      } else if (kind == "back-matter" ) {
        pageNum = romanize(++backPageCount).toUpperCase();
        paginationStyles.push({ S: 'R' });  // Uppercase roman
      } else if (kind == "required") {
        pageNum = ++mainPageCount;
        paginationStyles.push({ S: 'D' });  // Decimal
      } else if (kind == "handout") {
        pageNum = `(handout)`;
        paginationStyles.push({ S: 'r', P: 'handout ' });  // Lowercase roman
      } else if (kind == "optional") {
        pageNum = `(optional)`;
        paginationStyles.push({ S: 'r', P: 'optional ' });  // Lowercase roman
      }
      if(path.includes("back-cover")) pageNum = "";
  ...
  const pagination = computeLabels(paginationStyles);
  //console.log(pagination)

  // Add page catalog (page numbers when viewed in PDF reader)
  const pageLabels = workbook.context.obj({ Nums: pagination });
...
// If we're in node (building locally from make book),
// Grab the COURSE_DIR variable and generate all the different workbooks
if(NODE_PROCESS) {
...
if(typeof module !== 'undefined'){
  module.exports = {
    makeBooksFromConfigFile: makeBooksFromConfigFile,
