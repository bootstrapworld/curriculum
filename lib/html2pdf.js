...
async function convertPage(page, htmlFileSpec) {
  ...
  // Resolve against the PAGE's own URL, not the top-level en-us/ dir: a page's
  // links are relative to its own location, so e.g. "../../../lessons/..." from a
  // .../lessons/<lesson>/pages/ page must climb from there. Resolving against
  // en-us/ instead makes those "../" overshoot and drop the /materials/<sem>/en-us
  // prefix entirely.
  const pageSubpath = htmlFile.replace(/^(.*\/)?en-us\//, '');
  const pageUrl = `https://www.bootstrapworld.org/materials/${semester+year}/en-us/${pageSubpath}`;
