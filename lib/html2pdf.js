...
async function HtmlToPdf(htmlFileSpecs, tabsPerCore=2) {
  ...
  const cachable = (url) => thingsToCache.includes(url);
  ...
  await cluster.task(async ({ page, data }) => {
    ...
    page.on('request', async request => {
      ...
      if(!url.includes('file://') && !CACHE[url]) {
        if(cacheDebug) console.log(
          `\nCACHE MISS FOR ${url.slice(0,50)}:`,  
          ', which is not in\n  ', 
          Object.keys(CACHE).sort().join('  ,\n'));
      }
  ...
  if(warnings.length > 0) {
    warnings = [...new Set(warnings.map(w => w.split('/').slice(-3).join('/')))];
