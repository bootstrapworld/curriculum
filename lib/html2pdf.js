...
async function HtmlToPdf(htmlFileSpecs, tabsPerCore=1) {
  ...
  // If the URL isn't a file:// url, and includes a specific string, it's cachable
  const CACHE = {};
  const cacheDebug = false;
  function cachable(url) {
    return !url.startsWith('file://') && 
      ['.css', '.woff', '.js', 'css2'].some(str => url.includes(str))
  }
  ...
  await cluster.task(async ({ page, data }) => {
    ...
    page.on('request', async request => {
      ...
      REQ_TIMING[url] = Date.now();
            
      // We don't want to load at all. Abort!
      if (blocked_domains.some(domain => url.includes(domain))) {
        if(cacheDebug) console.log('ABORTING ', url);
        return request.abort();
      }

      // We've loaded it before. Use the cache!
      if (CACHE[url]) {
        if(cacheDebug) console.log(`RETRIEVING ${url} from cache`);
        await request.respond(CACHE[url]);
        return;
      }

      // An external resource we haven't loaded before? Log it!
      if(cachable(url)) {
        if(cacheDebug) console.log('NOT SAVING ' + url);
      }

      // An external resource we haven't cached? Log it!
      if(!url.includes('file://')) {
        if(cacheDebug) console.log(`CACHE MISS FOR ${url}:`, CACHE[url]);
      }
    ...
    page.on('response', async (response) => {
      ...
      if(!url.includes('file://')) {
        REQ_TIMING[url] = Date.now() - REQ_TIMING[url];
        if(cacheDebug) console.log(`${url} took ${REQ_TIMING[url]}ms to load`);
      }
      ...
      if(cachable(url)) {
        if(cacheDebug) console.log(`SAVING ${url} in cache`);
        CACHE[url] = {
        ...
        if(cacheDebug) console.log(`NOT caching ${url}`);
      }
    });
