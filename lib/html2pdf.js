...
const blocked_domains = [
  ...
  'play.google.com',
  'youtube.com',
  // this link is used by the makeQuizLink code, to fetch a hashed teacherId
  // we don't need it when printing workbooks!
  'https://beta.bootstrapworld.org/wp-admin/admin-ajax.php',
];
...
async function HtmlToPdf(htmlFileSpecs) {
  ...
  // Cache object and how to cache something
  const CACHE = {};
  async function addThingToCache(url) {
  ...
  await Promise.all(thingsToCache.map(addThingToCache))
  ...
  await cluster.task(async ({ page, data }) => {
    ...
    page.on('request', async request => {
      ...
      if(!(url.startsWith('file') || url.startsWith('data') || CACHE[url])) {
        console.error('This file was fetched from the web, instead of local cache:');
        console.error('"'+url+'"\n')
      }
