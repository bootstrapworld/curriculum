...
const removeFormatting = w => w
  ...
  .replace(/[\*\_\`\:]{1}$/, "")
  .replace("’", "'"); 
const isNumber = w => w.match(/^-?\d+(\.\d{1,2})?(%|in|em|px){0,1}?$/) || false;
...
process.stdin.on('data', (chunk) => {
  // break the text into words, and filter out stuff we shouldn't spellcheck
  ...
  const cleanedWords = uniqueWords
    ...
    .filter(w => !(doNotCheck(w) || isDirective(w) || isNumber(w)));
