...
(define *glossary-list*
  '(("accessor functions" "functions to extract values from a data structure")
    ...
    (("correlation" "correlations") "a single number somewhere between -1 and +1 that reports the direction and strength of the linear relationship between two quantitative variables")
    ...
    (("interquartile range" "IQR") "(IQR) is one possible measure of spread, based on dividing a data set into four quartiles. The values that divide each part are called the first (Q1), the median (Q2), and third quartiles (Q3)")
    ...
    ("linear regression" "modeling the relationship between a dependent variable, and an explanatory variable using a straight line")
    ...
    (("outlier" "outliers") "a data point that is unusually far above or below the line of best fit")
    ...
    ("predictor" "a function which, given a value from one data set, makes an educated guess at a related value in a different data set")
    ...
    ("quartile" "three values that divide a data set into four equal-sized groups")
    ("quantitative data" "data with values that measure some amount or quantity; don't necessarily have a fixed number of possible values")
    (("r" "r-value") "a number between −1 and 1 that measures the direction and strength of a linear relationship between two quantitative variables")
    ...
    (("response" "response variable") "the variable in a relationship that is presumed to be affected by the other variable")
    ...
    (("skew" "skewness") "lack of balance in a dataset’s shape, arising from more values that are unusually low or high. Such values tend to trail off, rather than separated by a gap (as with outliers)")
    ...
    ("spread" "how much values in a data set vary, either from one another or from the center")
