...
(define *glossary-list*
  '(("accessor functions" "functions to extract values from a data structure")
    ...
    (("axis" "axes") "a number line on a scatter plot, used to place points that represent pairs of values in datasets")
    ...
    (("category" "categories") "the set of possible values that a value in a categorical dataset can have")
    ("categorical data" "data whose values are qualities that are not subject to the laws of arithmetic.")
    (("circle of evaluation" "circles of evaluation") "a 'sentence diagram' of the structure of a mathematical expression")
    ("clause" "a question and its corresponding answer in a conditional expression")
    (("chart" "charts") "visual reprsentations of how values in a dataset are distributed")
    (("column" "columns") "set of values in a table of a particular type.  Every row has exactly 1 element in every column")
    (("combination" "combinations") "the number of possible arrangements in a collection of items where the order of the selection does NOT matter. (ABC and CBA would be considered the same combination)")
    ...
    (("contract error" "contract errors") "errors where the code makes sense, but uses a function with the wrong number or type of arguments")
    ...
    (("data types" "data type") "a way of classifying values, such as: Number, String, Image, Boolean, or any user-defined data structure")
    ...
    (("frequency" "frequencies") "how often a particular value appears in a dataset")
    ...
    (("interquartile range" "IQR") "(IQR) is one possible measure of spread, based on dividing a dataset into four parts. The values that divide each part are called the first quartile (Q1), the median, and third quartile (Q3). IQR is calculated as Q3 minus Q1.")
    ...
    (("mode" "modes") "the most commonly appearing categorical or quantitative value or values in a dataset")
    ...
    (("permutation" "permutations") "the number of possible arrangements in a collection of items where the order of the selection matters (ABC and CBA would be considered different permutations)")
    ...
    (("predictor function" "predictor") "a function which, given a value from one dataset, makes an educated guess at a related value in a different dataset")
    ...
    (("quartiles" "quartile") "three values that divide a dataset into four equal-sized groups")
    ...
    (("row" "rows") "horizontal elements in a table, consisting of some group of values; each table is a collection of rows")
    ...
    (("syntax error" "syntax errors") "errors where the computer cannot make sense of the code (e.g. - missing commas, parentheses, unclosed strings)")
    ...
    (("variable" "variables") "a letter or symbol that stands in for a value or expression")
    ("variable name" "name of the information that can be different each time a function is used")
    (("vertical axis" "y-axis") "the number line on a coordinate plane that runs from bottom to top, indicating values in that direction")
