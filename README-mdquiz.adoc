...
1. `git pull`
2. You're ready to rock `./build-mdquiz` once you write an assessment!
3. Navigate to `distribution/lessons/name-of-lesson/assessments/name-of-assessment/index.html` to see the assessment.
...
4. Two newlines (`\n\n`) are used to insert a new line in the middle of a string.

5. Images can be added to any field (prompt, answer, etc) with a double-newline `\n\n`, followed by the markdown syntax for adding an image. For example:

```
"prompt": {
  "prompt": The chart below shows an lr-plot of carnival worker salaries since 1915 \n\n ![image title for mouseover/screen-readers](./salary-lr-plot.png)`
  }
```

If the auto-generated size with a max width of 640 pixels is unsatisfying, image sizes can be specified using the following formatting

   ![image title](./pic/pic1_50.png =100x20)
...
7. MultipleChoice Questions can also be used as Multiple Select / Multi Check questions by declaring more than one answer. _Here's an example from the dot-plots formative assessment._

```
"prompt": {
  "prompt": "Consider how the dot plot below would be different if the student named Sebastian weren’t in the class. Select each statement below that correctly describes a new dot plot of the class’ names without Sebastian. ![](./names2.png)",
  "distractors" : [
...
When you complete the quiz WHAT HAPPENS?
