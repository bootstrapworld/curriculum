...
@n On the three lines below, write the code to lookup the `species` column from `dog-row`, `old-row`, and `young-row`.
...
@n On the three lines below, write the code that to use the `text` function to show the species of those same three rows in __red, 15px letters__.
