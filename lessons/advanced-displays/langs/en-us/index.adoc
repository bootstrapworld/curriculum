...
But how do we process each Row separately? You've learned how to take a complex value like a Row, and lookup a column. We could easily write a line of code that does that for every Row in the table, but that would take a long time and be a LOT of code! But you've also seen how to define functions that consume simple values, and __do all the repetitve work for us__. Let's put those two together, and define functions that consume Rows!
...
Invite students to share their ideas and explain their rationale.
...
- Discuss as a class, or in small groups: what is the Contract for `image-scatter-plot`?
- Discuss as a class, or in small groups: what do you think the resulting scatter plot will look like? You may want to refer back to @printable-exercise{age-dot.adoc}.
- Complete the remaining questions on @printable-exercise{image-scatter-explore.adoc}.
...
* the `image-scatter-plot` function can label each point with a custom image, computed by a helper function that consumes a Row and produce an Image.
