...
For each function, draw a second Circle of Evaluation that changes the order of the arguments. Translate both Circles of Evaluation to code, and take a moment to predict what both expressions will return in @proglang. Then test your code and record the result.

== Is @show{(code 'beside)} Commutative?   Does the order of the arguments matter?
...
== Is @show{(code 'above)} Commutative?   Does the order of the arguments matter?
...
== Is @show{(code 'overlay)} Commutative?   Does the order of the arguments matter?
