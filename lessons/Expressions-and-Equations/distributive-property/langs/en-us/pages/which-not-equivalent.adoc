...
@show{(coe '(+ (* 5 2) (* 5 4)))}
@show{(coe '(* 5 6))}
| @ifsoln{The second expression adds 4 to @math{5 \times 2}. +
The rest multiply 5 by @math{2 + 4}.}
...
@show{(coe '(+ (* 7 3) (* 7 m)))}
| @ifsoln{The third circle is neither a sum of products nor a product of sums.}
...
| @ifsoln{The fourth circle swaps 45 and `h`, but subtraction is not commutative.}
