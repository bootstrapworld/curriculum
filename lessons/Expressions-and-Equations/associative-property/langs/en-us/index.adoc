...
=== Overview
Students consider whether various image-producing @proglang functions are associative and will produce the same output, regardless of how they're grouped.

=== Launch
@slidebreak{Launch}

@lesson-instruction{
- Turn to @printable-exercise{associativity-and-code.adoc} and open the @starter-file{expressions-equations}.
- With your partner, make a prediction about whether the order in which the groupings are made will change the output for each function.
}
...
@QandA{
@Q{On @printable-exercise{associativity-and-code.adoc}, we discovered that `overlay`, `beside`, and `above` were all associative. Can you think of any @proglang functions that are _not_ associative?}
...
- Now try @printable-exercise{equivalent-or-not.adoc}. Determine if each equation is true, and be sure to explain your response.
}
