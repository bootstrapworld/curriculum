...
@slidebreak{Launch}@ifslide{OPTIONAL: }
...
@lesson-instruction{
- Turn to @printable-exercise{translating.adoc}. Each row represents a single arithmetic expression, written in three different forms. Complete each row so that it contains *two verbal expressions*, a Circle of Evaluation, and the arithmetic expression that they would all translate to.
- Then complete @printable-exercise{matching-words-to-circles.adoc}.
}
