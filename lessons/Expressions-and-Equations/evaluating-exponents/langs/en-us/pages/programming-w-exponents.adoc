...
td { padding: 0em 0px !important }
td .fitb { min-width: 1em; }
td { --blank-height: calc(0.2em + 1em); }
...
@n Open the @starter-file{is-it-16} and click "Run". @ifproglang{pyret}{Select "Show Details" to the right of example block `A`.} Which tests failed? @fitb{}{@ifsoln{2, 3, and 6.}}

@n The three failing examples are all wrong for the same reason. That's because the students who wrote them don't understand something about how exponents work! What do they not understand?? @fitb{}{} +
@blanklines{2}{
@ifsoln{Rather than multiplying the base times itself as many times as the exponent indicates, they multiplied the base times the exponent.}
}
