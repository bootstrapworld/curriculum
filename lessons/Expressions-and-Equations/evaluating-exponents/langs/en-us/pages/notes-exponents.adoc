...
| @big{@math{2^5}}
|
| @big{@math{2 \times 2 \times 2 \times 2 \times 2}}
...
In multi-step exponent expressions with no grouping symbols, we evaluate the exponent before the other operations. The two expressions below are *not* equivalent because the parentheses influence the order in which we evaluate.
