...
- Let's collect all of the Circles of Evaluation you drew using @math{2, 4} and @math{8} on the board. Please add them one at a time, making sure not to repeat someone else's contribution. 
  * @math{10 + 6}, for instance, is not exactly the same as @math{6 + 10} -- so, if we were including circles with @math{6} and @math{10}, we would want to see both @show{(coe '(+ 6 10))} and @show{(coe '(+ 10 6))} on the board.
