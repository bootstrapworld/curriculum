...
@lesson-instruction{
- Practice applying `negate` and finding @vocab{opposites} on @printable-exercise{true-or-false-negate.adoc}. 
- Evaluate each expression to determine whether or not they are equivalent.
- Then complete @printable-exercise{true-or-false-negate-2.adoc}. You'll be exploring Circles of Evaluation with @vocab{Variables} and reviewing properties (Identity, Commutative, Associative, and Distributive).
}
