...
@lesson-prereqs{additive-inverse}
...
@QandA{
@Q{How would you read this Circle of Evaluation out loud? @show{(coe '(negate 2))}}
@A{The opposite of 2.}

@Q{What does it evaluate to.}
@A{@math{-2}}
}
...
@QandA{
@Q{What happens if we nest a negating Circle of Evaluation _inside of_ another negating Circle? How would you translate this Circle of Evaluation into words? And what does it evaluate to? @show{(coe '(negate (negate 2)))}}
@A{The opposite of the opposite of the opposite of 2. It evaluates to @math{2}.}
@A{The opposite of the opposite of any number is the number itself.}
}

@slidebreak{Investigate}

@QandA{
@Q{What do each of these Circles of Evaluation evaluate to? Be sure to translate them into words for yourself as you think about them.}
...
@lesson-instruction{
- Practice applying `negate` and finding @vocab{opposites} on @printable-exercise{equivalent-or-not-negate.adoc}. 
- Evaluate each expression to determine whether or not they are equivalent.
- Then complete @printable-exercise{equivalent-or-not-negate-2.adoc}. You'll be exploring Circles of Evaluation with @vocab{Variables} and reviewing properties (Identity, Commutative, Associative, and Distributive).
}
...
In math we want students to read for _comprehension_, not just mechanically sounding out each symbol in order. We read the @vocab{arithmetic expression} @math{60 - 45} as "60 minus 45" _because subtraction is the "verb"_, not because the minus sign happens to be in the middle!

When we encounter a value such as @math{-45}, however, it is confusing and misleading to name that value "minus 45." Subtraction is not happening here -- negation is! 

Being precise with how we read @math{-45} sets the stage for being able to make sense of more complex expressions like @math{-60 - -45}.
}
...
@QandA{
@Q{How could we find the distance between   @math{-8}   and   @math{5}   on the number line?}
@A{Give students a minute to contemplate, and then invite them to verbally share their strategies. Record students' thinking on the board, annotating the number line. All strategies are welcome.}
@A{Especially of interest is any discussion that hones in on the idea of _the distance of a number from zero._}
}

We have a term for _the distance of a number from zero_ -- it's @vocab{absolute value}.

@lesson-point{Absolute value is the (positive) distance of a number from zero.}
...
@QandA{
@Q{Let's try evaluating some expressions involving absolute value. What does each of these Circles of Evaluation evaluate to?}
...
@lesson-instruction{
- On @printable-exercise{equivalent-or-not-abs-val.adoc}, compare expressions with `abs` to expressions with `negate`
...
- Complete @printable-exercise{wodb-abs-val-negate-2.adoc}. Cross out any Circle of Evaluation that does *not* meet the condition stated on the left.
}
...
@Q{Describe what a 90-degree turn looks like.}
...
@A{A 180-degree turn rotates the image two quarter-turns, or halfway back to its starting position, effectively flipping the image upside down.}
