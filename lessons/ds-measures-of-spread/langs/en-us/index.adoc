...
=== Overview
Students assess the degree of visual overlap of two numerical distributions with similar variabilities.

=== Launch
Before we start answering *Relate Questions*, such as "do older animals take longer to get adopted?", we need to compare the spread of those two variables. Box plots make it easy to compare the variability between two sets of values.

=== Investigate
Have students break into groups of 3-4, and compare the box plots for `age` and `weeks`. Which one has the higher median? What is the overlap between them?

*Note:* they can either generate these plots themselves, or use @link{images/side-by-side.png, this side-by-side image}.
