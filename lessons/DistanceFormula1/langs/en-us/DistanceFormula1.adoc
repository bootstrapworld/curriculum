...
Have students write a function `distance` that takes in two coordinate pairs (four numbers) of two characters (x1, y1) and (x2, y2) and returns the distance between those two points.
...
Have students write a function 'collide?' that takes in two coordinate pairs (four numbers) of two characters (x1, y1) and (x2, y2) and returns a Boolean as to whether or not the two characters have gotten within 50 pixels of each other.  
