...
@fitb{}{@ifsoln{A dot plot forms, categorizing animals as fixed (true) and not fixed (false). The labels for the dot plot are on the y-axis. The dots are stacked horizontally.}}
...
@fitb{}{@ifsoln{When I select "count," I see the number of animals represented by each bar. When I select "percentage," the percentage of animals represented by each bar appears.}}
...
@fitb{}{@ifsoln{Bar charts show categories, therefore are conducive to representing categorical data. They classify, rathern than measure}}
