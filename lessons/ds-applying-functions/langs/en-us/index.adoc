...
- What is the name of this function?
- What do we think the expression @show{(sexp->code `(sqrt 16))} will evaluate to?
...
=== Synthesize
Debrief the activity with the class. What kind of value was produced by that expression? (An Image! New data type!) Which error messages were helpful? Which ones weren't?
...
When students typed `triangle(50, "solid", "red")`, they created an example of a new data type, called an _Image_.
