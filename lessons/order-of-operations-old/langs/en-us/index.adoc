...
order-of-operations-old/langs/en-us/index.adoc
...
Remember, we're only trying _numbers_ for now, not operations like @smath{(- 3 6)}, @smath{(sqrt 16)} or @smath{(expt 4 2)}
--
...
A mathematical expression is like an instruction: the expression @smath{(+ 4 5)} tells us to _add_ 4 and 5. To evaluate an expression, we follow the instruction. The expression @smath{(+ 4 5)} evaluates to 9.
...
Mneumonic devices for Order of Operations like PEMDAS, GEMDAS, etc. focus on how to compute the answer. What we need is a __better way to read math__.

==== Circles of Evaluation _help us read math_

Instead of a rule for computing answers, let's start by diagramming the math itself! We can _draw the structure_ of a mathematical expression using something called the *Circles of Evaluation*. The rules are simple:
...
Optional: 
If you are looking for more practice with Circles of Evaluation before introducing code, we have lots of options!
...
The Circles of Evaluation are a critical pedagogical tool in this course. They place the focus on the _structure_ of expressions, as a means of combating a harmful over-focus on _computing the answer_. They can be used to diagram arithmetic sentences to expose common misconceptions about Order of Operations, and make an excellent scaffold for tracing mistakes when a student applies the Order of Operations incorrectly. They are also a bridge representation, which naturally connects to function composition and converting arithmetic into code.
|===
...
@ifproglang{pyret}{
*Note:* In Pyret, we treat _operators_ like `+`, `-`, `*`, and `/` differently - they are written in between their inputs, just like in math. }

We also use letters instead of symbols for function names, so taking the square root is written as @show{(sexp->code `sqrt)} and squaring is written as @show{(sexp->code `sqr)}.
