...
| Materials
|
* Lesson slides (@link{https://docs.google.com/presentation/d/1YPbKXTTIaA8CuWr_KhG6vXLJ6u5L5IBM7cuFqMS5zy8/, Google Slides})
...
That's a lot of code, and it also requires us to come up with names for each intermediates step! Pyret allows table methods to be _chained together_, so that we can build, filter _and_ order a Table in one shot. For example:
