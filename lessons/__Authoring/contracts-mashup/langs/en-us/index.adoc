...
Let's play with a function to get the hang of it. @ifselfguided{Be sure to read any error messages you get back carefully! They might have clues to help you understand what's going on.}
...
@ifselfguided{
@htmlverb{<input type="button" value="Click here" onclick='addToEditor(&#96; string-length("rainbow") &#96;)' />} to add @show{(code '(string-length "rainbow"))} to the editor.

What do you think the `string-length` function does? How many arguments does it expect? What does it produce? 

@vspace{1ex}
What do you think @show{(code '(string-length "good work!"))} will produce? 

@htmlverb{<input type="button" value="Click here" onclick='addToEditor(&#96; string-length("good work!") &#96;)' />} to add it to the editor and test it out.

What do you think @show{(code '(string-length 215))} will produce?

@htmlverb{<input type="button" value="Click here" onclick='addToEditor(&#96; string-length(215) &#96;)' />} to add it to the editor and test it out.

What happened? Why?
}

@ifnotselfguided{Be sure to read any error messages you get back carefully! They might have clues to help you understand what's going on.}
...
@scrub{NOTE: INDENTATION IN THE EDITOR CONFIG RESULTS IN INDENTATION IN THE DEFINITIONS AREA.}
@editorconfig{{
...
@QandA{
@Q{How would you explain Domain and Range to someone who missed today's class?}
@Q{What questions do you have about Contracts?}
}
...
@ifnotlessonplan{Doesn't show in lesson plan, shows everywhere else}
...
@slidebreak{Investigate}
// there was an empty imageconfig here
...
| @ifnotselfguided{@show{(code expd)}}
|
| @show{(code exprd)}
|===
...
@ifselfguided{What do you Notice? What do you Wonder?}
...
@htmlverb{<input type="button" value="Click here" onclick='addToEditor(&#96; sample-logo-1 &#96;)' />} to add @show{(code 'sample-logo-1)} to the editor.

@htmlverb{<input type="button" value="Click here" onclick='addToEditor(&#96; sample-logo-2 &#96;)' />} to add @show{(code 'sample-logo-2)} to the editor.

@htmlverb{<input type="button" value="Click here" onclick='addToEditor(&#96; sample-logo-3 &#96;)' />} to add @show{(code 'sample-logo-3)} to the editor.
}
...
- Describe how the function `overlay` works as if a friend missed class and needed to know.
...
@imageconfig{images/frequency-table.png}
@scrub{DORAI - Does it really make sense for us to write @imageconfig{images/frequency-table.png} here? Or should the image also be implicit from the lesson plan?}
...
@ifselfguided{
SG page continues in self-guided book!
