...
== Rules for Labeling Tables
[.no-flex-section]

* For autonumbering inside a table
++++
<style>
/* remove parens from autonums */
...
Every header on a page defines a "section", and these sections can be identifies as `no-flex` or not. 
BUT if the first part of the page doesn't have a header, AsciiDoctor calls it a "preamble", and it needs to be styled separately. 

* For pages with lots of empty space at the top, when no part of the page should use flexible fill vertical space
++++
<style>
...
* For pages with several sections where you don't want the top to grow
++++
<style>
...
* To shrink vertical spacing on fitb, fitbruby, and any blanklines sharing
  the page (they follow the same height, so a blanklines block keeps
  matching what repeated fitbs would look like)
++++
<style>
...
* To shrink vertical spacing on fitb/fitbruby ONLY, deliberately leaving any
  blanklines on the page at the default height
++++
<style>
.fitb, .fitbruby { --fitb-height: calc(0.5rem + 1em); }
</style>
++++
...
* To standardize or control row heights on tables
++++
<style>
/* Force fractional row heights on the 6 body rows, making them 4x the height of the header */
  .FillVerticalSpace { grid-template-rows: 1fr 4fr 4fr 4fr 4fr 4fr 4fr !important; }
</style>
++++
...
* Circles of Evaluation are inline (shrink-to-fit) by default, so they already sit alongside other content. To instead force a circle onto its own line (e.g. one that ends a sentence in running prose, where it would otherwise glue onto the end of the text)
++++
<style>
.circleevalsexp { display: block; }
</style>
++++
