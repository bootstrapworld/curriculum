...
export let snippetToNode = ({ snippet, language, lineNumbers }) => {
...
export let renderIde = (container, options) => {
    if (!rustEditor.Editor)
        return;
    ...
    let roots = ideNodes.map(node => {
        ...
        let darkMode = ["navy", "coal", "ayu"].some(theme => document.documentElement.classList.contains(theme));
        let newEl = document.createElement("div");
        let root = ReactDOM.createRoot(newEl);
        let gutterConfig = options?.lineNumbers
            ? {
        ...
        root.render(React.createElement(rustEditor.Editor, { contents: s, highlights: marks, disabled: true, exactHeight: true, editorOptions: {
        ...
        node.parentNode.replaceWith(newEl);
        return root;
    });
