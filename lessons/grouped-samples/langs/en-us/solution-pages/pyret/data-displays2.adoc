...
|=== 
*code:* @fitb{}{@ifsoln{`pie-chart(animals-table.filter(is-dog).filter(is-heavy), "fixed")`}}
...
*code:* @fitb{}{@ifsoln{`histogram(random-rows(animals-table, 20), "weeks", 2)`}}
...
*code:* @fitb{}{@ifsoln{`box-plot(animals-table.filter(is-cat).filter(is-young), "pounds")`}}
