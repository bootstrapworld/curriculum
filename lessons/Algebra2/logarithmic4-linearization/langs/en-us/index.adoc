...
Now that we know how the model settings in our linear model of transformed data relate to the model settings in our logarithmic model, let's return to Pyret to run linear regression on the transformed data, find the @vocab{best possible linear model}, and use the model settings to define our *optimal _logarithmic_ model!*
