...
@lesson-instruction{
- Open the @starter-file{alg2-countries}, and turn to @printable-exercise{logarithmic-models.adoc}.
...
@slidebreak{Launch}
@QandA{
@Q{What kind of data does the function `g` consume?}
@A{A Row}

@Q{Given a Row, what does `g` do to its input?}
@A{It extracts the `pc-gdp`, and produces the log of the value}

@Q{What does `countries-transformed = build-column(countries-table, "log(pc-gdp)", g)` do?}
@A{It defines a new table called `countries-transformed`, which is the same as the original table but with a new column called `log(pc-gdp)` added on the right. This column is built using the function `g`, and contains the log of each country's `pc-gdp`}

@Q{Compare the slope and y-intercept of the optimal linear model you made from `lr-plot` to the slope and y-intercept you estimated for `logarithmic2` on @printable-exercise{logarithmic3-changing-scale/pages/fitting-logarithmic-models.adoc}}.
@A{They are both very similar!}
}

@slidebreak{Launch}
@lesson-point{
The @vocab{slope} in the transformed, linear model is the same as the @vocab{log coefficient} in the __un__-transformed logarithmic model.
}

@lesson-point{
The @vocab{vertical shift} in the transformed, linear model is the same as the @vocab{vertical shift} in the __un__-transformed logarithmic model
}
...
1. Defined the transformation function @math{g} to produce the log of the `pc-gdp` column.
...
@right{@image{images/lr-plot-transformed.png, 350}}Transforming the `pc-gdp` column with a log function produces a scatter plot showing a linear pattern in the data!

Pyret's `lr-plot` tool computes the optimal (best possible) linear model for our transformed data:

@indented{@math{y = 11.9012x + ~24.2636}}

Our @math{S} has dropped to 4.48, showing a much better correlation than before.
...
- At each point in our optimized @vocab{linear model}, @math{y} is the _predicted median lifespan_, and @math{x} is the _log of per-capita gdp in dollars_.
...
@right{@image{images/logarithmic-model.png, 350}}The model settings used in the transformed, linear model are the same ones used in the logarithmic, __un__-transformed model, but one of them changes its meaning:
