...
- the @vocab{slope} in the transformed, linear model is the same as the *log coefficient* in the untransformed logarithmic model
...
Now that we know how the model settings in our linear model of transformed data relate to the model settings in our logarithmic model, let's return to Pyret to run linear regression on the transformed data, find the best possible linear model (@vocab{the line of best fit}), and use its model settings to define our *optimal _logarithmic_ model!*
