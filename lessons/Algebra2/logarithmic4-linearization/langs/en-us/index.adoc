...
@teacher{Sync or pace students to *Wealth-v-Health: Scaling the axes from pc-gdp to log(pc-gdp)* of @starter-file{alg2-log-desmos}.}

@lesson-instruction{
- Let's return to the *Fitting Wealth-v-Health and Exploring Logarithmic Models Desmos file*.
- You should now be on Slide 8: *Wealth-v-Health: Scaling the axes from pc-gdp to log(pc-gdp)*.
...
Our @math{S} has dropped to 4.4786, showing a much better correlation than before.
