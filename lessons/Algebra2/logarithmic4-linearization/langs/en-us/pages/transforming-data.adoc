...
[.nomargin, cols="1a, 2a, 1a", frame="none"]
|===
| 
`Wealth vs. Health` 
...
-  **@math{g(x)}** -- a function! +
takes in a number @math{x} and produces @math{log_{10}(x)}
-  a new table!
...
*Notice:* When the slide first loads the red dots from the `Wealth vs. Health` table are dispersed on the graph exactly as they were on Slide 5!

|===

@n Each of the folders contains a table. Let's take a moment to compare them.

[cols="4a,5a,10a", frame="none", grid="none", stripes="none"]
|===
| `Wealth vs. Health`
| `Log(Wealth) vs. Health`
...
@blanklines{6}{@ifsoln{The 2nd columns are the same, but the 1st columns are really different. The header for the x-column of the 2nd table is a _function_ that takes in the @math{x_1}-value from the first table. The first table is the same one we've seen before.}
}
...
- Click on the corresponding circle in front of the `Log(Wealth) vs. Health` folder to turn ON the points from its table.

@n How does the new point cloud look different from the original? @fitb{}{@ifsoln{All the black datapoints are smashed against the y-axis!}}
...
@n Transforming the *x-values* makes our data look @fitbruby{12em}{@ifsoln{linear}}{linear / quadratic / exponential}. Why? @fitb{}{@ifsoln{The amount of extra income needed to create an extra year }}

@fitb{}{@ifsoln{of predicted lifespan increased exponentially. By taking the log of that income we are "canceling out" that exponential growth and making it linear.}}
