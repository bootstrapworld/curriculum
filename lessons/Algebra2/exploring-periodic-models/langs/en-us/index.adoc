...
=== Investigate
@lesson-instruction{
- Open the @starter-file{alg2-co2}, and complete Questions 1-5 of @printable-exercise{exploring-co2.adoc}.
...
@Q{A periodic function completes one cycle every @math{2\pi}. How long is one period in _years?_}
@A{One year (this makes sense, since the seasonal cycle repeats every year!)}
@Q{What did you get for @math{b}?}
@A{@math{2\pi}, because @math{{2\pi \over 1.0} = 2\pi}.}
}
