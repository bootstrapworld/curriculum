...
@Q{Using straight lines to connect these dots is an *approximation*. Why might this approximation not be accurate?}
@A{We don't know for sure what the data looks like between those points, and we certainly don't know if it's linear!}
@Q{DO you think there's a relationship between `time` and `altitude`? Why or why not?}
...
@QandA{
In our clock example, we divide the circle into twelve "slices", each representing one hour (Can you picture a "15m slice"? A "45m" slice?). Of course, there are other ways besides 12 slices of "hours" to measure this angle!
...
** _Degrees_, divide a circle up into 360 slices instead of 12. What kind of slice would 90 degrees look like? 45 degrees?
** _Compass Directions_ like North, South, East, and West, which divide our circle up into 4 slices instead of 12. Can you picture the distance between North and Southeast?
}
