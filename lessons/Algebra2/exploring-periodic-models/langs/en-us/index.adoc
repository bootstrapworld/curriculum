...
Your teacher's seat on the Ferris Wheel can be thought of as a point on the edge of a rotating circle. If we can compute the y-coordinate of that point, we'll know the altitude of the seat! But how can we compute the y-coordinate, based on the rotation of the circle?
...
For any angle of rotation, we can draw a right triangle that crosses through both the point on the edge and the center of the circle. We can talk about angles using degrees or radians, but radians tend to be the easiest to reason about because they use multiples of @math{\pi}. **For this lesson we'll be working in radians.**
...
- @vocab{Frequency} (@math{b}): the number of cycles in a given interval (can be divided by @math{2\pi} to compute the @vocab{period})
