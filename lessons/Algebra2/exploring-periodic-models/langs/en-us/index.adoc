...
== Around and Around We Go @duration{45 minutes}
...
What ride do you think they were on?
...
Starting at some initial value at @math{x=0}, a periodic function will rise or fall as @math{x} increases until it reaches some maximum (@vocab{peak}) or minimum value (@vocab{trough}), before returning to its initial value. As @math{x} continues to increase, it will repeat this pattern over and over, fluctuating between its maximum and minimum value over regular intervals (@vocab{periods}).
...
@lesson-instruction{
- Now that you have a sense for what terms like @vocab{amplitude} and @vocab{midline} mean, complete @printable-exercise{matching-periodic-descriptions.adoc} by matching the graphs of periodic functions to their written descriptions.
...
@lesson-instruction{
- With your partner, complete @printable-exercise{fitting-ferris-wheel.adoc} using the last slide of the Desmos activity.
}
