...
Try changing the value of @math{k}, then @math{a}, then @math{b} to find three promising exponential models, graphing each one and labeling your values on the graphs below. Remember: {nbsp} @vocab{growth rate} (@math{r}) can be calculated from the base: @math{\textit{base} = (1 + \textit{rate})}
