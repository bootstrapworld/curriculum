...
Try changing the value of {nbsp}@math{k}, {nbsp} then {nbsp}@math{a},{nbsp} then {nbsp}@math{b} {nbsp} to find three promising exponential models, graphing each one and labeling your values on the graphs below. Remember: {nbsp} @vocab{growth rate} (@math{r}) can be calculated from the base: {nbsp} @math{\textit{base} = (1 + \textit{rate})}
