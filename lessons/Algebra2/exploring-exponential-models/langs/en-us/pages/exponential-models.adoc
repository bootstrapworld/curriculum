...
* _You'll notice that @math{x} is being multiplied by `~1`. This special code tells Pyret to prioritize efficiency over precision (in this instance only) and round to 10 decimal places (as most calculators do). We've added this to the function definition so that you won't have to wait for several minutes for Pyret to run `fit-model` to get an answer for question 4._
