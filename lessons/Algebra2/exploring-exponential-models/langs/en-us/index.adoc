...
// in the slide deck, we should show stuents the table without the Tourists column... then advance the slide, and include the Tourist column //
...
We observe the same phenomenon in our Covid data. There is a relationship that we can clearly see _within_ groups, but that relationship almost disappears when we lump all the groups together.
...
Exponential functions, like the ones you identified on tables 2 and 6, can be written in the form @math{y = ab^x}.
...
Sometimes, we write exponential functions like this: @math{y=ab^x + k}.
...
** _Vertical shift (@math{k}) refers to the point where the line levels out. Adjusting this value shifts the curve up and down._
- Graphically, what does @math{b} tell us about an exponential function?
** _Base (@math{b}) determines whether the function growth is positive (@math{b > 1}), flat (@math{b = 1}), or negative (@math{b < 1}). See table below._
...
A base if @math{1} means that @math{y} will remain constant.
