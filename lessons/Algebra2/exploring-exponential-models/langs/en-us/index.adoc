...
@teacher{Make sure you've:

- set your teacher dashboard of @starter-file{alg2-covid-desmos} to the first slide so that students are looking at the "Quadratic Models" screen 
...
- Use it to complete the first section ("base") of @printable-exercise{graphing-models.adoc}.
}
...
| @math{b = 1} +
When the base is *equal* to 1, the function stays flat without any growth at all (raising 1 to _any_ power will always produce 1!).
