...
- Working in pairs or small groups, complete questions 1-9 of @printable-exercise{exploring-covid.adoc}
...
Students explore their newly-filtered `MA-covid` dataset, trying to different kinds of models to it.
...
@right{@image{images/MA-covid.png, 300}} Quadratic models change their rate of growth over time, which definitely made them a better fit than linear ones. But they still don't have the explosive growth we need to model this data, which starts out incredibly slow and then suddenly takes off like a rocket!
