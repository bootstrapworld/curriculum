...
=== Investigate
Let's start out by looking at _just one state_, and fitting a linear model to it! We'll use Massachusetts for this investigation, but you can do your own investigation about any state you like after finishing this one!

@lesson-instruction{
Complete questions 1-2 from @printable-exercise{linear-models.adoc}.
}
...
@lesson-instruction{
Complete the rest of @printable-exercise{linear-models.adoc}.
}
...
@teacher{Make sure you've set your teacher dashboard of @starter-file{alg2-covid-desmos} to the first slide so that students are looking at the "Quadratic Models" screen.}

@lesson-instruction{
- Complete @printable-exercise{quadratic-models.adoc}, using the first slide of @starter-file{alg2-covid-desmos}.
...
- The data starts out really slowly, and then grows incredibly fast. Can you think of any other situations in real life that act like this?
...
@teacher{Make sure you've advanced your teacher dashboard of @starter-file{alg2-covid-desmos} to the second slide ("Exploring Exponential Functions") so that students are looking at the correct screen.}

@lesson-instruction{
Use the second slide ("Exploring Exponential Functions") of @starter-file{alg2-covid-desmos} to complete the first section ("base") of @printable-exercise{graphing-models.adoc}.
}
...
@lesson-instruction{
Use the same slide ("Exploring Exponential Functions") to complete the second section ("vertical shift") of @printable-exercise{graphing-models.adoc}.
}
...
@lesson-instruction{
Using the same slide ("Exploring Exponential Functions") to complete the last section ("initial value") of @printable-exercise{graphing-models.adoc}.
}
...
- But since any value raised to the power of zero is 1, when @math{x = 0} in exponential equations, part of the exponential term _remains_, for example: @hspace{2em} @math{4(2^0) = 4(1) = 4}.
...
How well can you identify linear, quadratic, and exponential plots?
...
How well can you identify linear, quadratic, and exponential definitions?
...
@teacher{Make sure you've advanced your teacher dashboard of @starter-file{alg2-covid-desmos} to the third slide ("Exponential Models") so that students are looking at the correct screen. In the next activity, students use Desmos to find promising exponential models, and then fit the model programmatically in Pyret!}

@lesson-instruction{
- Open to the third slide of @starter-file{alg2-covid-desmos} ("Exponential Models"), and complete the first section of @printable-exercise{exponential-models.adoc}.
...
When we filter by this important third variable, it turns out that for both Freshmen and Non-Freshmen, there is _no correlation_ between between deciding on a major and living on- or off-campus. What _looks_ like a correlation between having-a-major and living-on-campus is actually a correlation between _age_ and living-on-campus.
