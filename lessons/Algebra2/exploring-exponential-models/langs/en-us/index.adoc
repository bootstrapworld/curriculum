...
=== Overview
Having identified that _shape_ of exponential functions in the covid scatter plot, students deepen their understanding of exponential functions in graphical, tabular, and function notation form.
...
- Linear functions grow by *fixed intervals*. That means they don't grow very quickly, and they grow at the same (slow) speed all the time.
- Quadratic relationships grow by intervals that increase...by fixed amounts. That means they grow faster and faster over time, but the _rate of_ growth is fixed.
...
- A tree that's on fire will likely catch neighboring trees on fire, so that the fire spreads to 2 or more trees each time. That's why forest fires can be so dangerous - even a tiny fire can grow out of control in a short time.
...
Exponential sequences grow by equal factors, meaning the rate of growth gets bigger and bigger by equal leaps. Can you tell the difference between linear, quadratic, and exponential growth of a sequence?

[.lesson-instruction]
- Complete @printable-exercise{classifying-tables.adoc}
- Be ready to discuss your answers with the class!

@teacher{Have students share their answers. For exponential functions, it's especially important to have students discuss the _growth factor_ for exponential relationships on this page.}
