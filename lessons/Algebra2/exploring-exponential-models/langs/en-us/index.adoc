...
@lesson-instruction{
- We're going to look at the data for Rhode Island, Maine, Vermont, New Hampshire, Massachusetts, and Connecticut - all the states formed out of what the English called called "New England".
...
- You should be on Slide 1 (Quadratics Model).
...
- In the last question, why was it so much harder to compare the huge numbers?

@slidebreak

*Why is it so much harder to compare huge numbers?*
...
But when numbers get really, really, _really_ huge...we get lost! Our brains lose track of differences when the two numbers get really enormous. _Mathematically,_ the number line is composed of equal intervals forever.

@link{https://www.scientificamerican.com/article/a-natural-log/, *But we don't actually process it that way at all.*}
