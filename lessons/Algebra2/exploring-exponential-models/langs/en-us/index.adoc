...
| Exponential functions grow so rapidly that looking for "what is _added_ to y?" isn't helpful at all. The only way to talk about their growth is to start noticing "what y is _multiplied_ by?"
...
[.lesson-instruction]
- Use the third slide of @starter-file{alg2-covid-desmos} complete the first section ("base") of @printable-exercise{graphing-models.adoc}.
...
[.lesson-instruction]
- Use the third slide of @starter-file{alg2-covid-desmos} complete the second section ("vertical shift") of @printable-exercise{graphing-models.adoc}.

An exponential function with a growth factor will always start close to a horizontal line, then gradually shoot up to ever-increasing values. An exponential function with a decay factor will drop quickly, then level out close to a horizontal line. This horizontal line is called an @vocab{asymptote}, and the equation of the line will always be @math{y = k}. Adjusting @math{k} shifts the asymptote up and down, along with the rest of the exponential curve that approaches it.

[.lesson-instruction]
- Use the third slide of @starter-file{alg2-covid-desmos} complete the last section ("initial value") of @printable-exercise{graphing-models.adoc}.

The coefficient @math{a} is the initial value of the exponential term, where @math{x = 0}. That's because any value raised to the power of zero is 1: @math{4(2^0) = 4(1) = 4}. As with linear and quadratic functions, if a coefficient like @math{a} is "missing", the value of the coefficient is is @math{1} (After all, @math{1(2^x) = 2^x}!)

One area where exponentials are _different_ from linear and quadratic functions is the y-intercept: in both linear and quadratic functions, we could cross out the linear or quadratic term. After all, anything multiplied by zero is zero, so whatever value was left was our y-intercept. But in exponentials, part of the exponential term _remains_ when @math{x = 0}, The y-intercept of an exponential function is computed by @math{a + k}.
