...
Exponential Functions are generally written in the following form: @math{f(x) = ab^{(x-h)} + k}

More commonly, @math{h = 0} and "disappears", so that we exponential functions written as @math{f(x) = ab^x + k}.

Sometimes @math{k=0} and also disappears, so we'll see some exponential functions written as @math{f(x) = ab^x}. 
