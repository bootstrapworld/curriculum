...
The coefficient @math{a} is the initial value, where @math{x = 0}. The y-intercept of an exponential function is @math{a + k}. As with linear and quadratic functions, if a coefficient like @math{a} is "missing", the value of the coefficient is is @math{1} (After all, @math{1(2^x) = 2^x}!)
