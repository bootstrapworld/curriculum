...
@big{@center{@math{\text{median-income}(\text{pct-college}) =} {nbsp} @fitbruby{1em}{m}{slope} @math{\times \text{pct-college} +} @fitbruby{3em}{b}{y-intercept}}}

We want to know: _Are there values of @math{m} and @math{b} that will fit the data well?_
...
@lesson-point{
@math{f(x) = m(x-h) + k} {nbsp} is the expanded slope-intercept form, which allows us to change both the horizontal (@math{h}) and vertical shift (@math{k}).
}
