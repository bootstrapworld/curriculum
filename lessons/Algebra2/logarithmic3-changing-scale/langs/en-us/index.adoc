...
@A{The x-variable of a function with a slope of @math{\frac{1}{3}} frows _three times as fast_, relative to its y-variable.}
}
...
You've graphed all of these lines and curves on axes with *linear* scales: the numbers labeling the tick marks along the x- and y-axes are a linear sequence, increasing by the same amount from each one to the next.
...
When we changed the scale of the y-axis from `Linear` to `Logarithmic`, the numbers labeling our y-axis went from increasing at a constant rate to increasing exponentially: _each value on the y-axis is 10x as large as the one before it._

@slidebreak{Investigate}
Our x-axis labels are still consistently spaced, so this has the effect of grabbing the top and bottom sides of the graph and "squishing" it. For our function to appear to increase by one inch on the y-axis, it actually needs to increase 10x as much as the inch before it!

With the y-axis appearing to grow more slowly than the x, we *perceive* our linear function to be "bending" down and looking logarithmic!
...
@slidebreak{Investigate}
When we changed the scale of the x-axis from `Linear` to `Logarithmic`, the numbers labeling our x-axis went from increasing at a constant rate to growing exponentially.

Our y-intervals are still consistently spaced, so this has the effect of grabbing the left and right sides of the graph and "squishing" it. For our function to appear to increase by one inch on the x-axis, it actually needs to increase 10x as much as the inch before it!
...
@Q{Why did changing the *x*-axis scale to `Logarithmic` make our linear curve look exponential?}
