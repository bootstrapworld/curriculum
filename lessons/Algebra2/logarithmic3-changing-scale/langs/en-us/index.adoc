...
Let's try finding the best logarithmic model we can, starting by eyeballing it in Desmos and then fitting it in Pyret.

@lesson-instruction{
- Return to the *Fitting Wealth-v-Health and Exploring Logarithmic Models Desmos file*.
- You should now be on *Slide 5: Wealth-v-Health (Logarithmic)*. Use the sliders to fit the best logarithmic model you can.
- Open the @starter-file{alg2-countries}, and find the function template for `logarithmic`. Fill in the model settings and use `fit-model` to complete the first section of @printable-exercise{fitting-logarithmic-models.adoc}.
}
...
With the y-axis appearing to grow more slowly than the x, we *perceive* our linear function "bending" down and looking logarithmic!
