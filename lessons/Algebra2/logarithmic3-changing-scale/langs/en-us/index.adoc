...
Consider an exponential sequence, with a growth factor of 2: @math{1, 2, 4, 8, 16}...

@QandA{
@Q{What are the next 3 numbers in the sequence?}
@A{32, 64, 128...}

@Q{What is the @vocab{growth factor} of this sequence?}
@A{The growth factor is 2, since the values double each time}
...
Let's explore the inverse relationship between exponents and logs in Desmos...

=== Investigate


@teacher{Sync or pace students to *Slide 5: Changing Scale (Number Line)* of @starter-file{alg2-log-desmos}.}

@lesson-instruction{
- Return to the *Fitting Wealth-v-Health and Exploring Logarithmic Models Desmos file*.
- You should now be on the Slide 5: "Changing Scale".
- Use it to complete the first section of @printable-exercise{changing-scale-points.adoc}.
}
...
Our y-values are still growing exponentially! But now that the intervals are _also_ growing exponentially, this growth is cancelled out.

@slidebreak

@lesson-instruction{
- Complete the second section of @printable-exercise{changing-scale-points.adoc}.
- Challenge: what happens to the linear function @math{g(x)} if you make the *x-axis* logarithmic? What if you make *both axes* logarithmic?
...
Hide everything but the linear function @math{g(x)}, and change the x-axis to `Logarithmic` (keep the y-axis `Linear`).
...
Hide everything but the linear function @math{g(x)}, and change _both_ the x- and y-axes to `Logarithmic`.

@QandA{
@Q{What happens?}
@A{The line still appears linear!}
...
@lesson-instruction{
- Complete the last section of @printable-exercise{changing-scale.adoc}.
