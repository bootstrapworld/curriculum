...
- Then open the @starter-file{alg2-countries}, scroll down to the section titled "Define some models", and find the function template for `logarithmic` on line 45. Enter the Pyret code you wrote on question 2 of @printable-exercise{fitting-logarithmic-models.adoc} and click "Run" to load your definition.
- Use `fit-model` to find the @math{S} value and complete the rest of the first section of @printable-exercise{fitting-logarithmic-models.adoc}.
}
...
With that scale, the linearly-moving rocket appears to take off quickly at first, but then slows to a crawl. This is intentionally frustrating for students: they want to see what else is in the image!
...
- Return to @printable-exercise{fitting-logarithmic-models.adoc} and the *Fitting Wealth-v-Health and Exploring Logarithmic Models Desmos file*. You should now be back on *Slide 5: Wealth-v-Health (Logarithmic)*.
- Complete the second section of @printable-exercise{fitting-logarithmic-models.adoc}
}
