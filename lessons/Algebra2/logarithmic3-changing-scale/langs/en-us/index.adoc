...
When we changed the scale of the x-axis from `Linear` to `Logarithmic`, our intervals went from growing linearly to growing exponentially: _each interval on the x-axis represents 10x the length of the interval before it._

Our y-intervals haven't changed, so this has the effect of grabbing the left and right sides of the graph and "squishing" it. To see the y-values move up, _we need to go 10x as far to the right._

With the y-axis appearing to grow more slowly than the x, our linear function "bends" up and looks exponential!

@lesson-instruction{
- Reset your axes back to linear, then complete the rest of @printable-exercise{changing-scale-linear.adoc}
}
...
@A{With the y-axis appearing to grow more quickly than the x, our linear function "bends" down and looks logarithmic!}
