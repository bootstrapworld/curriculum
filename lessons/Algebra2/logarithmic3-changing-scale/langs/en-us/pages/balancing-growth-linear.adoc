...
@n Both axes are labeled (0, 2, 4...). What kind of growth is this? (circle one) 
@hspace{3em}
...
@n Did our two points on the line change? @fitb{2em}{@ifsoln{No}} @hspace{1em} Did the slope of @math{f} change? @fitb{2em}{@ifsoln{No}} @hspace{1em} How did the graph change, and why? @fitb{}{}

@blanklines{1}{
@ifsoln{The line looks steeper. The graph *looks* like it changed because we changed the scale, but all we did was "stretch" the y-axis}
}

@n *Without changing the y-axis*, change the scale on the x-axis to make @math{f} look like the original graph. Record your domain: @hspace{1em} @fitb{2em}{@ifsoln{~ -4}} ≤ @math{x} ≤ @fitb{2em}{@ifsoln{~ 4}}
...
@n What kind of growth does the sequence of numbers on the *y-axis* show _now?_ +
@hspace{3em}
Linear 						@hspace{1em}
Quadratic 					@hspace{1em}
...
@n How did the graph change? Can you explain why? @fitb{}{@ifsoln{It looks exponential!}}
