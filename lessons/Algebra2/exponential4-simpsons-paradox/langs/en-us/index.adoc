...
@teacher{Make sure you've advanced your teacher dashboard of @starter-file{alg2-covid-mi-desmos} to the fifth slide ("Exponential Model for VT") so that students will be looking at the correct screen when they are directed to return to Desmos part way through @printable-exercise{modeling-other-states.adoc}}

Now that we've explored the Michigan data, we are ready to explore some of the other subsets.

@lesson-instruction{
- Working in pairs or small groups, complete the first section of @printable-exercise{modeling-other-states.adoc} using the @starter-file{alg2-covid-mi}.
}
...
- You will need both the @starter-file{alg2-covid-mi} and Slide 5 of the *Modeling Covid Spread Desmos file*.
}
...
Earlier versions of this lesson had students build an exponential model for Vermont using @opt-starter-file{alg2-covid-new-england}. You can still access:

- @opt-printable-exercise{models-for-VT.adoc} 