...
@Q{What strategies did you use on this worksheet, to find out which tables showed exponential growth v. linear or quadratic?}
...
Most textbooks only present exponential functions with a horizontal shift of zero. When @math{h=0}, we can use this simplified form of the equation and have @math{h} hide in secret: @math{f(x) = ab^x + k}. This is the form we'll be using, but don't forget that sneaky @math{h}! Someday it might not be zero...
...
Since any value raised to the power of zero is 1, when @math{x = 0} in exponential equations, the @math{a} part of the exponential term _remains_, so we can't just look at the constant term.
...
@QandA{
@Q{How does is finding the y-intercept for exponential functions different from finding it for the linear and quadratic functions you've seen before?}
...
@Q{How can we calculate the amount of money in the account after *1 year*?}
@A{3% of $100 is $3, which we add to $100 to get $103.}
@Q{How can we calculate the amount of money in the account after *2 years*?}
@A{3% of $103 gives us $3.09, which we add to the previous total of $103 to get $106.09}.
@Q{How can we calculate the amount of money in the account after *3 years*?}
