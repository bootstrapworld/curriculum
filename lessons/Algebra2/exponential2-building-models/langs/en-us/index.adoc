...
Exponential Functions can be written in the form: @hspace{1em} @big{@math{f(x) = a(b)^{(x-h)} + k}}
...
Most textbooks only present exponential functions with a horizontal shift of zero.  And {nbsp}@math{x-0 = x}{nbsp}. So, as we build exponential models, we'll be using a simplified form: @hspace{1em} @big{@math{f(x) = a(b)^x + k}} 
