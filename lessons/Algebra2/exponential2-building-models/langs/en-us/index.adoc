...
In order to talk about the growth of an exponential function we need to identify the @vocab{base} (or @vocab{growth factor}) by asking ourselves "what is being _multiplied_ by y?" +
{empty} +
*In this case, we can see that the y-values are doubling each time*, so the @vocab{base} / @vocab{growth factor} is 2!

@slidebreak{Investigate}

@lesson-instruction{
- Turn to @printable-exercise{classifying-tables.adoc}, and decide what kind of pattern is represented in each table.
- For each table that is growing exponentially, identify the @vocab{base} / @vocab{growth factor}.
}
...
@teacher{Advance your teacher dashboard of @starter-file{alg2-covid-mi-desmos} to the *Slide 2: How does h transform exponential functions?*}
...
@teacher{
Make sure you've advanced your teacher dashboard of @starter-file{alg2-covid-mi-desmos} to the third slide ("Exploring Exponential Functions") so that students are looking at the correct screen.
...
*Adjusting @math{k} shifts the asymptote up and down*, dragging the rest of the exponential curve that approaches it.

@slidebreak{InvestigateC}
==== Initial Value @math{a}

What do you think the @vocab{asymptote} for the exponential function below will be?
...
@teacher{@opt{If you'd like your students to do some computations with exponential functions, you can print copies of @opt-printable-exercise{saving.adoc}.}}
...
- Unstable atomic nuclei decay into stable nuclei over time, emitting ionizing radiation as a byproduct. We use the term _half-life_ to refer to the length of time it takes for 50% of the atomic nuclei in a radioactive sample to decay: the function @math{\text{unstable-atoms}(\text{half-lives})} decays exponentially.
