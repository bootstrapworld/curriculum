= Graphing Exponential Models: {nbsp}{nbsp} __f(x) = ab^x^ + k__
++++
<style>
...
The curve you'll see is the graph of {nbsp}@math{h(x) = 2^x}. Another curve {nbsp}@math{f(x)}{nbsp} is hiding behind it with identical model settings: {nbsp} @math{k=0}, {nbsp}@math{b=2} and {nbsp}@math{a=1}.
}
