...
The curve you'll see is the graph of {nbsp}@math{g(x)}, an exponential function. Another, *identical* curve {nbsp}@math{f(x)}{nbsp} is hiding behind it. +
@n Use the starting values of @math{a}, @math{b}, and @math{k} in Desmos to complete the following equation: {nbsp} @math{g(x) = f(x) = } 
@fitbruby{2em}{@ifsoln{1}}{@math{a}}
(@fitbruby{2em}{@ifsoln{2}}{@math{b}}) @math{^x +}
@fitbruby{2em}{@ifsoln{0}}{@math{k}} 

== Base @math{b}
@n Make sure {nbsp}@math{k = 0}{nbsp} and {nbsp}@math{a=1}{nbsp}. Experiment with @math{b}. *For what values of @math{b} is the function _undefined_*, with the line disappearing? @fitb{}{@ifsoln{negative}}

@n Keeping {nbsp}@math{a=1}{nbsp} and {nbsp}@math{k=0},{nbsp} change {nbsp}@math{b}{nbsp} to {nbsp}0.5, {nbsp} 1, and {nbsp}2, graphing each curve below. For each curve, *label the coordinates at {nbsp} x=1, {nbsp} 2, and {nbsp} 3.*
...
@n Keeping {nbsp} @math{a=1} {nbsp} and {nbsp} @math{b=2}, {nbsp}try changing the value of {nbsp}@math{k} to {nbsp}-10, {nbsp} 0, and {nbsp} 10, graphing each curve in the squares below. For each curve, find and label the y-value where the curve is "most horizontal", then *draw a horizontal line at that y-value*.
...
@n Set {nbsp}@math{k=0}{nbsp} and {nbsp}@math{b=2}{nbsp}. Change the value of {nbsp}@math{a} to {nbsp}10, {nbsp}2, and {nbsp} -5, graphing each curve in the squares below. +
