...
The definition @show{(code '(define MI-table (filter covid-table is-MA)))} _filters_ our dataset, keeping only the rows for which `state = "MI"`. We could create other helper functions like `is-MI`, and use them with the `filter` function to get datasets for any state we want!
...
Earlier versions of this lesson provided scaffolded materials for Massachusetts. You can still access the @opt-printable-exercise{linear-models-MA.adoc, linear} and @opt-printable-exercise{quadratic-models-MA.adoc, quadratic} pages.
