...
Students explore the Covid dataset, focusing on the growth in positive test cases for the state of Michigan (they can explore other states later on, as well).
...
@lesson-instruction{
- We're going to look at the daily total of confirmed, positive cases for a handful states – California, Michigan, Illinois, Wisconsin, and Minnesota - during 2020 (January 1st, 2020 through January 1st, 2021).
- Open the @starter-file{alg2-covid-mi}, select "Save a Copy", and click "Run."
- Working in pairs or small groups, complete @printable-exercise{exploring-covid.adoc}.
}

@strategy{Why just 2020?!?}{

This dataset is @link{https://docs.google.com/spreadsheets/d/1GFWesAyYshYXDDSTxoHYmFrPVDTQd12rEVR-ZGn11hg/, available for all 50 states (and Washington, D.C!)}, but for pedagogical purposes we've pulled out just a handful of states for the 365 days of January 1st, 2020 to January 1st, 2021.

We've made this choice in order to showcase the most purely-exponential behavior of the infection curve, for the sake of this lessons' math learning goals.

For students who are comfortable with polynomial functions involving a mix of exponential, quadratic/cubic/etc, and linear terms, we recommend showing them _all_ the data and letting them choose a state to explore. The first portion of the infection curve shows a gradual, linear growth pattern at the start of 2020, before exploding a few months later. A purely exponential function will under-predict the growth during this time period, adding significant friction to the exponential modeling goal of this unit!
...
To use all available data, open the @starter-file{alg2-covid-mi} and change the source sheet on line 12 from `"CA, MN, MI, WI, IL, and OH in 2020"` to `"All"`.
}
...
@lesson-instruction{
@center{@image{images/multiple-models.png, 600}}
...
=== Overview
Students use Pyret to perform linear regression on the data from Michigan and discuss whether or not the optimal linear model is a good fit for the data.

=== Launch
@slidebreak{Launch}
Let's start out by looking at _just one state_. We'll use Michigan for this investigation, but you can do your own investigation about any of the other states after finishing this one!

=== Investigate
@slidebreak{Investigate}

@lesson-instruction{
- Turn to @printable-exercise{linear-models-MI.adoc}, and complete the first two questions.
- In the Definitions Area, find the definition of `is-MI` underneath the comment `# Define some helper functions`
- With your partner, come up with an explanation of what you think this function does
- A few lines farther down, find the definitions of `MI-table`. What do you think is happening here?
- Don't be afraid to experiment by changing the code!
}

@slidebreak{Investigate}

- The first part of this code defines a _new function_ called `is-MI`, which tests a single Row to see if `state == "MI"`.
...
- The second part uses Pyret's `filter` function. This function consumes a table (in out example, `covid-table`) and a _function_ (`is-MI`), and produces a new table containing only _rows for which that function returns `true`._ This new table -- containing only rows in Michigan -- is given the name `MI-table`.
...
@lesson-instruction{
- Complete @printable-exercise{linear-models-MI.adoc}.
}

The definition @show{(code '(define MI-table (filter covid-table is-MI)))} _filters_ our dataset, keeping only the rows for which `state = "MI"`. We could create other helper functions like `is-MI`, and use them with the `filter` function to get datasets for any state we want!
...
@right{@image{images/MI-covid-linear.png, 300}}If we make the line go from the start to the peak of the curve (top line), almost all of the points bulge out below our @vocab{line of best fit}. +
If we make the line hit the _bottom_ of the curve, all the points fall above it (bottom line).

@slidebreak{InvestigateR}

@ifslide{@right{@image{images/MI-covid-linear.png, 300}}}Splitting the difference (orange line) is better than both of those options, and we might even get a halfway decent @math{S}! +
...
- Clicked on "pacing" and set your teacher dashboard of @starter-file{alg2-covid-mi-desmos} to the first slide so that students are looking at the "Quadratic Models" screen
...
- Using the file, complete @printable-exercise{quadratic-models-MI.adoc}
}
...
Earlier versions of this lesson provided scaffolded materials for Massachusetts using @opt-starter-file{alg2-covid-new-england} and @opt-starter-file{alg2-covid-ma-desmos}. You can still access:

- @opt-printable-exercise{linear-models-MA.adoc} 
- @opt-printable-exercise{quadratic-models-MA.adoc}
