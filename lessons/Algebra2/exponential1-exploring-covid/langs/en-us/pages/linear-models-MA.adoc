...
@n In the Definitions Area, find the definition of `is-MA` and `MA-table`. What are these things? @fitb{}{} +
...
@n Change the definition of the `linear` function in the @opt-starter-file{alg2-covid-new-england} to match the model produced by `lr-plot` and "Save".
