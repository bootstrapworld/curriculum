[.beta]
= Exploring Linear Models (CA only)
...
- Let's start by practicing matching the scatter plots to their descriptions on @printable-exercise{identifying-form-matching.adoc}.
}
...
When we interpret a model, we try to make sense of the slope, the axes, the @math{S} value, and the real data behind them. In this example, _a model built from 1940 and 1950 predicts that each year the adjusted minimum wage will be *$0.152* less than the year before!_
