...
/* Shrink horizontal and vertical spacing on fitbruby */
.fitbruby {mid-width: 1em; --fitb-height: calc(0.5rem + 1em);}
...
@n Use the starting values from Desmos for @math{a}, @math{h}, and @math{k} to complete this equation: @math{g(x) = x^2 = f(x) = } 
@fitbruby{1em}{@ifsoln{1}}{a} @math{(x -} 
@fitbruby{1em}{@ifsoln{0}}{h} @math{)^2 +} 
@fitbruby{1em}{@ifsoln{0}}{k}

== Magnitude @math{a}
@n In the first square below, make a sketch of the original graph you see (@math{a = 1}, @math{h = 0}, @math{k = 0}). +
Then try changing the value of @math{a} to 10, 0.1, 0, -10 and -2, graphing each parabola in the squares below. +
*Label the vertex "V" and any roots with "R"!*
