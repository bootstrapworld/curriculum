...
@teacher{
@opt{If your students could use a warmup with distinguishing between linear and quadratic relationships, print copies of:
...
@opt{If your students could use more practice - including dealing with sequences where @math{x} doesn't just grow by 1 each time - print copies of @opt-printable-exercise{classifying-tables-gaps.adoc}
}
}
...
As we work with parabolas -- and eventually fit them our Miguel Cabrera dataset -- we need to know how to talk about and describe them.
...
@teacher{Make sure you have created a link or code for your class to @starter-file{alg2-exploring-quadratics-desmos-cabrera}.}
...
@A{If the minimum y-value is negative, it will fall below the x-axis. That will make the parabola cross the x-axis twice as it opens up, so there will be two roots. Similarly, there will be two roots if the maximum is positive.}
@A{If the minimum or maximum y-value is zero it will fall directly on the x-axis and there will be one root}
@A{If the minimum y-value is greater than zero or the maximum y-value is less than zero, the parabola won't cross the x-axis and there won't be any roots.}
