...
The standard form of periodic models is @math{f(x) = a \sin(\frac{2\pi}{P}(x - h)) + k}. Let's explore the role of @vocab{amplitude} @math{a} in periodic functions! @link-instructions{Open the Desmos File *Exploring Periodic Functions* to Slide 1: *Modeling the Ferris Wheel Dataset (sine)*. You should see four sliders for @math{P},  @math{a}, @math{h}, and @math{k}. Adjusting @math{P} also changes @math{b}, which is always @math{2\pi \div P}.}
...
@n Leaving the other sliders where they best fit the data, change *ONLY* the slider for @math{a}, experiment with values at 100, 50, -50, and 0, graphing each curve below. *For each curve, label the coordinates at time=@math{15}, @math{30}, and @math{45}.*
