...
@link-instructions{For this section, use *Slide 1: "Modeling the Ferris Wheel Dataset (sine)"* of the *Exploring Periodic Functions* Desmos File.} You'll find *the data from the Ferris wheel plotted in blue*, along with a basic periodic model of the form @math{f(x) = a \sin(\frac{2\pi}{P}(x - h)) + k}.
...
@math{cos(2\pi \div}
 @fitbruby{70pt}{@ifsoln{30}}{period} (x - @fitbruby{70pt}{@ifsoln{@math{2.375\pi}}}{horizontal shift})
) + @fitbruby{70pt}{@ifsoln{105}}{vertical shift}

| @vspace{1ex} +
*Pyret Notation*
|
@show{(code '(define (f x) (+ (* (?ANS 100) (sin (* (/ 6.28 (?ANS 30)) (- x (?ANS (* 2.375 PI)))))) (?ANS 105))))}
|===
...
@n Adjust the sliders so that the function @math{g} perfectly overlaps the function @math{f}. What is the value of @math{a}? @fitb{5em}{@ifsoln{10}} @math{b}? @fitb{5em}{@ifsoln{1}} @math{k}? @fitb{}{@ifsoln{2}}
...
@n In row 1 below, write the definition of @math{g} using the values of @math{a}, @math{b}, and @math{k} that you found earlier. Then *change some of @math{f}'s model settings* and write it in the second row, then show how @math{q} would need to change to overlap it.
...
| @math{f(x) = 10 sin(\frac{2\pi}{2\pi}\times(x-0)) + 2}
| @math{g(x) =} @ifsoln{@math{10 cos(\frac{2\pi}{2\pi}\times(x-\frac{\pi}{2})) + 2}}
...
*Pyret Notation*
|
@show{(code '(define (g x) (+ (* (?ANS 100) (cos (* (/ 6.28 (?ANS 30)) (- x (?ANS (* 4.75 PI)))))) (?ANS 105))))}
|===
