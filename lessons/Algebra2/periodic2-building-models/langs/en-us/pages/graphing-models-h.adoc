...
The standard form of a periodic model is @math{f(x) = a \sin(\frac{2\pi}{P}(x - h)) + k}. On this page, we'll explore the role of @vocab{phase shift} @math{h} in periodic functions. @link-instructions{Open the Desmos File *Exploring Periodic Functions*. You should be on *Slide 1: Modeling the Ferris Wheel Dataset (sine)* and see four sliders for @math{a}, @math{P}, @math{h}, and @math{k}. Adjusting @math{P} also changes @math{b}, which is always @math{2\pi \div P}.}
