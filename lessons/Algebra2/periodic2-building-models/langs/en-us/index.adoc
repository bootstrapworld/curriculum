...
@QandA{
@Q{What strategies did you use to match the graphs to the descriptions?}
}
...
While most math textbooks introduce the parent sine and cosine functions using @math{b} instead of @math{P}, we've consciously chosen to do the opposite for pedagogical reasons. Frequency is a much more abstract concept, and the calculation with @math{\pi} also makes for "messy" numbers with long decimals that are more difficult to work with. Even strong students who are comfortable calculating with numbers may not connect the math back to common sense.

In the Ferris Wheel example, it's easy for students to see where the 30m period fits into the equation. By building concrete connections with @math{P} first and then transitioning to the more abstract @math{b}, the parent function becomes more accessible to struggling learners while deepening the advanced students' understanding.
}
...
- With your partner or group, experiment with the model settings of periodic models to complete these three pages:
  * @printable-exercise{periodic-hkp.adoc}
  * @printable-exercise{periodic-a.adoc}
}
...
Now that we've explored the model settings for the period-based model, it's time to consider another model for defining periodic relationships that works better for engineers.

@lesson-instruction{
- Rather than focusing on _how long it takes_ for the ride to complete one cycle, we can think about _how many cycles the ride completes_ in 30 minutes. 
- Return to *Slide 1: Modeling the Ferris Wheel Dataset (sine)* of your *Exploring Periodic Models* Desmos file and complete the first section of @printable-exercise{periodic-b.adoc}. 
}
...
@lesson-instruction{
Turn to @printable-exercise{grading-ai.adoc}, and grade the performance of an AI that was told to fit periodic models to real data!
}
...
@teacher{Pace students to __Slides 2 through 3 of @starter-file{alg2-exploring-periodic-desmos}__. Note: Slide 2 will look almost exactly like the slide they've been working with so far, but the model and sliders here are frequency-based.}
...
- With your partner, complete @printable-exercise{modeling-ferris-wheel.adoc}.
  * For the first section you should still be on *Slide 2: Modeling the Ferris Wheel Dataset (sine)*.
  * For the second section you'll advance to *Slide 3: Translating from sine to cosine*.
}

@slidebreak{Investigate}
@QandA{
@Q{What is the relationship between the sine and cosine models on Slide 3?}
@A{All of the model settings are the same, except for the horizontal shift}
@A{Cosine shifts @math{\frac{\pi}{2}} to the right.}

@Q{What is the relationship between the horizontal shift of `q` and the @vocab{preiod}?}
@A{The shift is one quarter of the period.}

@Q{Do you think that all sine models could be rewritten as cosine models, and viceversa?}
}

@slidebreak{Investigate}
@QandA{
@Q{Will the horizontal shift between a cosine and sine model always be the same?}
@A{It will always be 1/4 of the period.}
}

@slidebreak{Investigate}

@teacher{Sync or Pace students to __Slides 4 of @starter-file{alg2-exploring-periodic-desmos}__.}

@lesson-instruction{
- You should now be on *Slide 4: Modeling the Ferris Wheel Dataset (cosine)*.
- With your partner, complete @printable-exercise{modeling-ferris-wheel.adoc}. 
...
- Suppose a student volunteers "the temperature, because it gets cold in the winter and warm in the summer":
  ...
  * The temperature in your area might fluctuate between 95° in the summer and 25° F in the winter. 
    *** That's a range of 70° F.
    *** And an amplitude of 35° F.
    *** So the midline and the vertical shift are at 60° F, found by adding the minimum temperature (25) plus the amplitude (35)
