...
Solicit a few definitions for each term from different students, always looking for definitions that are different from those that have been shared before. (Of course, be sure to correct any incorrect definitions!)
...
Now that we've explored the _period-based model_, it's time to consider the _frequency-based model_ that works better for engineers.

@lesson-instruction{
- Rather than focusing on _how long it takes_ for to complete one cycle, we can think about _how many cycles_ the ride completes in 30 minutes.
