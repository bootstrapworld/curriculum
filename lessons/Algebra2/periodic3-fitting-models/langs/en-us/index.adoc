...
@lesson-instruction{
- Complete @printable-exercise{modeling-recent-co2.adoc}.
...
@lesson-instruction{
- Let's use these values to construct our `sine` model, and then fit our model to the data in Pyret!
...
@QandA{
@Q{When you look at the `sine` model graphed on the `recent-table` scatter plot, do you think it makes sense to use a periodic model for this data? Why or why not?}
...
@Q{Samuel says that the `sine` model is a good fit for the data in the `recent-table`. +
