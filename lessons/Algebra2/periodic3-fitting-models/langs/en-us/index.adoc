...
@lesson-instruction{
- Let's use these values to construct our `periodic` model, and fit it with Pyret!
...
@QandA{
@Q{How well does your `periodic` model fit the data in the `recent-table`?}
@A{It fits fairly well. The shape looks more or less like what we are seeing in the scatter plot. We expect an error of about 1.2 ppm of @math{\text{CO}_2} and the data in the `recent-table` ranges from 415.91 to 424.}
...
@lesson-instruction{
- Let's see how well our model fits our the larger dataset.
