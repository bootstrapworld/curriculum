...
|===
|
@span{.title}{Patterns Worth Exploring}

The @starter-file{alg2-states} has a lot of interesting data, and endless possible combinations of columns to explore. But randomly smashing columns together in a scatter plot is _not_ the habit we want students to cultivate! Instead, make sure students are actually talking with their partners about why two columns may or may not be related. To get you started, here are a few possible scatter plots worth exploring:

Can students predict these relationships? (If so, probably not worth having them spend time on them!)
...
Surprises in the District of Columbia (DC often shows up as an outlier, or as an extreme value. Why?)
...
@right{@image{images/difference-table-linear.png}} Linear relationships grow by fixed amounts, meaning that the difference between two y-values will always be the same over identical intervals. In the table shown here, you can see arrows pointing out the "jumps" between y-values for intervals of 1. Each jump is the same size. **If the "size of the growth" is constant, the relationship is linear.**

* Try of comparing intervals of 2, instead of intervals of 1. Is the difference between x=1 and x=3 the same as the difference between x=2 and x=4?
** Yes.
