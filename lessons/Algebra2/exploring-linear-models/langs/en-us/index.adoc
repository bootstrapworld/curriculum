...
=== Overview
@teacher{Students explore the State Demographics dataset and, building on a discussion of the visualizations they previously made using the animals dataset, recognize the unique opportunity scatter plots offer for exploring relationships between columns.}

=== Launch
We're going to search for relationships within a dataset about all the states in the US. But first, let's take a moment to (1) develop confidence in our ability to use functions for working with tables and making visualizations, and (2) build familiarity with a new dataset that we are going to spend a lot of time with.
...
- Share your scatter plots with one another. _(Perhaps by copying and pasting the scatter plots into a shared document, and then labeling those visualizations?)_
...
@QandA{
@Q{Based on the @vocab{S} values of the scatter plots you created on this page, what do you think @vocab{S} means?}
