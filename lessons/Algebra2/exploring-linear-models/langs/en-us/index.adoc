...
* This lesson establishes a structure for exploring relationships in data: (1) build a model from samples, (2) fit a model, and (3) better modelling. Students will repeat this process in subsequent explorations.
...
- Working in pairs or small groups, complete the _Notice and Wonder_ section on @printable-exercise{exploring-states.adoc}.
** _Have students share back what they Notice and Wonder about the dataset._
- Now complete the rest of the worksheet.

=== Investigate

In the `# Define some rows` section of the Definitions Area of @starter-file{alg2-states}, we see how Pyret defines values:
