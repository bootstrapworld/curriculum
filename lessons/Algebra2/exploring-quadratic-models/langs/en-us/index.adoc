...
Linear functions grow by _fixed intervals_... the rate of change is _constant_. In the table above, each time the x-value increased by 1, we saw that the y-value increased by 2.

Quadratic functions grow by intervals that _increase by fixed amounts_... the amount of change is constantly changing! In the table above, every time the x-value increased by 1, we saw that the y-value increased by _2 more than it did in the in the previous interval._ You probably observed that y increased first by 1, then 3, then 5, then 7.
