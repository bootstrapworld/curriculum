...
What do each of the coefficients in Vertex Form mean?

[.lesson-instruction]
- Open @online-exercise{https://www.desmos.com/calculator/geyx4aetop, Exploring Quadratics}.
- Complete @printable-exercise{graphing-models.adoc}, using this Desmos activity.

Now that we have a sense for what these cofficients mean, let's try to model the speed-v-mpg relationship using a quadratic model!
