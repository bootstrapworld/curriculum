...
There's definitely a relationship here. Even though some cars are more efficient than others, fuel efficiency generally increases from 15mph to around 40-55mph. But after that, fuel efficiency _decreases._
...
=== Overview
Students are introduced to quadratic functions. They become familiar with scenarios involving quadratics, terminology to describe parabolas, how to identify quadratic sequences, and explore the meaning of coefficients in one form (vertex).
...
Relationships like the one in the fuel efficiency scatter plot are best described by a curve! Today, we will learn about one kind of curved model called a @vocab{quadratic model}.  Graphs of quadratic relationships are often described as "u-shaped" or "looking like an arch". More formally, mathematicians and data scientists call these kinds of curves @vocab{parabolas}.
...
@optional For more practice identifying quadraric v. linear situations, have students complete @opt-printable-exercise{classifying-descriptions.adoc}
...
That means that if we're looking at a list of x-y pairs, we need to consider whether the distance between the y-values _remains constant_ or does something different. In the next activity, we're going to spend some time doing exactly that.
...
=== Investigate
We've been using quadratic models that look like this: @math{f(x)=a(x−h)^2+k}

We already know that @math{h} in this model is the x-coordinate of our @vocab{vertex}, and that the @vocab{vertex} of our efficiency curve is predicted at roughly @math{45mph}. All we need to do is figure out @math{a} and @math{k}!
