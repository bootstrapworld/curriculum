...
@QandA{
@Q{Of the models you sketched and described, which do you think it would make the most sense to try fitting to this scatter plot? Why?}
@A{Linear, because the data is steadily rising.}
@Q{Any other notices or wonders you would like to share?}
}

=== Investigate

We built our `periodic` model using one year's worth of data. Does that mean we can use it to predict carbon dioxide levels for other years?
...
We built our `periodic` model using the `recent-table` and it fit the data fairly well. Does that mean we can use it to predict carbon dioxide levels for other years?
...
What do you think will happen if we try to fit this model to _all_ of our data in the `co2-table`? Try it out!
