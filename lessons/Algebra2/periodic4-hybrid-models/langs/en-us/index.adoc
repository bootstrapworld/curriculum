...
=== Overview
Students explore the historical data, discovering that there's a stronger pattern at work than the seasonal periodicity: a linear pattern of rising @math{\text{CO}_2} over time. They try fitting a linear model first, then combine it with their periodic model to find a better fit.

=== Launch

Our `periodic` model fit the `recent-table` pretty well with an S-value of about 1.2ppm. But how well does it fit, if we try it with data from more than just the one year?
