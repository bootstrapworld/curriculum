...
@lesson-instruction{
- In the @starter-file{alg2-co2-hybrid}, find the code where you're asked to "Define your periodic-sin functions..."
...
- Use the `year-table` to make a scatter plot of the possible relationship between `water-level-ft` and `time` with `month-name` as the label. 
}
...
@lesson-instruction{
- Use the `lakes-table` to make a scatter plot of the possible relationship between `water-level-ft` and `time` with `month-name` as the label. 
}
...
@Q{Do you expect your `periodic-sin` model to be a good fit for this dataset?}
