...
.studentAnswerMedium { min-width: 60pt !important; }
...
@link-instructions{
This page focuses on the `modern-table` of the @starter-file{alg2-co2-hybrid}, which tracks atmospheric @COtwo (parts per million) over the course of 168 monthly readings taken from 2010-2023.
}
...
Towards the bottom of the Definitions Area, find the section of the starter file where you're asked to "Define your sine functions..."

@n Define `sine` to be *the periodic model you found earlier*, for @math{CO_2} levels from 2022-2023.
...
@n Using the deconstruction of `cosine` as your model, change the other three functions in this section to show how to separate the wave and midline of your `sine` model and define `sine-composed` using function composition.
...
|===
|
@n Use `fit-model` to fit `sine` to the data in the `modern-table`. 
