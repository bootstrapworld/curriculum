...
@n In the first row of the table below, we've fit the `cosine` model to all three datasets.
...
The `cosine` model was built from `recent-table`, _so there's no surprise_ it was a good fit for `recent-table`! But for datasets that reach farther and farther back in time, that model fits worse and worse.

@n It's tempting to compare the @math{\text{S-values}} in this table, to quantify how much more error we'd expect from the `sine` model for the `co2-table`. *Why shouldn't we?*
