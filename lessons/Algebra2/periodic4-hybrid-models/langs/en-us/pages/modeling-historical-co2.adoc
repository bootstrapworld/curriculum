...
#content td .autonum { padding-top: 1.2em !important; }
</style>
++++
...
@link-instructions{
For this page, we are going to work with the data in the `co2-table` of the @starter-file{alg2-co2-hybrid}, which tracks the amount of @COtwo in the atmosphere (parts per million) over the course of 596 monthly readings from 1974 to 2023.
}
...
@link-instructions{Make sure you've updated the @starter-file{alg2-co2-hybrid} so that `sine` matches what you found on @printable-exercise{periodic3-fitting-models/pages/modeling-recent-co2.adoc, Modeling Recent CO2 Levels}}.

[cols="5a, 2a", grid="none", frame="none"]
|===
|@n Use `fit-model` to fit your `sine` model to the data in the `modern-table` and _sketch it_ on the scatter plot to the right.

@n Complete the interpretation stem below.

@indented{
__**The error in the model** is described by an @vocab{S-value} of about__ +
@fitbruby{3em}{@ifsoln{55.89}}{S}
@fitbruby{4em}{@ifsoln{ppm}}{y-units}. @hspace{1em} _I_
...
@n Which model is better, the `linear` model or the `sine` model? @fitb{}{@ifsoln{linear}}

@n How much
@fitbruby{7em}{@ifsoln{less}}{more / less }
error do we expect in predictions made with the `linear` model than with the `sine` model?
