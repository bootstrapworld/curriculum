...
The y-coordinate of @vocab{roots} is always zero. And @math{y} will equal zero if @math{x = r_1} or @math{x = r_2}, because that will make one of the parenthetical terms be zero, and anything times zero is zero.
...
Standard form of any polynomial is written with terms from highest to lowest degree. The standard form for quadratic models puts the quadratic (squared) term first, followed by the linear term, followed by the constant term for vertical shift. Knowing that an equation is in standard form makes it easy for someone to know exactly what kind of equation it is just by looking at the first term. The coefficients in standard form can also be computed using the @link{https://en.wikipedia.org/wiki/Quadratic_formula, quadratic formula}, when factoring isn't simple.
