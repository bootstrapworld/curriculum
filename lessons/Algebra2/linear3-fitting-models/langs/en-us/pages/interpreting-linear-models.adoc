...
/* shrink line height on fitb, fitbruby, AND any blanklines sharing the page */
#content { --blank-height: 1.7em; }

.sectionbody .paragraph:first-child .autonum { padding-top: 0.5em !important; }
</style>
++++
...
@n Evaluate @show{(code '(lr-plot states-table "state" "pct-college-or-higher" "median-income"))}. What is @math{S}? @fitb{}{@ifsoln{5659.212}}

@n On the line below, write the optimal linear model that was computed through linear regression:

@center{
 @math{optimal(x) =} @fitbruby{7em}{@ifsoln{@math{1142}}}{slope (m)}x + @fitbruby{8em}{@ifsoln{20868.14}}{y-intercept / vertical shift} @hspace{5em} @show{(code '(define (optimal x) (+ (* (?ANS 1142) x) (?ANS 20868.14))))}
}

== Interpret the `al-ak` Model

We started with a model based on Alabama and Alaska (@show{(code '(define (al-ak x) (+ (* 5613.67 x) -83616.02)))}). We got an @math{\text{S-value}} of `~36164.68`. We can interpret this as follows:
...
@fitbruby{15em}{percent attended college}{explanatory variable (x)} is associated with a
@fitbruby{12em}{~5614 dollar}{rate of change in y-units}
@fitbruby{10em}{increase}{increase / decrease} in
@fitbruby{18em}{median household income}{response variable (y)}. +
...
@n Use this model to predict the median income for a state where 30% of the population has attended college @fitb{}{}

@fitb{}{@ifsoln{@math{5614 \times 30 + 83616 = ~252306}}}


== Interpreting the Optimal Model

@n This model was built using data about 
@fitbruby{8em}{@ifsoln{51}}{how many?}
@fitbruby{8em}{@ifsoln{states}}{what?} whose +
@fitbruby{30em}{@ifsoln{percent of the population who attended college}}{x-variable} ranged from
@fitbruby{7em}{@ifsoln{18.3}}{x-min} to
@fitbruby{7em}{@ifsoln{52.4}}{x-max}
@fitbruby{8em}{@ifsoln{percent}}{x-variable units}. +

@n Possible limitations of this model include @fitbruby{}{@ifsoln{Data is all from one year.}}{uneven distribution, high variability, small sample size, etc.}

@n The @fitbruby{15em}{linear-regression}{sensible name} model predicts that a 1
...
@n With @fitbruby{15em}{@ifsoln{median household income}}{response variable (y)} ranging from
...
@blanklines{2}{
@ifsoln{The model is only designed to make predictions that fall within the domain of the dataset (between 18 and 52 percent). This model predicts a median income of -83616 dollars for zero pct-college-or-higher!}
}
