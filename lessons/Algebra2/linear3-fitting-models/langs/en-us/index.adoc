...
@teacher{
Make sure your students know to how to calculate what percent less error we expect in predictions from the `ma-nv` model than the `al-ak` model. Students are about to calculate what percent more or less error we expect from predictions made with their models, which will all have different @math{S-values}. Right now everone is looking at the same @math{S-values} so supporting them will be much less work!
