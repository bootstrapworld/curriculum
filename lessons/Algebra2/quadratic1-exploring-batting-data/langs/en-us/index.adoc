...
@Q{Use the @math{S}-value to explain why this model doesn't fit.}
...
=== Launch
Linear models work well for fitting simple relationships, but many relationships in the real world are more complex! _Straight-line, linear models are often too primitive_.
...
- *If your students aren't interested in baseball*, the @opt-printable-exercise{exploring-mpg.adoc} engages students in exploring the @opt-starter-file{alg2-fuel-efficiency}. +
_(Thinking for this page parallels @printable-exercise{curve-data.adoc})_
