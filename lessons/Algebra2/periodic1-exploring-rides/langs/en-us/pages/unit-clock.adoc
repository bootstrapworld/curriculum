...
.autonum { padding-top: 0.6em !important; }
.fitb:has(.solution .MathJax) { --fitb-height: calc(0 + 1em); }
...
@link-instructions{Read the questions below, then open the @starter-file{alg2-unit-clock} and click "Run" to see the animation that will help you answer them.}
...
@n How would you describe these curves? @fitb{}{@ifsoln{They go up and down between a maximum and a minimum over and over again.}}

@fitb{}{@ifsoln{They are identical, except that they are shifted horizontally so that one crosses the x-axis when the other is its farthest from the x-axis.}}