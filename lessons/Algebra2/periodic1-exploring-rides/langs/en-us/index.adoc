...
- The Babylonians chose to use 360 slices to map out the "circle" representing the night sky, because 360 is roughly the number of days in a year and most easy-to-see constellations repeat their cycles annually. 360 is also easily divided by common numbers like 2, 3, 4, 6, 8, 9, 10, and 12, making it great for doing math
- The Babylonians also noticed that there were roughly 12 full moons each year, and that a hand has 12 finger joints that can be touched with the thumb. For people who only have their hands to count with, 12 is a really convenient number to work with! They divided the night and day into 12 slices each (giving us 24 hours), and used @link{https://en.wikipedia.org/wiki/Sundial, sun dials} to tell time. Thousands of years later, the 12 slices of a circular sun dial became the 12 hours on a clock.

In other words, we can chop up circles based on whatever kind of math we want to do, and what devices we have to do it on.
