...
This worksheet had you create a *new type of data visualization*, called a @vocab{line graph}. Not every scatter plot can be turned into a line graph!

We might only have data for the Ferris Wheel every 5 minutes, but we know that there are an infinite number of points _between_ those times. We also know that the Ferris Wheel existed at every one of those points, meaning there was `height` data to be gathered at all times. When the explanatory variable is continuous, as it is here, it makes sense to draw a line between our points.

Not every explanatory variable is continuous, so it's not appropriate to connect the dots on every scatter plot!
...
Modeling cyclical relationships is _incredibly important_, for everyone from farmers to fishermen to healthcare providers! So many things in nature come in cycles:
...
- The "hand" of the clock is just a radius, which hits the circumference at some point (A, B).
...
@A{6pm?}
@Q{When does @math{B(\textit{time})=0}, where the radius sits along the x-axis?}
@A{3pm lands on (1,0)}
@A{9pm lands on (-1,0)}
@Q{At which time(s) does @math{A(\textit{time})=B(\textit{time})}, where the legs @math{x} and @math{y} are equal?}
@A{1:30 and 7:30}
@Q{When @math{A(\textit{time}) = B(\textit{time})}, how could we calculate the length of @math{x} and @math{y} from this right triangle?}
...
- *When you have completed the graph at the bottom*, open @starter-file{alg2-unit-clock}
}
...
@QandA{
In our clock example, we divide the circle into twelve "slices", each representing one hour. How many of those slices would represent 15 minutes? An hour and a half?
...
Would the shape of the curves change if we used 360 degrees instead of 12 hours?

@lesson-instruction{
In our clock animation we have 12 "slices", and drawing 4 evenly-spaced labels around the clock.
...
This is really convenient when it comes to doing math involving circles! If we want to talk about _7th of a circle_, we might choose slices that are multiples of 7. In other words, we can chop up circles based on whatever kind of math we want to do. The Babylonians chose to use 360 slices, because 360 is easily divided by 2, 3, 4, 6, 8, 9, 10, and 12.

@slidebreak

We often want to talk about the distance traveled around the circumference of a circle.

- We might want to know how far our teacher traveled on the Ferris Wheel
- If we're building an arch out of bricks, we want to know how many bricks to use

Calculations involving circumference all involve the *radius* of the circle. Is there a way to divide the circle so that our calculations are easy? It would be nice to have a measurement of circumference that's _expressed in terms of radius_, to make the math cleaner...

@slidebreak

--
@right{@image{images/clock-6-equilateral.png, 200}}
...
@QandA{
@Q{We could go all the way around the clock circle with 6 of those triangles. Would that completely fill up our circle?}
@A{No - it just creates a hexagon!}
}
--
