...
In fact, *we can fit exactly 6.28 (@math{2pi}) of these "radius slices" into a circle*!
...
@lesson-instruction{
- Return to the @starter-file{alg2-unit-clock}.
...
@teacher{This last thought is for thinking about, not for testing in the starter file, which isn't set up to run in radians, so can't handle @math{\pi}.}
...
Mathematicians use special names for these functions. They call them @vocab{sine} and @vocab{cosine}, rather than "A" and "B"!
...
@teacher{
We've also exposed `sin-deg` and `cos-deg`, which consume degrees instead of radians.
}
