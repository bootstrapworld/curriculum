...
- @vocab{sine} -- the y-coordinate (vertical position) of a point that is exactly one unit from the origin at a given angle 
- @vocab{cosine} -- the x-coordinate (horizontal position) of a point that is exactly one unit from the origin at a given angle
...
@QandA{
@Q{Does computing @vocab{sine} using @vocab{degrees} instead of @vocab{radians} change the shape of the curve? Why or why not?}
