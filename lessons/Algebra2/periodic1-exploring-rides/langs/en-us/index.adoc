...
We're going to explore a new class of functions - _@vocab{periodic functions}_ - that we can use to model cyclical relationships like these.
...
@right{@image{images/unit-circle-clock.png}}Let's think about a simpler example: a clock with a radius of 1, centered around the origin. +
 +
...
@lesson-instruction{
- With a partner, complete the first section of @printable-exercise{unit-clock.adoc}.
}
