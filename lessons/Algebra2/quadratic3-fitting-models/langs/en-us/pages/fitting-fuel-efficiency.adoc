...
We've been talking about coefficients and vocabulary for quadratic functions that look a lot like the ones you might see in a textbook: clean and predictable. Real-world data, however, is messy! Let's return to our @opt-starter-file{alg2-fuel-efficiency} to dig into that messiness.
...
@lesson-instruction{
Return to the @starter-file{alg2-aaron-judge} and work through @opt-printable-exercise{model-speed-v-mpg.adoc}.
}
