= Modeling Bat Angle v. Hit Distance - A.Judge (Sliders)
++++
<style>
/* Shrink fill in the blanks */
.studentAnswerMedium { min-width: 10em !important; }
.contract .editbox { width: 100% !important; }
.fitb, .fitbruby, .fitb.stretch::after, .fitbruby.stretch::after { padding-top: 0.6em !important; }
...
@link-instructions{
For this section, open *Exploring Quadratic Functions - A.Judge (Desmos)* to *Slide 5: Fitting a Model: Bat Angle v Hit Distance (Sliders)*. +
}
...
@link-instructions{
Open your copy of the @opt-starter-file{alg2-aaron-judge}. Adjust the definition for `sliders(x)` on line 48 and click "Run".
}
