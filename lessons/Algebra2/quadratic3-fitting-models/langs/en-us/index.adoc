...
@lesson-instruction{
- Build a scatter plot using the `curve-table`, with `hit-angle` as the @vocab{explanatory variable} (on the x-axis) and `hit-distance` as the @vocab{response variable} (on the y-axis).
}
...
- Return to the @starter-file{alg2-miguel-cabrera} and modify the `curve` function on line 47 using the model settings from the model you fit in Desmos.
...
@opt-printable-exercise{cabrera-model-4seam.adoc, 4-seam}, @opt-printable-exercise{cabrera-model-curves.adoc, curves}
...
- If your students are more into the Yankees, you can find parallel versions of each of these pages focused on data from Aaron Judge, which rely on @opt-starter-file{alg2-exploring-quadratics-desmos-judge} and @opt-starter-file{alg2-aaron-judge}:
