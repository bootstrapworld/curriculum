...
++++
@description{Students use a custom-built Desmos slider activity to visual fit a model to the data, compute its fit in Pyret, and interpret the results.}
...
- Then complete the second section, which explores a new Pyret function: @show{(code 'sqr)}.
}

@QandA{
@Q{What does @show{(code 'sqr)} do and why will we need it in order to build quadratic models in Pyret?}
