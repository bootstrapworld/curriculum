 ...
 *** Open @starter-file{alg2-exploring-quadratics-desmos-cabrera}.
 *** Make a link or code to share with your students.

- _If you're a first-time Desmos user, fear not!_ @dist-link{pages/desmos-instructions-alg2.adoc, _Here's what you need to do._}
- Decide whether you want students to model and interpret all of the datasets in the Desmos file. If it's a priority for you, you may want to print the optional page for each dataset (
@opt-printable-exercise{cabrera-model-4seam.adoc, 4-seam},
@opt-printable-exercise{cabrera-model-all.adoc, full}) to scaffold that work.
}
...
@lesson-instruction{
- Open the @starter-file{alg2-miguel-cabrera} and click "Run".
...
@lesson-instruction{
- Build a scatter plot using the `sliders-table`, with `id` as the label, `hit-angle` as the @vocab{explanatory variable} (on the x-axis), and  `hit-distance` as the @vocab{response variable} (on the y-axis).
}
...
*Let's try to build the best possible quadratic model we can for `hit-angle` v. `hit-distance`*, using what we know about how @math{a}, @math{h}, and @math{k} influence the shape of a parabola.

@lesson-instruction{
- Complete the first section of @printable-exercise{cabrera-model-hit-angle-v-hitdistance.adoc}, using the scatter plot you just made in Pyret and what we know about the model settings in the Vertex form of a Quadratic Model.
}
...
@A{@math{k} should be about 390, because the y-value of the vertex is around there.}
}
...
@teacher{
Make sure you have shared the link or code you made for your class to @starter-file{alg2-exploring-quadratics-desmos-cabrera}.
}

@slidebreak{Investigate-DN}

@lesson-instruction{
- Open *Exploring Quadratic Functions (Desmos)*. You should be on  *Slide 3: Fitting a Model: Hit Angle v Hit Distance*.
- Turn to the *Fitting the Model Visually in Desmos* section of @printable-exercise{cabrera-model-hit-angle-v-hitdistance.adoc} and define your best-guess model for the relationship between `hit-angle` and `hit-distance` in the curve ball data.
- Return to the @starter-file{alg2-miguel-cabrera} and modify the `sliders` function on line 39 using the model settings from the model you fit in Desmos.
- Most estimated vertices probably have an x-coordinate close to @math{31} or @math{32}. Evaluate `sliders(31)` (or your estimated x). Did you get your estimated y back?
- Use `fit-model` to determine how well your model predicts hit distance from hit angle.
- Record and interpret the results by completing @printable-exercise{cabrera-model-hit-angle-v-hitdistance.adoc}.
- @opt{Slides 3-6 of the Desmos file each focus on other subsets of Miguel Cabrera's batting data.
 * Visually fit models for these datasets in Desmos.
 * Update the last three functions defined in the Pyret file.
 * Write `fit-model` expressions to test them out and interpret the @vocab{S-value}.
 * Your teacher may have printed pages to support you with modeling and interpreting these other datasets: 
  *** @opt-printable-exercise{cabrera-model-4seam.adoc}
  *** @opt-printable-exercise{cabrera-model-curves.adoc, curves}
  *** @opt-printable-exercise{cabrera-model-all.adoc, full}
  }
}
...
@Q{Slides 4-5 in the Desmos file display data for specific pitch types. Slide 6 shows the full dataset. Which of the subsets is most visually similar to the full dataset and what does that tell us?}
...
@A{`count(hit-table, "pitch-type")` reveals that 93 of the 262 pitches are four-seam fastballs.}
}
...
- If your students are more into the Yankees, you can find parallel versions of each of these pages focused on data from Aaron Judge, which rely on @opt-starter-file{alg2-exploring-quadratics-desmos-judge} and the @opt-starter-file{alg2-aaron-judge}:
  ** @opt-printable-exercise{judge-model-sliders.adoc}
  ** @opt-printable-exercise{judge-model-4seam.adoc}
  ** @opt-printable-exercise{judge-model-2seam.adoc}
  ** @opt-printable-exercise{judge-model-all.adoc}

- If baseball isn't their focus, @opt-printable-exercise{model-speed-v-mpg.adoc} engages students with visually fitting a quadratic model for @opt-starter-file{alg2-fuel-efficiency, fuel efficiency data} using Slide 8 of the Desmos file and what they know about the model settings in the vertex form from the shape of the data. As with the activity they just completed, students will then define and fit their model in Pyret, and interpret the results.
