...
*Let's try to build the best possible quadratic model we can for our Miguel Cabrera Batting Data*, using what we know about how @math{a}, @math{h}, and @math{k} influence the shape of a parabola.
...
- Turn to the *Fitting the Model Visually in Desmos* section of @printable-exercise{model-batangle-v-hitdistance.adoc} and define your best-guess model for the relationship between `hit-angle` and `hit-distance` in the curve ball data.
