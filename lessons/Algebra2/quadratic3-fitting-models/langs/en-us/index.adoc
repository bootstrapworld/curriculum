...
- Return to the @starter-file{alg2-aaron-judge} and modify the `curve` function on line 47 using the coefficients from the model you fit in Desmos.
...
- @opt-printable-exercise{model-speed-v-mpg.adoc} engages students with visually fitting a quadratic model for @opt-starter-file{alg2-fuel-efficiency, fuel efficiency data} using Slide 8 of the Desmos file and what they know about the coefficients in the vertex form from the shape of the data. As with the activity they just completed, students will then define and fit their model in Pyret, and interpret the results.
