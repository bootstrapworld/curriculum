...
<style>
/* Make autonums bold */
.autonum { font-weight: bold;  padding-top: 4px !important; }
.fitbruby { min-width: unset !important; }
.fitruby, .fitb { --fitb-pitch: calc(1.1rem + 1em); }
...
The curve you'll see is the graph of @math{r(x)}, a logarithmic function. Another, *identical* curve @math{s(x)} is hiding behind it.

@n Use the starting values of @math{a}, @math{b}, and @math{k} to complete the following equation: @math{r(x) = s(x) = }
@fitbruby{5em}{@ifsoln{1}}{log coefficient}
@math{\text{log}}@fitbruby{1.5em}{@math{_{\;}}}{base}@math{(x)}@hspace{0.5em}@math{+}
@fitbruby{5em}{@ifsoln{0}}{vertical shift}
...
Change @math{s(x)} to @math{s(x) = 1 \log_{10} (x) + 0} so that the dashed blue curve lands on top of the solid red curve.
