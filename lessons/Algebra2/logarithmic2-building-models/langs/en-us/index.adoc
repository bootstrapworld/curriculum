...
Just as there are fact families for multiplication and division (_e.g._ {nbsp}@math{2 \times 3 = 6} @hspace{1em}@math{3 \times 2 = 6} @hspace{1em}@math{6 \div 2 = 3} @hspace{1em}@math{6 \div 3 = 2}), there are fact families for logarithms and exponents.
...
@lesson-point{Since any number to the zero power is @math{1}... {nbsp}no matter the base, {nbsp}{nbsp}@math{log(1)}{nbsp} will always be zero!}
...
The term {nbsp}@math{k}{nbsp} is the vertical shift of the function, which moves the curve up or down.

- Since {nbsp}@math{\log_{anything}1 = 0},{nbsp} the value of a standard log model at {nbsp}@math{x=1}{nbsp} will always be {nbsp}@math{0 + k}.
...
The term {nbsp}@math{a}{nbsp} is called the logarithmic coefficient, which - along with the base (@math{b}) - affects the vertical stretch or compression of the logarithmic function and helps us determine how quickly the function grows.
...
Extremely observant students may notice that there's a relationship between @math{a} and @math{b}, where the value of {nbsp}@math{2 \log_{10}(10) = \log_{10}(10^2)}!}
