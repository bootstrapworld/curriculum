...
|@image{images/MI-covid.png}
|
|@image{images/MI-covid-flipped.png}

|===

@slidebreak{LaunchR}

@ifslide{@image{images/MI-covid-flipped.png}}
We see that:
...
@lesson-point{Logarithmic functions have vertical asymptotes. +
Unless the model specifies a horizontal shift, the asymptote of a logarithmic function will fall on the y-axis.
}
...
@QandA{
@Q{Which of the functions on this graph is logarithmic, and how can you tell?
@center{@image{images/expt-log-asymptotes.png, 400}}
}
@A{Exponential fucntions have horizontal asymptotes, whereas logarithmic functions have vertical ones!}
@A{The blue function has a vertical asymptote at @math{x = -5} so it is logarithmic.}
@A{(The red function has a horizontal asymptote at @math{y = 5}.)}
}
...
@lesson-instruction{
Turn to @printable-exercise{evaluate-log.adoc} and complete the table in the first section by writing the related exponential or logarithmic equation for each row.
}
...
@lesson-instruction{
- Complete the rest of @printable-exercise{evaluate-log.adoc} to get some practice translating and evaluating log expressions.
...
When it comes to fitting logarithmic models to data, can you out-model an AI?

@lesson-instruction{
Turn to @printable-exercise{grading-ai.adoc}, and grade the performance of an AI that was told to fit logarithmic models to real data!
}
...
@QandA{
@Q{What _similarities_ do you see between exponential and logarithmic functions?}

@Q{What _differences_ do you see between exponential and logarithmic functions?}
}