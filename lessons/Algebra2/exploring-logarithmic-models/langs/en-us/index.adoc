...
@right{@image{images/difference-table-logarithmic.png}}The behavior in this relationship is the _reverse_ of exponential growth: instead of multiplying @math{y} by a constant (called the @vocab{growth factor}) to find the next value, we need to _multiply the size of our x-interval_ by a constant (called the @vocab{base}) just to get a constant growth in @math{y}! This relationship can be modeled using a @vocab{logarithmic function}.
...
@lesson-instruction{
- Complete @printable-exercise{classifying-tables.adoc}. If the table shows a logarithmic relationship, what do you think the base is?
