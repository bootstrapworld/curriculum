...
@right{@image{images/inverse-covid-graph.png, 375}}Swapping the x- and y-axes makes the graph _start_ with rapid growth, and then flatten out.

- Now we can answer those inverted questions!
...
_A logarithmic relationship looks exponential, if your x's and y's trade places!_
...
- As humans with logarithmic _perceptions_ of loudness, we instead measure "loudness" on a scale that matches our _perception._
...
The base @math{b} of the logarithm is similar to the base (or @vocab{growth factor}) in an exponential expression...it just answers the opposite question!
...
@QandA{
@Q{What question is @math{log_2(1)} asking?}
@A{"To what power must 2 be raised, to reach 1?"}
...
@QandA{
@Q{Thinking back to exponential functions, what is *any* base raised to the power of zero?}
@A{1.}
@Q{What do you think @math{\log_{\mbox{anything}}(1)} will be?}
}

@slidebreak

@center{@math{\log_{\mbox{anything}}(1) = 0}}

Remember, logs and exponents are closely related, with one being the inverse of the other! So if an exponential function to the power of zero is one _regardless of base_, then any logarithmic function of one is zero, _regardless of base!_
...
@QandA{
@Q{What do you remember about the asymptote of an exponential function?}
