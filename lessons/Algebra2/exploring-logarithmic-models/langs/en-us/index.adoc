...
Logarithmic relationships grow very quickly at first, but then level out as @math{x} increases until they approach a horizontal line.

Optional: For practice differentiating between quadratic, exponential, and logarithmic relationships, optional exercises are available for @opt-printable-exercises{classifying-tables.adoc, Tables}, @opt-printable-exercises{classifying-graphs.adoc, Graphs} and @opt-printable-exercises{classifying-defs.adoc, Definitions}.
