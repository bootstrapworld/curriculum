...
@lesson-instruction{
- Working in pairs or small groups, complete @printable-exercise{fitting-models.adoc} using the *Desmos* link I've shared with you to a file called *Fitting Wealth-v-Health and Exploring Logarithmic Models*.}

@teacher{
This activity involves lots of trial and error, as students try to come up with a good linear, quadratic, or exponential model. The goal is to discover that a new kind of model is necessary, not to find a perfect fit with linear, quadratic, or exponential models!
}
=== Synthesize
...
|Instead of multiplying @math{y} by a constant to find the next value, we need to _multiply the size of our x-interval_ by a constant (called the @vocab{base}) just to get a constant growth in @math{y}! 
...
- You should now be on a new slide: *Slide 4: Exploring Logarithmic Functions* 
...
Note: *The base @math{b} of the logarithm* is similar to the base in an exponential expression.
...
@lesson-instruction{
- Complete the first section of @printable-exercise{changing-scale.adoc} ("Fitting a Logarithmic Model"), using __Slide 5: Wealth-v-Health (Logarithmic)__ of the *Desmos* file.
