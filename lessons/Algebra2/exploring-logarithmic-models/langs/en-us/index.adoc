...
@teacher{Direct students to look at the scatter plot they created in Pyret of `median-lifespan` versus `pc-gdp-in-thousands`.}

@QandA{
@Q{What's the best model you came up with?}
@Q{What kinds of lines or curves did you draw?}
@Q{How would you describe the shape of the scatter plot for `median-lifespan` versus `pc-gdp-in-thousands`?}
@A{It starts out rising fast on the left, before tapering off as `pc-gdp-in-thousands` increases to the right.}
...
@strategy{@math{\log = \log_{10}}}{
While logarithms can technically have any base, it is implied that the @math{b=10} for any instance when @math{log} appears without @math{b}.

The Bootstrap pyret function `log` always uses base 10. If you want to work with another base in pyret, you can look up the `log-base` function in the @dist-link{Contracts.shtml, contracts pages}.

Most math books, desmos, and calculators only offer @math{\log_{10}} and @math{ln}, which is equivalent to @math{\log_e} - the natural log.
}
...
@lesson-point{
We need something like Pyret's `lr-plot` function, which uses computational methods to find the best possible model.} 

Unfortunately, `lr-plot` only finds linear models!
...
- With that image in mind, follow the directions in the last section of @printable-exercise{changing-scale.adoc}.
}
...
We tried changing the scale on the x-axis from linear to exponential, which cancels out the logarithmic behavior by "shrinking the axis". Another strategy is to "shrink the data", by _transforming the x-coordinates themselves_. Instead of plotting `pc-gdp-in-thousands` on a *logarithmic* x-axis, we could plot `log(gdp)` on a *linear* x-axis.
...
@lesson-instruction{
- Let's return to the *Fitting Wealth-v-Health and Exploring Logarithmic Models Desmos file*.
- You should now be on the slide 6: "Wealth-v-Health (Transformed)".
...
@A{Record different students' responses for @math{m} and @math{b} on the board. These numbers should be somewhat close to their earlier responses for @math{a} and @math{c}!}
...
We transformed the `pc-gdp-in-thousands` column in three steps:

1. We defined a transformation function, @math{g(x)}, which produces the log of whatever it's given (the `pc-gdp-in-thousands` column).
...
@right{@image{images/lr-plot-transformed.png, 350}}Just like in Desmos, transforming the `pc-gdp-in-thousands` column with a log function produces a scatter plot showing a linear pattern in the data! Pyret's `lr-plot` tool computes the best possible linear model for our transformed data, determining it to have a slope of `11.9011` and a y-intercept of `59.967`. Our @math{S} has dropped to 4.49, showing a vastly better correlation than before.

@slidebreak

With the transformation applied, our linear model is (in function and pyret notation):

@indented{@math{f(x) = 11.9011x + ~59.967}}

@indented{@show{(code '(define (f x) (+ (* 11.9011 x) 59.967)))}}
...
@indented{@math{h(x) = 11.9011 \log_{10}(x) + 59.967}}

@indented{@show{(code '(define (h x) (+ (* 11.9011 (log x)) 59.967)))}}
