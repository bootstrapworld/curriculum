...
- Is there another kind of function that starts off rising fast, but then levels off quickly?
...
- What clues did you use to help you identify if a function was linear, quadratic, exponential, or logarithmic?
...
- The term @math{a} is the _base_ of the logarithm. Look back to your answers on @printable-exercise{classifying-tables.adoc}. For tables that represent logarithmic functions, by what factor does @math{x} need to grow in order to get a constant increase in @math{y}?
...
- The term @math{c} is something we've seen before: it's the _vertical shift_ of the function, which moves the curve up or down (in other kinds of functions, this is often given the name @math{c}). It's also the y-value of the @vocab{asymptote} for the function.
