...
To complete this page, you'll need to open @starter-file{alg2-exploring-logarithms-desmos}. The curve you'll see is the graph of @math{h(x) = 1 \log_{2} x + 0}. Another curve @math{g(x)} is hiding behind it, with its coefficients set to make it equivalent to @math{h(x)}.
