...
To complete this page, you'll need to open @starter-file{alg2-exploring-logarithms-desmos}. The red curve is the graph of @math{h(x) = 1 \log_{2} x + 0}. It has @math{a=1}, @math{b=2}, and @math{c=0}. You can modify the curve @math{g(x)} (behind @math{h}, shown in blue) by changing its @math{a}, @math{b}, and @math{c}.
