...
For each question below: (1) explore in @starter-file{alg2-wealth-v-health-desmos}; (2) define and fit your model in Pyret (the starter file already contains sample functions for you to change, called `linear1`, `quadratic1`, and `exponential1`!). Then write the model and the @math{S}.
...
@n Find the best **quadratic model** you can, using the second slide __(quadratic)__ in the Desmos activity.
...
@n Find the best **exponential model** you can, using the third slide __(exponential)__ in the Desmos activity.
