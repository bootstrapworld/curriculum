...
=== Overview
Students explore the State Demographics dataset and, building on a discussion of the data visualizations they previously made using the animals dataset, recognize the unique opportunity scatter plots offer for exploring relationships between columns.

=== Launch
We're going to be working with a dataset about the states in the US.
...
Before we dig deeper into the State Demographics data, let's think back to our @lesson-link{fitting-models} lesson...

@QandA{
@Q{Does a lizard's weight play a role in how long it takes to be adopted? Why or why not?}
@A{The goal here is to have an open discussion and draw students in. Allow students to share their opinions freely.}
@A{(For example: Yes, little baby lizards get adopted quickly because they're so cute! No, baby lizards require too much work when they are young.)}

@Q{What kind of data does the `age` variable represent? What about `pounds`?}
@A{Both `age` and `pounds` are quantitative variables.}

@Q{What kind of data visualization would helped us analyze the relationship between weight and adoption time?}
...
@A{We used @math{S} - the Standard Deviation of the Residuals - to measure fitness. The smaller the @math{S}, the better the fit.}
}
