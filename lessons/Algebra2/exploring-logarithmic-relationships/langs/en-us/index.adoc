...
- Working in pairs or small groups, complete questions 1-3 on @printable-exercise{exploring-countries.adoc}.
--
...
Since the values on the y-axis (`median-lifespan`) appear grow logarithmically, we could _transform_ the x-axis (`pc-gdp`) to grow logarithmically as well. With both x- and y-valus growing logarithmically, they would appear grow at a constant rate relative to one another. A constant rate will appear linear, which allows us to use `lr-plot` to find the optimal model!
...
[.lesson-instruction]
Complete the remainder of in @pages{transforming-wealth.adoc}
