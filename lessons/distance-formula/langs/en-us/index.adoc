...
Our game computes distance using a function called `line-length`. Let's explore how it works!
...
Using the `line-length` function explicitly connects the distance formula back to the 1-dimensional distance students know, allowing them to apply prior knowledge and better connect back to the Pythagorean Theorem itself. This effectively rewrites the distance formula as:
...
* If we had a right triangle whose legs were 3in and 4in, how would we use the Pythagorean Theorem to find the hypotenuse of the triangle?
--
...
* Then turn to @printable-exercise{pages/distance-coe.adoc} in your student workbook. Convert this expression to a Circle of Evaluation, and then to code.

Optional: Have students use this @printable-exercise{pages/Distance-bw-Two-Points-Multiple-Representations.pdf, Multiple Representations: Distance between two points} to model the distance between two point with the Circles of Evaluation, Code, Computations and a Graph. +
