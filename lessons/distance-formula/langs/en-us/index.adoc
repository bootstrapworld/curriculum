...
*Here's a link to a @opt-starter-file{sample-game-no-distance-lines} you can use if you're demoing on the board.*
...
[.lesson-instruction]
Turn to @printable-exercise{pages/distance-coe.adoc} in your student workbook. Convert this expression to a Circle of Evaluation, and then to code.
...
- Have students practice using the Design Recipe to solve `line-length` on their own, and/or  modify it to make use of the absolute value function: @show{(code 'abs)}.
