...
@ifproglang{codap}{
== Ordering Tables, Part 1 @duration{15 minutes}}
...
@ifproglang{codap}{
== Ordering Tables, Part 2 @duration{10 minutes}}
...
@ifproglang{codap}{
* Open the `Transformer` plugin, and choose the transformer `Sort`. Select `animals-dataset`. In the formula expression box, type `sortItems(Name)`. Select `ascending` as the direction and `Apply Tranformer`. What happens?
...
* Now, enter `is-cat` as the `Name of New Attribute` and try typing `Species = “cat”` if the formula expression box. What do you get? What do you think is going on?
