...
Hand the Filter Method a function card from the @opt-printable-exercise{pages/function-cards.adoc, Function Cards} set.  Publicly tell that student that this is __their function__. Instruct them to review their function’s name, contract, and purpose statement, but to keep that information a secret.

Explain to the class that the Filter Method will walk from one student to the next, referring to their unique function (on the card) in order to determine if each student should stand or sit.

Here’s how that might look if the Filter Method chose a function called `likes-pizza`.

Filter Method to Student 1: Do you like pizza?

Student 1: Yes

Filter Method: Remain standing. (Student 1 remains standing.)

Filter Method to Student 2: Do you like pizza?

Student 2: No.

Filter Method: Sit down. (Student 2 sits.)

After the Filter Method has applied the function to each student, ask the class: Based on who sat and who stayed, what function was the Filter Method applying? _What does the Filter Method do to a dataset?_
