...
|
@span{.title}{Why import a sheet into Pyret?}

- When Pyret imports a sheet, it allows us to keep the original sheet separate from our work environment. We can't delete or overwrite rows, cells, or columns, guaranteeing that these kinds of human errors will not jeopardize the validity of our analysis.
- Pyret holds us accountable for using clean and complete data so that all displays can be trusted. _When using a spreadsheet, which will make a pie-chart out of any column, no matter what's missing, each cell has to be checked by hand, in order for any data displays to be trusted!_
- When we filter and build columns in Pyret we make new tables. This means that we can always go back to the original. _When working with spreadsheets we have to choose between making copies of tables or modifying the original table. Modifying the table means we can never go back to the original. And if we discover that we need to remove two of the dogs from our analysis, and we've made copies, we need to remember to remove those dogs from every copy!_
- The ability to undo a mistake is highly beneficial for students, teachers, and Data Scientists alike!
- For students who are comfortable writing code, programming is a vastly more powerful and learner-friendly tool for Data Science!
|===
