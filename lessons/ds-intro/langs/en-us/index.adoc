...
| Preparation
|
* *You know your students best!* You can use the sample @opt-printable-exercise{pages/OpeningQuestions.adoc} we've provided, but we recommend changing or adding your own questions that are _appropriate, relevant, and engaging_ for your students.
...
=== Launch
Have students open the @link{https://docs.google.com/spreadsheets/d/1VeR2_bhpLvnRUZslmCAcSRKfZWs_5RNVujtZgEl6umA/edit?usp=sharing, Animals Spreadsheet} in a browser tab, or turn to @printable-exercise{animals-dataset.adoc} in their Student Workbooks.
...
After that, we see a line of code that _defines_ `shelter-sheet` to be a spreadsheet. This table is loaded from Google Drive, so now Pyret can see the same spreadsheet you do. If you look carefully, you'll notice that the address listed here is the same address as the spreadsheet we just looked at! After that, we see the following code:
