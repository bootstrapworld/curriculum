...
Every table is made of cells, which are arranged in a grid of rows and columns. _The first row and first column_ are special. The very top row (shaded light blue, beneath the spreadsheet name) indicates how many _cases_ are represented in the table. The table's first row is called the @vocab{header row}, which gives a unique name to each variable (or “column”) in the table. The first column in the table is the @vocab{index}, which contains a unique number for each row, starting with 1.

[.lesson-instruction]
- How many variables are listed in the header row for the Animals Dataset? What are they called?
