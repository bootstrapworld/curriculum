...
* @link{pages/OpeningQuestions.html, Opening Questions}
...
@ifproglang{codap}{
== Meet CODAP! @duration{10 minutes}

=== Overview
Students open CODAP and see the Animals Dataset reflected there.
...
Every table is made of cells, which are arranged in a grid of rows and columns.The very top row (shaded light blue, beneath the spreadsheet name) indicates how many _cases_ are represented in the table. _The first row and first column_ are special. The table's first row is called the @vocab{header row}, which gives a unique name to each attribute (or “column”) in the table. The first column in the table is the @vocab{index}, which contains a unique number for each row, starting with 1.

[.lesson-instruction]
- How many attributes are listed in the header row for the Animals Dataset? What are they called?
- Try changing the name of one of the columns. How did you do it?
- Can you figure out how to add an extra attribute to the table?
- Can you add an additional case to the table?
- What happens when you click the "case card view" icon in the upper lefthand corner of the table?

After the header, CODAP tables can have any number of @vocab{cases}.
Each data row - or case - has values for every column variable. A table can have any number of data rows, including _zero_, as in the table below:
...
CODAP lets us use many different kinds of data. In the animals table, for example, there are Numbers (the number of legs each animal has), Strings (the species of the animal), and Booleans (whether it is true or false that an animal is fixed).
...
In this course, you'll be learning to do all three in CODAP: Display, Filter, and Build.
