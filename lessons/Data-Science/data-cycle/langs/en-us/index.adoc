...
@teacher{
In the future, students will be coming up with their own questions. But, for demonstration purposes, this round we've identified a question for them.
}

(1) **Ask Questions:**
__First we have to think of a good question.__

@lesson-instruction{Let's investigate: __**Are more animals fixed or unfixed?**__
...
(2) **Consider Data:**
__To answer our question we'll need to decide what part of our dataset to focus on!__

@lesson-instruction{
- Turn to the @dist-link{courses/data-science/back-matter/pages/animals-dataset.adoc, Animals Table} or open @link{https://docs.google.com/spreadsheets/d/1VeR2_bhpLvnRUZslmCAcSRKfZWs_5RNVujtZgEl6umA/edit?usp=sharing, the Animals Spreadsheet}.
}
@QandA{
...
(3) **Analyze the Data:**
__Once we know where to look, we can write code to build a display or perform other analysis.__

@ifproglang{pyret}{We could use `bar-chart` __or__ `pie-chart` to do this analysis. Pie chart is the best choice, because we care more about the ratio ("2x as many fixed as unfixed") than the actual count ("20 fixed vs. 10 fixed").} 
...
@QandA{
@Q{What did your display look like?}
@ifproglang{pyret}{@A{Students will see the pie chart below, with a wedge for true and a wedge for false, rather than a wedge for fixed and a wedge for unfixed. They may need help thinking through what the Booleans mean. @image{images/fixed.png}}
} 
...
We've completed our first Data Cycle... and we may want to start a new cycle with the questions that we generated!  But first...

@slidebreak

**We're Ready to Tell our Data Story...**   
...
@lesson-instruction{Let's read this example together.

__"We wanted to know if more animals at the shelter were fixed or unfixed. To answer this, we made a @ifproglang{pyret}{`pie-chart`}@ifproglang{codap}{bar chart}. We found that more animals were fixed (18) than unfixed (14). This made us wonder if about 56% of all species and all ages would be fixed - and whether fixed animals got adopted faster than unfixed ones."__
}

@QandA{
@Q{What information did the Data Scientists include in the data story?}
@A{The question. The process. The findings. And the wonderings those findings generated.}
}
...
@A{It's an __arithmetic question__.}
@Q{What kind of question is "How old is Toggle?"}
@A{It's a __lookup question__.}
@Q{What kind of question is "Are older animals adopted more quickly than younger animals?"}
@A{It's a __statistical question__.}
}
...
**For now, complete __only__ the "Type" column - ignore the other columns titled "Which Rows" and "Column(s)".**
}
...
Tables are made of **Rows** and **Columns**. Each Row represents one member of our population. 

- In the Animals Dataset, each row represents a single animal. 
- In a dataset of temperature readings, each row might represent the temperature at a particular hour.

Columns, on the other hand, represent information __about each row__. Every animal, for example, has columns for their name, species, sex, age, weight, legs, whether they are fixed or unfixed, and how long it took to be adopted.
...
@teacher{If you'd like to start your students with a mini version of the Data Cycle, have them complete @printable-exercise{analyzing-with-count.adoc}@ifproglang{pyret}{
and test their code in Pyret}. 
}
...
- When you're done, work on @opt-printable-exercise{data-cycle-categorical.adoc}. 
