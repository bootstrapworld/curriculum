...
- first, we'll identify the rows for cats 
- then, we'll focus on the `pounds` column __in those rows__

If we want to know how many fixed animals are rabbits:

- first, we'll make a new table of the __rows for fixed animals__, 
- then,  we'll focus only on the `species` column
...
Students who don't internalize the importance of identifying the rows we want to focus on __before__ thinking about which columns we'll need, might mistakenly think we need the `species` column to answer the question, "Which cat is the heaviest?"  But we're not thinking about columns until __after__ we focus in on the cat rows, so the `species` would be cat for all of the rows we're working with!
