...
Remind students that symmetry on a box plot, just like symmetry on a histogram, is not always precise; students should not expect a perfect mirror image every time. It is also important to remember that the data may have greater variability than the box plot presents. 
}
...
@teacher{
Want to check student mastery of the content you've just taught? Administer @assessment{box-plots-interpreting-spread}.
}
...
@teacher{
Students have the opportunity to choose a dataset that interests them from @lesson-link{choosing-your-dataset/pages/datasets-and-starter-files.adoc, "List of Datasets"} in the @lesson-link{choosing-your-dataset} lesson. 

Some teachers opt instead to focus their class on the @opt-starter-file{food}. Those teachers might find it valuable to print copies of @opt-printable-exercise{data-cycle-box-plots-food.adoc} for their students.
}
