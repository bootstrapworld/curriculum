...
- `build-column` consumes a Table, the name of a new column, and a _function that consumes Rows_. It produces a new tble with that new column, and fills in the values by applying the function to every Row.
