...
@right{@image{images/heads.png, 110}} A croupier (a casino dealer) invites you to play a game of chance. They’ll flip a coin repeatedly. On each flip, they'll give you a dollar if it comes up tails. But, if it comes up heads, you'll have to pay them a dollar.
...
Should we suspect a scam if the croupier’s coin flipped heads 10 times in a row? The probability of a fair coin getting no tails in 10 flips is @math{{\frac{1}{2}}^{10}}, or roughly 0.001. So here’s what we’d have to say about our hypothesis test:
...
** _The coin could be fair in all of these instances! Heads 20 times in a row, however, is extremely unlikely._
- What is the relationship between how weighted a coin is, and the number of flips you need to become suspicious that it's weighted?
** _A fair coin should land on heads about 50% of the time._ If a coin has been designed to land on heads _100%_ of the time, it wouldn't take long to figure out that something was up! A trick coin designed to come up heads _60%_ of the time, however, would go undetected much longer. The smaller the bias, the more flips we'll need to see it. A small bias might be enough to guarantee that a casino turn a profit, and be virtually undetectable without a massive sample!
