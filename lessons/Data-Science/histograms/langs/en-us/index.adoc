...
- The number of values in each bin determines the height of the bars on our y-axis.
...
Our @lesson-link{histograms-interpret} lesson addresses these questions... and our @lesson-link{measures-of-center} lesson explores the effect of the shape of a histogram on the mean (average).
}
...
Note: During the next lesson, @lesson-link{histograms-interpret}, students will learn additional vocabulary to help them describe what they see in their histogram. They can add to their histogram interpretations at that point.
}
