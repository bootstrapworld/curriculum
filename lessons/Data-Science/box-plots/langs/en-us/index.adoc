...
[cols="^.>1a,^.>1a,^.>1a"]
|===
| @image{images/dot-plot-2e.png, 90%}
| @image{images/dot-plot-1e.png, 90%}
| @image{images/dot-plot-3e.png, 90%}
|===
...
@A{Q1 is the middle number, but once again we need to take the mean of two middles (2 and 3): 2.5}
...
@A{Q3 is the middle number, but once again we need to take the mean of two middles (7 and 8): 7.5}
}
...
@teacher{@opt{
To reinforce vocabulary, have students complete the @printable-exercise{boxplot-vocab-map.pdf}, which is included in the printed workbook. You could also print copies of @opt-printable-exercise{identifying-shape-box-plots.adoc} to give students an open-ended opportunity to write about shape.
}}
...
@QandA{
@ifproglang{pyret}{@image{images/box-plot-pounds.png, 500}}
...
@A{The animal that weighs 172 pounds may be an outlier.}
@A{While the animals' weights range from 0.1 pounds to 172 pounds, 50% of the animals weigh 11.3 pounds or less.}
}
...
@teacher{
Want to check student mastery of the content you've just taught? Administer @assessment{box-plots-basics}.
}