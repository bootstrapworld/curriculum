...
Note: Decide _ahead of time_ how you wish to manage the "Other" option for eye color in the survey. Removing the option will keep your data clean (preventing misspellings, typos, or inappropriate answers!), but at the cost of completeness (leaving out people who have Heterochromia, for example). The trade-off between data hygiene and completeness is always something survey designers wrestle with!
...
Adding the "Other" option (where _anything_ can be entered!) adds some risk to our analysis.
 
- What possible issues could arise from letting people enter whatever they want?
- What possible issues could arise from forcing everyone to use only than the given choices?
...
- Then complete Part 1 of @printable-exercise{count-bar-pie.adoc}, by testing the expressions @show{(code '(count more-animals "species"))} and @show{(code '(bar-chart more-animals "species"))} in the Interactions Area.
}
...
- Then return to @printable-exercise{count-bar-pie.adoc} and complete parts 2 & 3.
}
...
@teacher{
@opt{
@scrub{LEAVE THIS BLANK LINE so bullets work}

- For more practice making and interpreting these chart types in @proglang, we have a second teaching dataset for you to work with! You can have students create additional visualizations using @opt-starter-file{food}.
