...
* Then complete Part A of @printable-exercise{count-bar-pie.adoc}, by testing the expressions @show{(code '(count animals-table "species"))} and @show{(code '(bar-chart animals-table "species"))} in the Interactions Area.
}
...
@lesson-instruction{Turn to @printable-exercise{bar-and-pie-notice.adoc} and take two minutes to notice and wonder about the displays. Then compare your notices and wonderings with your partner.}
...
@lesson-point{
Stacked bar charts make it easy to compare _between groups_. Multi bar charts make it easy to compare within groups_.
}
...
- For more practice making and interpreting these chart types in Pyret, we have a second teaching dataset for you to work with! Checkout the @opt-starter-file{food}.  
