...
| This lesson uses the @starter-file{expanded-animals}, which includes far more animals than the smaller sample represented in the @starter-file{animals} used in most of our lessons. 
...
@optional Have students complete @opt-printable-exercise{bar-chart-notice.adoc} and @opt-printable-exercise{pie-chart-notice.adoc}.
...
Stacked Bar Charts (left) put the _groups_ side by side, so it's easy to answer which species is the "most female". But it's more difficult to see whether there are more female dogs than male cats, because the bars don't all start from the bottom and they're in percentages rather than raw quantities.

Multi Bar Charts (right) put the _subgroups_ side by side, so it's easy to answer whether there are more female dogs than male cats in the shelter. But it's a little more difficult to see which species is the "most female", because we have to estimate the relative lengths of each bar.

@lesson-point{
Stacked bar charts make it easy to compare _between groups_. Multi bar charts make it easy to compare _within groups_.
}

@lesson-instruction{Complete @printable-exercise{data-cycle-fixed.adoc}.}

@teacher{Note: We have a dedicated lesson for introducing @lesson-link{data-cycle}, but that lesson is not a prerequisite for completing this exercise.}
...
- For more practice making and interpreting these chart types in Pyret, we have a second teaching dataset for you to work with! You can have students complete additional @opt-printable-exercise{data-cycle-categorical.adoc, data cycles} using @opt-starter-file{food}.  
