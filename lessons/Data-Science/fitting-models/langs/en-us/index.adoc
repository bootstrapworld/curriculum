...
It turn out that @link{https://stats.stackexchange.com/questions/12900/when-is-r-squared-negative, *r&sup2;* isn't computed by squaring *r*} and only actually equals @math{r \times r} when @math{r} is calculated using linear regression.

When students are coming up with models on their own, they aren't bound by the algorithm for linear regression and will come up with lines whose fit is worse than `lr-plot` could ever be!
}
