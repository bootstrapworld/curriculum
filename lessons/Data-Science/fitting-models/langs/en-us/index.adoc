...
@description{Students learn about model "fitness" through the value _S_ (Standard Deviation of Residuals).}
...
Let's return to the animals dataset, and see if there's a correlation between an animal's `age` and the `weeks` it takes to be adopted.

@lesson-instruction{
- Open the @starter-file{animals}.
- Make a scatter plot, using the animals' `name` as the labels, their `age` as the x-axis, and the `weeks` it took to be adopted as the y-axis.
}

@slidebreak

@QandA{
@Q{Does there appear to be a relationship here?}
@A{Solicit student answers, facilitate discussion.}
@Q{If so, what is the @vocab{form} of the relationship?}
@A{It appears to be linear.}
@Q{Would you say this relationship is strong? Weak? Moderate?}
@A{Solicit student answers, facilitate discussion.}
}

=== Investigate

Even two people think there's a linear relationship hiding in this data, they might each see a _different_ linear relationship. If they each draw the line they see, one person's line might be steeper than another!

@slidebreak

*How do we even measure how well a model fits?*
...
@Q{How do @math{r}, @math{R^2}, and @math{S} compare for the two models?}
@A{The values are positive for both models and both @math{S} and @math{R^2} values are smaller for `g` than they are for `f`.}
@Q{What does @math{r} tell us?}
@A{It tells us about the strength of the correlation.}
}

@slidebreak


**@math{S} tells us _how well a model fits the data!_**
...
@strategy-box{Optional: Which Model is Best?}{
