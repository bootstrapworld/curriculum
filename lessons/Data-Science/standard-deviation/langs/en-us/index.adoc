...
== Computations Don't Tell the Whole Story
...
The animation below scrolls through a collection of datasets. While the patterns in the scatter plots vary wildly, notice that the corresponding numbers that summarize the datasets barely change at all!
...
@lesson-point{
It is important to look at both the numbers and visualizations before drawing conclusions.
}
