...
MAD and SD are both measures of "how far from the mean all the points in the dataset are". We can imagine a dataset with two points as describing a right triangle with horizontal and vertical legs. How far apart are these two data points?

- Mean Absolute Deviation (MAD) flattens each points' deviation into a single "dimension", averaging the length of the legs of the triangle we imagined.
