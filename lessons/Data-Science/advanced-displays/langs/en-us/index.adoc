...
We've created a special function called `make-color`, which allows students to generate colors based on values in the table.

@show{(contract 'make-color '(("red" "Number") ("green" "Number") ("blue" "Number") ("opacity" "Number")) "Color")}

By supplying Numbers between 0-255 for each of the colors and 0-1 for the opacity, students can create custom colors and use them as arguments to functions like `circle`, `star`, etc. For example:

`star(50, "solid", make-color(100, 200, 0, 0.5))`

...will make a pale green star that's 50% transparent.

Students can get these Numbers from their dataset, use row accessors to extract the values: `star(50, "solid", make-color(r["age"], r["pounds"], 0, 0.5))`
}
