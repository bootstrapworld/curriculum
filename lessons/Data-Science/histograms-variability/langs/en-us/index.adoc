...
== Creating Histograms Manually and in @proglang
...
First, let's practice making histograms by hand and in @proglang.

@lesson-instruction{
- First, use the data provided on @printable-exercise{making-histograms.adoc} to complete the frequency table and corresponding data visualization.
- Then, open the @starter-file{tooth-data} to build a histogram @ifproglang{pyret}{by using the provided contract.}@ifproglang{codap}{by following the given instructions.}
}
