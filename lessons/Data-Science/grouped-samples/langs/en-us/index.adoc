...
@optional For classes where students are using their own datasets, students can use the @opt-printable-exercise{samples-from-my-dataset.adoc} page.
...
- @printable-exercise{design-recipe-helper-funs.adoc, For students who want more Design Recipe scaffolding for Grouped Samples of another dataset...} (For those using printed workbooks, a few of these can be found at the back.)}
