...
@learning-objective{Given a human-language request for a data display involving the entire Animals Dataset, break it down into parts and generate the display.}
@learning-objective{Given a Table, use the `row-n` function to extract any Row from that table}
@learning-objective{Given a Row, use the column lookups to extract the value of any column in the Row}
