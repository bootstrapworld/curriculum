...
@center{@show{(contract 'scatter-plot '((table-name Table) (labels String) (xs String) (ys String)) "Image")}}
}
...
@ifnotslide{@center{@image{images/pounds-v-weeks.png, 400}}}
...
@teacher{Reminder: Students have the opportunity to choose a dataset that interests them from our @lesson-link{choosing-your-dataset/pages/datasets-and-starter-files.adoc, "List of Datasets"} in the @lesson-link{choosing-your-dataset} lesson. If you'd prefer to focus your class on a single dataset, we recommend using the @opt-starter-file{food} and printing copies of @opt-printable-exercise{data-cycle-scatter-plot-food.adoc} to scaffold their work in this section .
}
