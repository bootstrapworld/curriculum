...
Scatter plots show us a collection of points, arranged along two axes. 

@lesson-point{If there's a relationship between the variables on these axes, we'll see clumps and clouds of points in the graph.}

@slidebreak

Here's an example of a point cloud in a scatter plot from a different data set. 

@centered-image{images/animal-p-v-cal-pc.png, 450}

@QandA{
@Q{Do you see a point cloud? If so, how would you describe it?}
@A{We could describe these points as clustering around a line that slopes from the bottom left to the top right.}
}
