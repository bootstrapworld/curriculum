...
@A{We can rule out bar and pie charts, histograms and box-plots because they only show one variable, and this question is about the relationship between two variables.}
@A{The contracts for scatter-plot and lr-plot seem to involve more than one column, so these might be the right choice.}
}
