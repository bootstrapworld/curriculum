...
@lesson-instruction{
- Read the brief story on @printable-exercise{case-study-michelle.adoc} and respond to the questions, providing as much detail as you can.
}

@teacher{Invite students to share their responses. Their responses will likely highlight that data-driven algorithms produce a higher quality output when we provide more data. If your students do *not* propose that the algorithm was *probably* updated (as we suggest in the answer key), that's okay! There's no need to reveal that possibility yet; instead, circle back to @printable-exercise{case-study-michelle.adoc} after your students complete and discuss @printable-exercise{song-recommending.adoc}.}
...
In order for Spotify to do the predictive work that we expect it to do (step 2, above), it must do more than keep a running list of the songs that Michelle listens to. Instead, it must _aggregate_ her listening history: over time, Spotify builds a detailed profile of each user's musical tastes. But what does that entail?

@lesson-instruction{
- Design your own song recommendation system!
- With a partner, complete @printable-exercise{song-recommending.adoc}.
}

@vocab{Training} is the act of transforming data into a model. As you discovered, song recommendation systems need @vocab{training} in order to recommend. In the case of Spotify, the model is a summary of the user's listening habits. Using this model, Spotify can answer questions about new, unseen data (e.g., will Michelle like the latest Taylor Swift song... or not?).
...
@Q{In machine learning, we generally start with a large chunk of data. A model is then generated from the data. That model is generally expected to be a lot smaller than the original chunk of data (but it may still be huge!). The model can be queried from to get answers.
