...
@ifproglang{codap}{
Right now, the easiest way to find the modes in CODAP is to sort a column. Do this by clicking on the column name and then selecting from the drop-down menu either Sort option.) Scan the column to see which values are the most common.}
