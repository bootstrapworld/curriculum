...
@QandA{
@Q{Why would looking at the histogram for a dataset to decide whether *mean* or *median* would be a better measuer of center?}
...
@ifproglang{pyret}{
@teacher{Note: Mode(s) are often used to describe categorical data but won't be in this lesson because Pyret can currently only calculate mode(s) from quantitative columns... keep your ears peeled for news of an update next year!}
}
