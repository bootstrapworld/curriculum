...
One of the oldest datasets studied by machine learning researchers is a dataset all about irises. It includes measurements of the different parts of 150 iris plants. The dataset provides on each iris' species (setosa, versicolor, and virginica), sepal length, sepal width, petal length, and petal width. It would probably be a lot more challenging to build a decision tree from this data!

@image{images/iris-data.png}

(British statistician and biologist Ronald Fisher first published his findings about this dataset in 1936.)
...
We have already built some extremely simple decision trees. We have a sense of heirarchical structure, flexibility, and versatility of decision trees. We know that decision trees sometimes fail when provided with ambiguous or unknown data. We understand the risk of overfitting our data, or building a tree that can only succeed with a narrow dataset.
...
...but how do we make such decisions?! Nowadays, there is a variety of tree-building software; the algorithm used by this software, however, is relatively straightforward.
