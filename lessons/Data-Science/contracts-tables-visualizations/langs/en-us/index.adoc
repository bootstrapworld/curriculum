= Tables, Rows, and Visualizations
++++
<style>
...
Building on their understanding of @lesson-link{contracts, Contracts}, students will be introduced to functions for working with _tables_, including: @show{(code 'sort)},  @show{(code 'count)} and @show{(code 'first-n-rows)}.
...
Let's explore the Contract for another function that works with Tables.

@lesson-instruction{
Complete @printable-exercise{functions-for-tables.adoc}.
}
...
As you've seen from @printable-exercise{functions-for-tables-cont.adoc}, Pyret also lets us define `Row` values. The second input to `row-n` is a Number, called the _index_ of the Row. The index tells Pyret how many data Rows to count from the first one in the table.
