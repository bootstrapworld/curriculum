...
@lesson-instruction{
Turn to @printable-exercise{intro-stacked-multi.adoc} and complete Part A now.
}
...
CODAP allows us to make _stacked_ bar charts, which let us specify both a group and a subgroup.

To create a stacked bar chart:

- Create a graph of randomly distributed points.
- Drag a column to the x-axis.
- From the `Configuration` menu, select "Fuse Dots into Bars".
- Now, drag a different column name onto the display area (*not* onto the y-axis).
