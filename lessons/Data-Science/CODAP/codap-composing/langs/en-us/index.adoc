...
A @vocab{Circle of Evaluation} is a way to organize our thoughts and diagram what we want a given Transformer to do.

The rules are simple:

**1) Every Circle must have one - and only one! - Transformer name written at the top.** Give your Transformers useful names that describe what they do.
...
| @show{(coe '(transformer-name-of-your-choice Dataset Expression))}
| @show{(coe '(transformer-name-of-your-choice Dataset "Name of New Attribute" Expression))}
|===

[cols= "^.^10",options="header"]
|===
| Transform
| @show{(coe '(transformer-name-of-your-choice Dataset "Attribute to Transform" "New Name for Transformed Attribute" Expression))}
|===

- How many arguments does Filter require? How about Build? Transform?
** _Filter reuqires 2 arguments. Build requires 3 arguments. Transform requires 4 arguments._
- Why is naming your Transformers important?
** _Naming Transformers will result in ease of use later. The Transformer's name should tell us what it does!_
--
...
- Respond to the prompts, drawing Circles of Evaluation that represent the Transformers needed to create the desired table.
...
Values like Numbers, String, and Booleans are still written by themselves. It's only when we want to _use a Transformer_ that we need to draw a Circle, and write the values inside from left-to-right.
