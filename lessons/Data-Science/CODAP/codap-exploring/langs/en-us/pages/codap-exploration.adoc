...
div#body.workbookpage .fitb { --fitb-pitch: calc(0.7rem + 1em);}
</style>
++++

Log into the @starter-file{animals} in CODAP and use this page to familiarize yourself with some of CODAP's features.
...
@n Move the `Age` column to between `Fixed` and `Legs`. Click on `Age`. Choose "Sort Ascending (A→Z, 0→9)" from the drop-down menu.

@n Now try "Sort Descending". How many animals have names that begin with S? @fitb{5em}{@ifsoln{6}}

@n Delete a column of the table. (Columns are sometimes called _attributes_.) Use the "Undo" button in the upper right to get your column back. 

@n Do the keyboard shortcuts for Redo (`Ctrl-Y` on PC, `Cmd-Opt-Z` on Mac) and Undo (`Ctrl-Z` on PC, `Cmd-Z` on Mac) work in CODAP? @fitb{10em}{@ifsoln{Yes}}
...
@n Click on the "Rescale" icon (it looks like four arrows pointing in four different directions) to zoom back out and display all data again. Once a graph shows two attributes, can you change it back to a graph with one attribute? @fitb{}{}
...
[.data-table,cols=">.^7a,^.^2a,1,^.^1a,.^10a", stripes="none", grid="none", frame="none"]
|===
