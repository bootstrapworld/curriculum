...
@ifproglang{pyret}{
@lesson-prereqs{contracts-tables, contracts-visualizations}
}
...
@opt-block{
@teacher{
You can launch today's class by taking a survey of your students' first names, then plotting the number of letters in each name as a dot plot on the board.

*Or,* if you've set up our *Live Pyret Survey* as described in the Preparation section at the top of this lesson, here are the instructions for your students:
...
Data Scientists think about two broad types of data:

- @vocab{Quantitative Data} is data where arithmetic makes sense: you can find the name with the "most letters" or the "average age".
- @vocab{Categorical Data} is all about _categories_, like eye color or left-v-right handedness. It doesn't make sense to use arithmetic with categories: you can't ask about who has the "most eye color" or the "average handedness."

@slidebreak{Launch}

This lesson focuses on another way to describe a dataset: its @vocab{variability}. In our discussion of dot-plots, we learned to describe the distribution of a @vocab{quantitative} dataset in terms of outliers, clusters, peaks, and gaps. We also considered what's typical -- or expected -- in the data.
...
In a categorical dataset, we can judge variability based on how _different_ or _similar_ the data points are.
...
@teacher{
- Want to check student mastery of the content you've just taught? Administer @assessment{dot-plots}.
- *Why don't we push a dot plots in the @starter-file{exploration-project}?* Most datasets are far too large for dot plots to be useful. The literature shows that dot plots are a powerful _stepping stone_ to displays like histograms and box plots, but are actually not great visualizations in their own right. It's best to think of them as a pedagogical scaffold, which students should discard as soon as they're comfortable -- which is why we don't want to elevate them as a work product in the exploration project.
}
