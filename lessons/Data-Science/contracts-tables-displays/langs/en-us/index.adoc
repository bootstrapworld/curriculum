...
@A{`<function:triangle>`}
@A{This means that the computer knows about a function called `triangle`.}
}
...
@lesson-instruction{
* Now we know that `triangle` needs 3 arguments. But what kinds of arguments are they?
...
@teacher{The contract is shown on the next slide.}

@slidebreak

@show{(contract `triangle `(Number String String) "Image")}
}
...
@teacher{If you have time and interest in digging deeper we have a @lesson-link{contracts, whole lesson focused on contracts for image-producing functions}, with many scaffolded pages of additional practice.}
...
@opt{If your students would benefit from seeing a few more examples before drawing their own Circles of Evaluation, have them @opt-printable-exercise{matching-coe-to-descriptions.adoc}.}
