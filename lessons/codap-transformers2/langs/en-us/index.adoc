...
We've already explored the `Filter` Transformer, which walks through a dataset, applying the @vocab{Boolean} expression that it was given to each row - and then creating a modified copy of our original dataset.
...
[.lesson-instruction]
Complete the worksheet @printable-exercise{pages/codap-transform.adoc} in the @starter-file{animals} to explore how `Transform Attribute` Transformer.
