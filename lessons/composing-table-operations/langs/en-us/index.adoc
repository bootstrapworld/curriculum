...
** __We need to filter, showing only rows that are greater than 9kg. We also need to add a column that shows weight in kilograms, dividing pounds by 2.2__
...
** __(Sample responses:) It will crash! The computer won't like it!___
...
Let's try diagraming what we need to do for the journalist, using the Circles of Evaluation. We always build first, so let's start there. @ifproglang{pyret}{According to the @vocab{contract}, we know the name of the function is `build-col`, and it needs three arguments: the animals table, the name of the new column `"kilos"`, and the `kilograms` function.} @ifproglang{codap}{We know that our transformer needs three things: the animals table, the name of the new column `"kilos"` and the formula expression.}

@ifproglang{pyret}{
@show{(coe '(build-col animals-table "kilos" kilograms))}
}
@ifproglang{codap}{
@show{(coe '(build-attribute animals-table "kilos" pounds/2.205))}
}

But we also need to filter by that new column, so that we only have animals weighing more than 9kg! That means we need _another_ Circle of Evaluation. We know @ifproglang{pyret}{`build-col`} @ifproglang{codap}{`build-attribute`} goes at the top. But what table are we using for the first argument? It can't be the animals-table again, because that doesn't have a `"kilos"` column.
...
@ifproglang{pyret}{
@show{(coe '(filter (build-col animals-table "kilos" kilograms) is-heavy))}
}
@ifproglang{codap}{
...
[.lesson-instruction]
Complete @printable-exercise{pages/composing-table-operations.adoc}. (For more of a challenge, students can tackle @opt-printable-exercise{pages/composing-table-operations-order-matters.adoc}).
...
A perk of composing Transformers is that everything is just a "view" on the original data, rather than a _change_ made to that data. Changes can cause tables to go out of sync, resulting in hard-to-find bugs and invalid results. With Transformers, any updates made to the original dataset will flow through the composition, keeping everything in sync. Transformers can also be re-used, eliminating duplicate work.
...
[.lesson-instruction]
- How well do you know your @ifproglang{pyret}{table methods} @ifproglang{codap}{Transformers}?
- Complete @printable-exercise{pages/building-from-circles-1.adoc} and @printable-exercise{pages/building-from-circles-2.adoc} in to find out.


=== Synthesize
Ask students about their answers to @printable-exercise{pages/composing-table-operations-order-matters.adoc}.
...
Armed with these tools, we can do some pretty complex analysis! We can even think of data displays as another kind of table operation:

@show{(coe '(box-plot (filter (filter animals-table is-dog) is-young) "age"))}

* What will this Circle of Evaluation produce?
** __A box plot showing the distribution of age, for young dogs__
* Why is it helpful to diagram our code using Circles of Evaluation?
** __Circles of Evaluation don't have any colons, parentheses, etc, so we can just think about **what we want to do**, without having to worry about syntax or typos.__
** __Circles of Evaluation make it easy to see the order-of-operations when we do complex analysis with multiple filters, sortings, and column builders.__
