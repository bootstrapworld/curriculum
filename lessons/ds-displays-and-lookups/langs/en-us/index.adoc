...
@lesson-description{Students continue to practice making different kinds of data displays, this time focusing less on programming and more on using visualizations to answer questions. They also learn how to extract individual rows from a table, and columns from a row. }
...
Suppose we wanted to generate a display showing the ratio of fixed to un-fixed animals at the shelter? How do we go from a simple sentence to working code that makes a data display?
...
. Finally, we need to know *what display* we are using. Is it a histogram? Bar chart? Scatter plots are essential for displaying relationships _between_ columns, but the other displays only deal with one column. Some displays work for @vocab{categorical data}, and others are for @vocab{quantitative data}.

Once we can answer these questions, all we need to do is find the Contract for that display and fill in the Domain!

To display the categorical data in this example, we can choose between pie and bar charts. Which one of these two is best, and why?
...
-  @exercise-link{pages/more-practice-w-lookups.adoc, More practice with lookups}