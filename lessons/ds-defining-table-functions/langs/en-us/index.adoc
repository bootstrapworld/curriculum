...
@workbook-link{ds-intro/pages/animals-dataset-2.adoc}, and walk down the table one row at a time, putting a check next to each animal that is fixed.
...
@link{pages/plot-practice.adoc, "Practice with Plots"}