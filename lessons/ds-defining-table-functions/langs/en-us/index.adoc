...
[.lesson-instruction]
Take two minutes to find all the fixed animals by hand. Turn to @worksheet-link{ds-intro/pages/animals-dataset.pdf}, and walk down the table one row at a time, putting a check next to each animal that is fixed.
...
[.lesson-instruction]
Turn to @worksheet-link{pages/design-recipe.adoc} in your Student Workbook.
...
[.lesson-instruction]
* Use the Design Recipe to solve the word problem at the bottom of @worksheet-link{pages/design-recipe-1.adoc}.
...
But what if we want to filter by a Boolean expression? For example, what if we want to find out specifically whether or not an animal is a cat, or whether it’s young? Let’s walk through an example of a Compute Function using the Design Recipe, by turning to @worksheet-link{pages/design-recipe-2.adoc}.
...
* For practice, try solving the word problem for `is-young` at the bottom of @worksheet-link{pages/design-recipe-2.adoc}.
