...
[.lesson-instruction]
Take two minutes to find all the fixed animals by hand. Turn to @worksheet-link{ds-intro/pages/animal-dataset-2.adoc}, and walk down the table one row at a time, putting a check next to each animal that is fixed.
...
[.lesson-instruction]
* Use the Design Recipe to solve the word problem at the bottom of @worksheet-link{pages/design-recipe.adoc}.
