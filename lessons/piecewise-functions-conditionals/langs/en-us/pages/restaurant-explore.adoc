...
Alice's code has some new elements we haven't seen before, so let's experiment a bit to figure out how it works! *Open the @ifproglang{wescheme}{@link{https://www.wescheme.org/openEditor?publicId=AcThgFgX9z, Alice's Restaurant starter file}} @ifproglang{pyret}{@link{https://code.pyret.org/editor#share=1_TxgoUCDKY3WYn_Vu9J1_2LXkY1o0dx_, Alice's Restaurant starter file}}, click “Run”, and try using the `cost` function in the Interactions window.*
