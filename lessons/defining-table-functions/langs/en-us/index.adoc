...
This would be pretty repetitive! Just as we saw when making green triangles in @link{../defining-functions/index.html, Defining Functions}, __there's got to be a better way!__ In this lesson, we'll learn a step-by-step process that helps us define functions, called the __Design Recipe__.
...
*Note:* the optional lesson @link{../piecewise-functions-conditionals/index.adoc, If Expressions} goes deeper into basic programming constructs, using `image-scatter-plot` to motivate more complex (and exciting!) plots.
