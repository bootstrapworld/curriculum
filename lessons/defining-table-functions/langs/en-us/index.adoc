...
In the previous lesson, we learned that representations of functions have to match. That means we can use these steps to check our work! Look at the Examples carefully - there is one mistake, where the Examples don't quite match the Contract and Purpose. Can you find it?
...
- What did you get? Does this help you explain what's the fourth argument is?
- Try changing the color of the name. Remember: all the representations for the same function need to match! How many places do we need to change the color?
...
[.lesson-instruction]
Turn to @printable-exercise{pages/design-recipe-1.adoc} in your Student Workbook, and fill out the Contract and Purpose Statement for this function. Make sure your Purpose Statement includes all the details you need!

[.lesson-point]
Step 2:  Examples

[.lesson-instruction]
Using the `dog-row` and `cat-row` values defined earlier, write examples for this function that _show their work_. If you're not sure what work to do, look back at the purpose statement!
...
**Now we can see the pattern!** Just as with `nametag`, the only thing changing is the Row itself. Once we circle and label the Rows, we're ready to define the function:
