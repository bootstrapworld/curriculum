...
This is the focus of the entire Bootstrap lesson in @link{../alg-problem-decomposition, Problem Decomposition}, and is used to solve @link{../alg-compound-inequalities, `onscreen?`} and build up the 2-dimensional @link{../alg-distance, distance function}.
...
[.lesson-instruction]
Sailaja is out for a jog, running by the train tracks at 6.5mph. A train passes her, moving in the same direction at 50mph. Ben, a passenger on the train is walking back to their seat, moving in the opposite direction as Sailaja at 2mph. Given a number of *minutes*, what is the distance between Sailaja and Ben?

Have students complete the @workbook-page{pages/top-down-bottom-up.adoc, Top-Down, Bottom-Up!} worksheet, using Problem Decomposition and the Design Recipe to solve this problem!
