...
* create custom scatter plots, using the functions they have defined
...
== Defining Functions over Numbers (Time 20 minutes)
...
== Defining Functions over Other Datatypes (Time 20 minutes)
...
== Scatter Plots with Images (Time 10 minutes)

You've used Pyret functions that use Numbers, Strings, Images, and even Tables and Rows. Now you've written functions of your own that work with these datatypes. But did you know that Pyret functions can even use _other functions_? Look at the Contract for `image-scatter-plot`:
----
 image-scatter-plot :: (t :: Table, xs :: String, ys :: String, f :: (Row -> Image)) -> Image
----
This function looks a lot like the regular `scatter-plot` function. It takes in a table, and the names of columns to use for x- and y-values. Let's take a closer look at the third input...
----
...f :: (Row -> Image)...`
----
_That looks like the contract for a function!_ Indeed, the third input to `image-scatter-plot` is named `f`, which itself is a function that consumes Rows and produces Images. In fact, we just defined a function that does exactly that!
