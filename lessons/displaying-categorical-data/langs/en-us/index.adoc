...
@ifproglang{codap}{
Open the @link{https://codap.concord.org/app/static/dg/en/cert/index.html#shared=https%3A%2F%2Fcfm-shared.concord.org%2FeZEwgF1qSm396MC6CNM9%2Ffile.json, Animals Starter File} in CODAP.

[.lesson-instruction]
- Click the `graph` icon from the horizontal toolbar in the upper left. (See toolbar, below.) What appears?

@span{.right}{@image{images/codap-upper-left-toolbar.png, "", 300}}
...
Note: If students report that a blank graph appears (rather than a scatterplot), prompt them to whitelist CODAP on their ad-blcoker. Ad-blockers do seem to inhibit some of the functionality of CODAP (which will fortunately never advertise to users!).
...
Note: Bar charts look a lot another kind of chart - called a "histogram" - which are actually quite different because they display _quantitative_ data, not categorical. This lesson focuses entirely on @ifproglang{pyret}{pie- and} bar-charts.
...
@ifproglang{codap}{
[.lesson-instruction]
- Have students open a scatterplot of randomly positioned points derived from the Animals Dataset.
