...
@ifproglang{pyret}{
The library included at the top of the file includes some helper functions that are useful for Data Science, which we will use throughout this course. Here is the @vocab{Contract} for a function that makes @vocab{pie charts}, and an example of using it:
...
;;;;; can I add an image of the display that appears in CODAP?
...
@ifprogroglang{codap}{
Students freely explore the CODAP display library. In doing so, they experiment with new charts and develop better intuition for the programming constructs they’ve seen before.}
