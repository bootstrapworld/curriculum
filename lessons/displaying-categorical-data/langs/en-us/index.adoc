...
@left{@image{images/AnalyzeData.png, "Analyze Data icon", 100}} This is an Arithmetic Question. We know it's not a lookup question because there's no ratio written somewhere in the table for us to read. Instead, we'll have to count all the fixed animals and the unfixed animals, then compare the totals.
