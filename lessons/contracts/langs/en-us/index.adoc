...
Sometimes it's helpful to have a contract that tells us more information about the arguments, like what the 3 numbers in a contract stand for. This will not be a focal point of our work, but to give students a taste of it, you can have them turn to @opt-printable-exercise{pages/radial-star.adoc} and use the contract to help them match the images to the corresponding expressions and then turn to  @opt-printable-exercise{pages/star-polygon.adoc} to work with the detailed contract for a `star-polygon`. Both of these functions can generate a wide range of interesting shapes!
