...
- These two starter files provide students with an image of a flag and code that starts out generating a jumbled pile of shapes. Students work to fix the code by resizing, rotating, and correctly locating the components on the background in order to compose an image that looks like the original flag: @starter-file{flags-mx} and @starter-file{flags-pr}

- For a quick dive into why it’s more efficient to define shapes before building the image, open
@starter-file{flags-ak}

- For practice scaling imported graphics, open @starter-file{flags-lebanon}.

- If you’ve already studied Pythagorean Theorem and are ready to apply it, open @starter-file{flags-trinidad}.
