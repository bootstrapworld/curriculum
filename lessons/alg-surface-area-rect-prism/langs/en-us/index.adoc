...
@ifproglang{wescheme}{
- Skip down to the code on line 19.  It reads `(define lst (list front))`, which defines `lst` to be a list of values.  This list should include all of the faces of the prism.  Right now it only includes front.  Add the names of each of the remaining faces to the list. (Order doesn't matter.)
...
[.lesson-instruction]
- When you finish, click run and type 
