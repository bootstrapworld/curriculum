...
Think back to our discussion of random sampling and statistical inference, and look at the scatter plot your drew on @printable-exercise{pages/creating-scatterplot.adoc}. If we picked two random animals out of this table and plotted them, we might not see any pattern at all. We might not see a pattern if we selected four animals, or even six. But as our sample size increases, the chance of us _missing_ a pattern gets smaller and smaller.
...
=== Launch
What other factors might contribute to how long an animal takes to get adopted? _Many! For example, many apartment buildings do not allow large breeds of dogs, and have a limit on how heavy a tenant's dog can be._
...
In the animation below, we see that, while the patterns in the scatter plot vary wildly between a circular pattern and no pattern at all, the numbers that summarize the points on the graph (mean, standard deviation, and correlation) barely change at all!
...
[.lesson-instruction]
--
For practice, consider each of the following relationships (phrased as "how is _explanatory variable_ related to _response variable_"). First think about whether you'd expect the variables to be related, then make the scatterplot to see if your hunch seems correct. If you see any *unusual observations*, try to explain them!
