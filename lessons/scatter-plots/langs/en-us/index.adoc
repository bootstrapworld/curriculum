...
Think back to our discussion of random sampling and statistical inference, and look at the scatter plot you drew on @printable-exercise{pages/creating-scatterplot.adoc}. If we picked two random animals out of this table and plotted them, we might not see any pattern at all. We might not see a pattern if we selected four animals, or even six. But as our sample size increases, the chance of us __missing__ a pattern gets smaller and smaller.
