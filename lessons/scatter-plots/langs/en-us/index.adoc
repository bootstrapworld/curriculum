...
Think back to our discussion of random sampling and statistical inference. If we picked two random animals out of this table and plotted them, we might not see any pattern at all. We might not see a pattern if we selected four animals, or even six. But as our sample size increases, the chance of us _missing_ a pattern gets smaller and smaller.
...
[.lesson-instruction]
--
For practice, consider each of the following relationships. First think about whether you'd expect the variables to be related, then make the scatterplot to see if your hunch seems correct. If you see any *unusual observations*, try to explain them!
