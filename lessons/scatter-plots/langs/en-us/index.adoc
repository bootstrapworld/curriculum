...
This PR moves all standard, practice, and textbook alignments from the build script (static, pre-computed) to JS (dynamic, computed on the fly).

Copycat states no longer need to processed at build time, meaning the dictionary of standards needs only a single copy. This alone saves about 90k LOC, and cuts the size of dictionaries.js by 90%
~700 lines of racket build script have been replaced with ~300 lines of JS
Build times should be marginally faster, since we're no longer hitting the filesystem to compute temp files for each and every standard, textbook chapter, or practice across every lesson
pathway-alignments files go away, replaced by a single, global Alignments.shtml file that can be parameterized by pathway via URL arguments
|===
