...
[.lesson-instruction]
ifeval::["{proglang}" == "wescheme"]
- Recruit three student volunteers to roleplay the functions `safe-left?`, `safe-right?` and `onscreen?`. 
- For each of them, ask the volunteers what their name, Domain and Range are, and then test them out by calling out their name, followed by a number. (For example, "(safe-left? 20)!", "(safe-right? -100)!") *Do not ask `onscreen?` to roleplay beyond their contract!* 
- Make sure that students understand that these functions _always return true_, because that is what the code says they should do! Obviously this is incorrect, and it's our job to fix those functions.
endif::[]
ifeval::["{proglang}" == "pyret"]
