...
During this project, students often discover a second method of producing flags - a method slightly different than the one that we teacher during @lesson-link{transform-compose-images}. Rather than composing functions, some students prefer to define the _layers_ of their flag before arriving at a final outcome.
