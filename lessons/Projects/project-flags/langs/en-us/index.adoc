...
- Consider defining `WIDTH` and `HEIGHT` of their flags as values - and then _replace the numbers in each flag_ with expressions relative to width and height. For example, if the `dot` in the Japanese flag is at (150, 100), those numbers would be replaced with @show{(code '(/ WIDTH 2))} and @show{(code '(/ HEIGHT 2))}.
