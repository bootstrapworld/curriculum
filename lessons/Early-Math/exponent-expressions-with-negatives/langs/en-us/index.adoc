...
@A{Student responses will vary. The Circle of Evaluation on the left is equivalent. In an expression like the one above, we apply the exponent before we apply the opposite. Negating can be viewed as multiplying by @math{-1}. Exponentiation precedes multiplication.}
}
