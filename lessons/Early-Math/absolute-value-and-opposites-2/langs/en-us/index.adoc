...
- When we apply absolute value _last_, we get a positive outcome. When we apply negation _last_, we get a negative outcome. Thinking about the structure of the expression can help us understand if it is positive or negative.
...
[.lesson-instruction]
- Turn to @printable-exercise{true-or-false.adoc} to analyze additional pairs of Circles of Evaluation that use both `negate` and `abs`.
...
[.lesson-instruction]
- Analyze Circles of Evaluation to determine @printable-exercise{wodb-abs-val-negate.adoc}. The workbook page starts with numeric values and then integrates @vocab{variables}. Place a check mark by each Circle of Evaluation that meets the condition stated on the left.
