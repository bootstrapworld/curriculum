...
In this last example, the Circle has been simplified down to just one number; since every Circle of Evaluation must have an operator or function, the Circle disappears.
...
[.lesson-instruction]
- On @printable-exercise{circles-and-computation.adoc}, the Circles represent a step-by-step computation, which results in an answer. Some of the steps are missing numbers!
...
The Circles in this activity were designed to support students in practicing various mental math strategies. If students can recognize structure and avoid computation, that's fine too!
...
[.lesson-instruction]
- Complete @printable-exercise{wodb.adoc}, looking closely at each Circle to determine the one that is not equivalent.

=== Synthesize

- What strategies did you use to determine if Circles were equivalent or not?
