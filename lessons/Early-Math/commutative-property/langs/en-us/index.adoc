...
As a programmer, you will definitely want (need!) to know if you can change around the order of a function's arguments... or if you need to adhere to just one single order. Let's dig in!

=== Investigate


@lesson-instruction{
- Turn to @printable-exercise{commutativity-and-code-images.adoc} and open the @starter-file{comm-and-assoc}
...
@teacher{There is a good chance your students will want to play with and explore @show{(code 'blend-images)}. Please note that color blending in @proglang does not behave exactly as it would in the real world!}
