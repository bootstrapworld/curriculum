...
An interesting feature of these functions is that they require just *one* input: an Image. In a mathematics context, we have applied the Distributive Property to functions that require two inputs... but never one. Let's give it a try now!

@slidebreak

A function `triple` takes a single value and multiplies it by three. Study the pair of Circles of Evaluation below to see if we can distribute the function `triple` over addition.

@ifnotslide{
[.embedded, cols="^.^4,^.^1,^.^4", stripes="none", grid="none", frame="none"]
|===

| @show{(coe '(triple (+ 12 5)))} | &rarr; | @show{(coe  '(+ (triple 12) (triple 5)))}

|===
}

@QandA{
@Q{How does this instance of applying the Distributive Property look familiar? How does it look different?}
@Q{Do both of the Circles of Evaluation (above) produce the same outcome?}
@A{Yes, both Circles of Evaluation result in 51.}
}
