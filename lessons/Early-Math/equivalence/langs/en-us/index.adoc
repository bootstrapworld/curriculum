...
We are now ready to think about whether _lines of code_ are equivalent! To do so, we will consider whether two lines of image-producing code produce perfectly identical images. If so, thoe slines of code are equivalent.
...
@lesson-instruction{
- On @printable-exercise{writing-equivalent-code}, test the provided line of code in @starter-file{editor}.
