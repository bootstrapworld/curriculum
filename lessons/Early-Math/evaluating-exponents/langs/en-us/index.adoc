...
|@show{(coe  '(num-expt 5 2))}		| @show{(coe  '(* 5 5))}
|===
- Are these two Circles equivalent?
** _No; multiplying the base by the exponent is a common student error which produces a different result than multiplying the base by the base however many times the exponent indicates._**
