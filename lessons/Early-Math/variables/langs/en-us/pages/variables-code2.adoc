...
@n Deena wants to edit the starter file's code so that when she hits "Run", all four tests pass. She suggests changing all the values of the variables so that `a`, `b`, and `c` are each equal to zero. Do you agree with her idea? _Before deciding, feel free to test out the idea!_ Explain. @fitb{}{}
