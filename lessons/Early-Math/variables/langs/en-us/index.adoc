...
A hash at the start of a line tells the computer that the rest of the line is for _humans only_. This allows humans to write notes in their code - also called @vocab{comments} - which the computer ignores. To make them stand out, comments are colored @ifproglang{pyret}{dark orange}@ifproglang{wescheme}{purple}. The comments lines 2 and 8 are used to break up the code into sections that make the starter file easier for users to interpret.

@slidebreak

When we want to include @vocab{examples} - what you see on lines 12, 14, 16, and 18 - we start by typing `examples:`, then listing one or more _examples_ of how we want some code to work. Finally, we close with `end`. This formatting not only improves readability, but tells @proglang to test all of these examples and let us know if any of them don't work.
...
@teacher{As students work, encourage them to interact with the starter file! They are free to change definitions, edit examples, and hit "Run" as many times as needed. If they break something, they can always open a fresh copy of the file.
...
@teacher{Students have a tendency to skip over the "prediction" part of these worksheets, but talking about these predictions before diving into the code is really valuable! Discuss student predictions as a whole class, or have students talk with their partner.}
