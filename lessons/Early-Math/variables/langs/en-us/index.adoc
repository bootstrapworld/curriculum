...
A hash at the start of a line of code indicates that the code is for humans only. When we open a line of code with a hash, the text's color automatically becomes dark orange. In other words, the dark orange section titles are read by the computer. Rather, they make the starter file easier for users to interpret.

@slidebreak

When we want to include several @vocab{examples}, we do so by first typing in `examples:` After the examples, we close with the word `end`. This formatting not only improves readability, but flags that we want @proglang to test all of the code within the example block.

@slidebreak

@lesson-instruction{
- Return to your starter file and click "Run".
- Respond to Q2 to Q10 on @printable-exercise{variables-code.adoc}.
}
