...
A hash at the start of a line of code indicates that the code is for humans only. When we open a line of code with a hash, the text's color automatically becomes dark orange. The dark orange section titles that you see (line 2, line 8) are _not_ read by the computer. Rather, they make the starter file easier for users to interpret.

@slidebreak

When we want to include @vocab{examples} - what you see on lines 12, 14, 16, and 18 - we do so by first typing in `examples:`. After the examples, we close with the word `end`. This formatting not only improves readability, but flags that we want @proglang to test all of the code within the example block.

@slidebreak

@lesson-instruction{
- Return to your starter file and click "Run".
- With your partner, respond to Q2 to Q10 on @printable-exercise{variables-code.adoc}.
}

@teacher{As students work, encourage them to interact with the starter file! They are free to change definitions, edit examples, and hit "Run" as many times as needed. If they break something... they can reopen a fresh copy of the starter file. Reading the messages that appear encourages students to think deeply about the Commutative Property *and* gives students exposure to tests - bits of code used to verify that code is working as we would expect. Examples and tests are widely used in programming! We explore examples in greater depth in @lesson-link{functions-examples-definitions}.}
...
@teacher{Students may discuss their predictions with a partner, or you may lead a full-class discussion. Get students talking about how they think @proglang will respond to make the remainder of the activity more impactful.}
