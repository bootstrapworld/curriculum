...
Today is an exciting day! Rather than using an @vocab{operator} at the top of each Circle of Evaluation, we are going to start using some @vocab{functions}!
...
[.lesson-instruction]
--
- Let's try evaluating some Circles with `abs`! What does each of the Circles below evaluate to?
