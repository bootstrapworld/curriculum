...
Students play with the `rotate` function to apply what they learned about absolute value and negation.

=== Launch

In previous lessons, you've explored the `rotate` function. Today, we're going to think deeply about rotations in @proglang. In particular, what angles of rotation represent clockwise turns? Counter-clockwise? What does it mean to _reverse_ a rotation, and how can we represent such a reversal in @proglang?
