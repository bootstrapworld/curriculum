...
- Explore Circles with Variables on @printable-exercise{true-or-false-negate-2.adoc}.
--
...
The function to represent absolute value is `abs`. It can be used like any other operator that we put at the top of a Circle of Evaluation - with one important difference: when we use `negate`, we include just one argument inside the Circle.

[.lesson-instruction]
- Compare expressions with `abs` to expressions with `negate` on @printable-exercise{true-or-false-abs-val.adoc}.
