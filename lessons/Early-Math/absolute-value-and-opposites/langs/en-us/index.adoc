...
- Practice applying `negate` on @printable-exercise{true-or-false-negate.adoc}. Evaluate each side of the equation to confirm if it is true or false.
- Explore Circles with Variables, and review previously explored properties (Identity, Commutative, Associative, and Distributive) on @printable-exercise{true-or-false-negate-2.adoc}.
--
...
In math we want students to read for _comprehension_, not just mechanically sounding out each symbol in order. We read the expression @math{12 - 9} as "12 minus 9" __because subtraction is the "verb"__, not because the minus sign happens to be in the middle!
...
Think about the expressions @math{|h|} and @math{-h}.
