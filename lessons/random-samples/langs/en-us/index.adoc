...
But a trick coin that comes up heads 60% of the time would need a much larger sample to be detected. There's a connection to be found here, between @vocab{sample size} and _deviation_.  A subtle deviation might be enough to guarantee that a casino turn a profit, and be virtually undetectable without a massive sample!
...
=== Launch
In the coin-flip activity, we took samples of each coin and used our knowledge of _probability_ to make @vocab{inferences} about whether the coins were fair or weighted.
