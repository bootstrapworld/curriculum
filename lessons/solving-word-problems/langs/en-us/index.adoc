...
The order of the recipe's steps is a recommendation based on years of research about what works for _most_ students, but that doesn't mean the order works for every student! Some may find it easier to work through a concrete example or two before thinking about Domain and Range, and there's nothing wrong with that. We encourage you to use the Recipe in your classroom as often as possible, teaching students to be flexible with the tools and representations it includes.
|===
