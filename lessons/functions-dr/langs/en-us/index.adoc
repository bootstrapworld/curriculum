...
**For teachers who cover quadratic and exponential functions, or have students who need more of a challenge**, here are some fun ideas and the accompanying @opt-printable-exercise{rocket-challenges.adoc, exercise page}:
...
You can use any word problems you like, but we have provided some two sets that lend themselves particularly well to the activity. One set can be used to collaboratively update the functions in a starter file, to generate a cool mystery image if all three problems are solved correctly!
