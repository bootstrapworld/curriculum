...
@fitb{}{@ifsoln{In the previous solution, the constant we add is the square of the constant we subtract from @math{s}. To reach a height of 1000: @math{height(seconds) = -(seconds - \sqrt{1000})^2 + 1000}}}
...
@fitb{}{@ifsoln{A good challenge for a unit on quadratics! @math{height(seconds) = 0.4seconds^2 + 40seconds}}}
