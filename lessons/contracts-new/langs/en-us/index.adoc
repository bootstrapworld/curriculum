...
- `square` and `star` have the same domain (number string string). Did you find any other shape functions with the same `domain` and `range`? _Yes! `triangle` and `circle`.
- A lot of the `domains` for shape functions are the same, but some are different. Why did some shape functions need more inputs than others? 
	...
	* _`triangle` draws equilateral triangles and all equilateral triangles have three 60 degree angles and 3 equal sides, so one number for the size of the triangle is enough._
