...
Contracts don't tell us _specific_ inputs. They tell us the _@vocab{Datatype}_ of input a function needs. For example, a Contract wouldn’t say that addition requires "3 and 4". Addition works on more than just those two inputs! Instead, it would tells us that addition requires "two Numbers". When we _use_ a Contract, we plug specific numbers or strings into an expression.
...
Open the editor and write code that satisfies each of these contracts. @ifpathway{algebra}{When you've figured out code for each of these, *write it down in the empty line beneath each contract*. These pages will become your reference for the remainder of the class!}
...
@ifpathway{algebra}{Add it to your contracts page!}
...
- How many output are there for a function? _For each input (or collection of inputs) there is exactly one output._
...
=== Investigate
[.lesson-instruction]
@ifpathway{algebra}{Have students turn to the contracts pages in the back of their workbook and experiment in the editor to write code and figure out contracts for each of the shapes listed.}
- Have students turn to @printable-exercise{pages/shapes-contracts.adoc} in the workbook and experiment in the editor to write code and figure out contracts for each of the shapes listed.
...
How to diagnose and fix errors is a skill you will developing throughout this class. Some of these errors will be _syntax errors_: a missing comma, an unclosed string, etc. All the other errors are _contract errors_. If you see an error and you know the syntax is right, ask yourself these two questions:
...
- The same Range? _yes. The range for all of the polygons was _image._
- Are they still different functions? _yes!_
--
