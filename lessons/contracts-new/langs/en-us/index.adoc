...
=== Overview
Students learn how to apply functions in @ifproglang{pyret}{Pyret} @ifproglang{wescheme}{WeScheme}, reinforcing concepts from standard Algebra.
...
Have students complete 
	@ifpathway{algebra}{@printable-exercise{pages/applying-functions.adoc}} 
	@ifpathway{data-science}{@printable-exercise{pages/shapes-contracts.adoc}}.
...
The first part of a contract is the function’s name. In this example, our functions are named `num-sqrt`, `string-length` and `triangle`.
...
@ifpathway{data-science}{*NEED TO MAKE A FILE FOR IMAGE EXPLORATION THAT'S NOT COE DEPENDENT*}
