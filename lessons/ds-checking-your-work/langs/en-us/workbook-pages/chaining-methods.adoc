...
| Produces a table containing only
Toggle and Maple

| `t.filter(is-fixed)`

| Produces a table of only young, fixed
animals

| `t.build-column("sticker", nametag)`

| Produces a table, sorted youngest-tooldest

| `t.filter(is-young)`

| Produces a table with an extra column,
named "sticker"
...
| Produces a table containing the same
four animals
...
| Produces a table with an extra "label"
column, sorted youngest-to-oldest
