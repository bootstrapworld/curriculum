...
There is something special happening in this file called *commenting code*. The programmer who wrote this code included a series of messages for us - humans - that will not be read by the computer.  Commenting can help programmers leave notes for one another when they are working on the same file, or keep track of what they want to remember when _they_ come back to  at their code another day.  Professional programmers use comments all the time because they work with teams who need to be able to follow each other's thinking in order to collaborate efficiently.
