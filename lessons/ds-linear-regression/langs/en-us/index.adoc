...
[.lesson-instruction]
Make two scatterplots for the `animals-table`, using `"age"` as the `xs` in one plot and `"pounds"` as the `xs` in the other. In both plots, use `"weeks"` as your source of `ys` and `"name"` for the `labels`.
...
@span{.right}{@centered-image{images/lin-reg.png, "", 300}}
...
What we need to do is find a _line_ -- called a @vocab{line of best fit}, or a “regression line” -- that is at the center of this cloud. Each point in our scatter plot “pulls” on the line, with points above the line yanking it up and points below the line dragging it down. Points that are really far away -- our outliers and our influential observations -- pull with more force than those that are close to the line. This line can be graphed on top of the scatter plot as a function, called the @vocab{predictor}.

If the line has a positive slope, the @vocab{correlation} is positive. If it has a negative slope, the correlation is negative. Given a value on the x-axis, this line allows us to “predict” what the corresponding value on the y-axis might be. This allows us to make predictions based on our data.
...
=== Investigate
Let's explore scatter plots for age-v-weeks and pounds-v-weeks:

@span{.left}{@image{images/age-v-weeks.gif,"age-v-weeks scatterplot", 400}}
@span{.right}{@image{images/pounds-v-weeks.gif,"pounds-v-weeks scatterplot", 400}}
@span{.clear}

After looking at the point clouds, we are left with two questions:
...
Our goal is to values of the the variable on our x-axis to _explain_ the variable on our y-axis, the x-variable is sometimes referred to as the _explanatory variable_ and the y-variable is referred to as the _response_ or _outcome_ variable.
...
The resulting scatterplot looks like those we’ve seen before, but it has a few important additions. First, we can see the @vocab{line of best fit} drawn on top. We can also see the equation for that line (in red), in the form @math{y = mx + b}. In this plot, we can see that the slope of the line is 0.792, which means that on average, each extra year of age results in an extra 0.792 weeks of waiting to be adopted (about 5 or 6 extra days). By plugging in an animal’s age for _x_, we can make a _prediction_ about how many weeks it will take to be adopted.

The intercept is `2.285`. This is where the best-fitting line crosses the y-axis. We want to be careful not to interpret this too literally, and say that a newborn animal would be adopted in 2.285 weeks, because none of the animals in our data set was that young. Still, the regression line suggests that the baby animals, whose age is close to 0, would take only about 3 weeks to be adopted.

We also see the @math{r}-value listed at the top of the plot, letting us know the strength of the correlation and the direction (though we can see that just by looking at the predictor, as well!).
...
=== Overview
Students learn how to _write_ about the results of a linear regression, using proper statistical terminology and thinking through the many ways this language can be mis-used.
...
[.lesson-instruction]
- Turn to @workbook-link{pages/reading-regression-lines-n-rvalues.adoc}, and match the write-up on the left with the line of best fit and @math{r}-value on the right.
...
- Many students assume the correlation statistic - the @math{r}-value - means that the predictor is accurate @math{r%} of the time. This is profoundly untrue, and is also a _really_ common misconception!
...
Here are some real-life correlations that have absolutely no causal relationship; they come about either by chance or because both of them are tied in with another variable that’s lurking in the background.
