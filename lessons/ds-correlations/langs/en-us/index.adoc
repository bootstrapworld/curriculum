...
@lesson-description{Students continue to interpret scatter plots, and think about correlations in their dataset.}
...
=== Overview
Students identify and make use of patterns in scatter plots, learning to identify patterns which are linear, patterns that are not, and scatter plots for which there is no clear pattern. This builds intuition for identifying a @vocab{form} that can be used with linear regression.

=== Launch
By now we have learned ways to summarize a single quantitative variable, like age or weight of an animal in our dataset: report the center, spread, and shape of the distribution. Together, those numbers tell us what age is typical, how much the ages vary, and what kind of age values are usual or unusual. 
...
Rather than summarizing the distribution in one dimension, we can summarize a _linear relationship_ between two quantitative variables. But this relationship only makes sense if the scatter plot follows a _straight-line pattern_, as opposed to being curved. So the very first assessment we have to make is to identify the @vocab{form} of the relationship as being linear or not.

[.lesson-point]
_Form:_ whether a relationship is linear or not

=== Investigate
The relationship between two variables can take many forms - some patterns are _linear_, and appear as a straight line sloping up or down. Some patterns are _non-linear_, and may look like a curve or an arc. And sometimes there is no pattern or relationship at all!
...
=== Synthesize
Data Scientists use their eyes all the time! It doesn't make sense to search for correlations when there's no pattern at all, and only linear relationships make sense if we're looking for a correlation/@math{r}-value.
...
A relationship between two quantitative variables is strong if the scatter plot points are tightly clustered together. If this is the case, then knowing the x value of a data point gives us a very good idea of what its y value will be. If the relationship is linear and strong, the scatter plot points are clumped together in a thin cloud.

- A *strong* relationship means that the points in the scatter plot are all clustered closely around an invisible line. If the cloud point is very tightly packed around a single line, the relationship is said to be strong. 
- A *weak* relationship means that the cloud is spread out, with points being farther away from the line.
...
An @math{r}-value of about ±0.65 or ±0.70 or more is typically considered a strong correlation, and anything between ±0.35 and ±0.65 is “moderately correlated”. Anything less than about ±0.25 ±0.35 may be considered weak. However, these cutoffs are not an exact science! In some contexts an @math{r}-value of ±0.50 might be considered impressively strong!

Calculating @math{r} from a data set only tells us the direction and strength of the relationship in _that particular sample_. If the correlation between adoption time and age for a sample of about 30 shelter animals turns out to be +0.44, the correlation for the larger population of animals will probably be _close_ to that, but certainly not the same.

=== Investigate
Have students turn to @workbook-link{pages/identifying-form.adoc, Identifying Form} in their student workbooks. For each scatter plot, identify whether the relationship is positive or negative, and strong or weak, and combine those two to guess whether the @math{r}-value is closer to -1, 0, or +1.
...
- *Direction:* If it's linear, does it appear to slow up or down as you move from left to right?
- *Strength:* Is the point cloud tightly packed, or loosely dispersed?
- Would you predict that the @math{r}-value is positive or negative? Will it be closer to zero or closer to ±1? 
...
- Students often believe that strength and sample size are interchangeable, leading to mistaken assumptions like "any correlation found in a million datapoints _must_ be strong!"
...
- Suppose we have two correlations, one drawn from 10 datapoints and one drawn from 50. If both correlations are identical in direction and strength, should we trust them equally when make an inference about the larger population?
...
- Investigate these correlations. If you need blank Design Recipes, you can find them at the back your workbook, just before the Contracts.
