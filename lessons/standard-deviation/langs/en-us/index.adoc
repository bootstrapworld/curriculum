...
One problem with Mean Average Deviation (MAD) is that by flattening each deviation into a single "dimension", MAD allows the contribution of one deviation to cancel out another (+1 cancels out -1, for example). SD is in fact related to the __distance formula__, treating each point as a separate dimension and allowing deviations to be considered independently. Squaring each deviation, taking the average, and taking the square root allows each deviation to contribute to the summary (just like `mean`!).

Standard deviation is used most often, but like `mean` it is sensitive to extreme outliers or skew. In those cases, the MAD is considered a better measure of spread.
|===
