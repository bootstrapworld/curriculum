...
On this section of the worksheet, students will need to stretch their visual imaginations a bit! In problem number 6, they are asked to summarize all 10 distances from the mean into a single number. The goal here is for students to make an educated guess about standard deviation _before_ learning the algorithm for computing it. Invite and encourage discussion about students' different approaches for guessing at the best summary number. Then, share with students the following key idea about standard deviation.
...
** _A standard deviation of zero means that every number in the sample is the exact same._
...
[.lesson-instruction]
- Use what you've learned to find the standard deviation for quantitative columns in your dataset? Complete @printable-exercise{analysis-stdev-my-dataset.adoc}, and add your findings to the "Measures of Center and Spread" section.
