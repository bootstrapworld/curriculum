...
We can diagram the function composition using Circles of Evaluation (see first column, below). In the second column, we've translated that same expression into an Order of Operations, replacing the function names in each Circle of Evaluation with _what each function does_:
