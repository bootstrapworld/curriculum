...
=== Launch
Three of the function cards we just used were for the functions `f`, `g` and `h`, which multiplied x by 3, added six and subtracted one, resepectively. We can compose those function in any order. If we composed them as `f(g(h(x)))` and evaluated them for `x = 4` what would happen?  

We could diagram the function composition using Circles of Evaluation, first using the function names, and then replacing those function names to make the math involved more visible.
...
- The domain and range for @show{(sexp->code `flip-horizontal)} is Image -> Image.  Why can I use the @show{(sexp->code `text)} function as an input for @show{(sexp->code `flip-horizontal)}?
