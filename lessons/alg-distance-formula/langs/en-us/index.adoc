...
[.lesson-instruction]
Practice computing the distance between two _different_ points, by completing @printable-exercise{pages/distance-on-coordinate-plane.adoc} @printable-exercise{pages/distance-from-game-coordinates.adoc}.
...
If we knew the hypotenuse and one leg of the triangle, could we use this formula to compute the other? 

@math{\sqrt{A^2 + B^2 = C^2}}

[.lesson-instruction]
Take a look at the two examples on @printable-exercise{pages/distance-comparing-code.adoc}. There's a subtle difference between the two examples! What is it? Once you see what's going on, try writing code of your own by completing @printable-exercise{pages/distance-write-code-from-lengths.adoc}.
