...
ifeval::["{proglang}" == "pyret"]
Have students open a new program file and name it "Exploring Images".  On Line 1, they should type the words *include image* and press "Run".
endif::[]
...
Have students turn to @worksheet-link{workbook-pages/DomainAndRange-WB2.pdf, "Domain And Range"} in their workbooks and use the Frayer models to explain the concepts of domain and range.
...
ifeval::["{proglang}" == "pyret"]
Students complete @worksheet-link{workbook-pages/DomainAndRange-WB3-Pyret.pdf, "Domain and Range - Practice"} with their partner.
endif::[]
