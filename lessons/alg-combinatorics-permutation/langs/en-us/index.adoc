...
Have students open the @link{https://code.pyret.org/editor#share=1nrGFh4nQGzcmi1caHlWoPSM1mH8kJqNi, Permutations and Combinations Starter File}, save a copy, and click "Run".
...
[.lesson-instruction]
In the Interactions Area, type `menu` to see a list of all the items Luigi will serve.
...
[.lesson-instruction]
- In the Interactions Area, type `permute-with-replace(menu, 1)` to see all the permutations for a 1-course meal. PROTIP: By clicking the first `list`, you can expand it to show each permutation on its own line.
- Does the number of permutations shown here match the number we computed earlier?
- How could we see all of the permutations for a 2-course meal? 3-course? 4?
- Does the number of permutations shown here still match what we expect?
- Write the number of permutations in the definitions area, under #2
...
[.lesson-instruction]
In the Interactions Area, type `small-menu` to see the new, reduced menu.
...
That's six different permutatons that starting with Chicken, and we have four different other possible meals to start with. 
...
[.lesson-instruction]
- In the Interactions Area, type `permute-no-replace(small-menu, 1)` to see all the permutations for a 1-course meal. PROTIP: By clicking the first `list`, you can expand it to show each permutation on its own line.
- Does the number of permutations shown here match the number we computed earlier?
- How could we see all of the permutations for a 2-course meal? 3-course?
- Write the number of permutations in the definitions area, under #5a-c
...
[.lesson-instruction]
- In the Interactions Area, type `permute-no-replace(menu, 1)` to see all the permutations for a 1-course meal. 
- How could we see all of the permutations for a 2-course meal? 3-course? 4?
- Write the number of permutations in the definitions area, under #5d-f
