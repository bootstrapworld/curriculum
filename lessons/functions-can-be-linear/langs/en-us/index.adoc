...
_Each row in the Table represents an (x, y) point on the graph. The "x" column is a list of positions along the x-axis, and the "y" column is filled with values that are computed from those positions according to some rule._
...
[.lesson-instruction]
* Pick another point from the third table and compute the @vocab{y-intercept}. Did you get the same answer?
