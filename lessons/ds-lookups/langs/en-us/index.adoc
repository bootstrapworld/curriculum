...
== Plotting Data @duration{20 minutes}

=== Overview
Students get some more practice applying the plotting functions and working with Contracts, and begin to shift the focus from _programming_ to _data visualization_.
...
=== Investigate
Let's get some practice going from human sentences to code, making visualizations.
[.lesson-instruction]

Turn to @workbook-link{pages/plot-practice.adoc}, and see if you can fill in these three parts for a number of data display requests. When you're finished, try to make the display in Pyret using the appropriate function.
