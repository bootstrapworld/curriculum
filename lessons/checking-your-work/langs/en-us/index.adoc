...
* Make sure all students can access @link{https://code.pyret.org/editor#share=1DBYGJDOlIARaWI3pBVzjPp6eewkQJGHW, the Trust-but-Verify Starter File}
...
* List the names of the animals that you would use in a Testing Table to verify whether the function works as advertised. When you’ve finished, open the @link{https://code.pyret.org/editor#share=1DBYGJDOlIARaWI3pBVzjPp6eewkQJGHW, Trust-but-Verify Starter File}. There are three versions of `fixed-cats` here. Are they all correct? If not, which ones are broken?
