...
[.lesson-instruction]
An AI writes code that claims to filter out any shelter data to show only the cats. Why would it be a bad idea to test it using a table that _already_ contains only cats?
...
[.lesson-instruction]
--
Another programmer writes a function called `fixed-cats` and _claims_ that, given a table of animals, it produces a table with only fixed cats.
