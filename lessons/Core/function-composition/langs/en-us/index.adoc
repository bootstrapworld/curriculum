...
For example, let's say we are going to compose two different functions: `add1` and `double`. If `add 1` takes in 4, we get `5`. After we pass `5` into the _next_ function (`double`), we get an output of `10`.
...
- *Option 1*: Each function card is a stop along the way to a final outcome. The journey from the starting number to the ending number is a process that we've broken down into pieces.

- *Option 2*: The function cards are strung together, forming a single unit. All of the work is done all at once.

@clear

[cols="3a,4a,^.^3a,3a", frame="none", grid="none"]
|===

|

| [cols="1a,2a,1a", stripes="none"]
!===
! step1	! 4			! 4
! step2	! `add1` 	! 5
! step3	! `double` 	! 10
