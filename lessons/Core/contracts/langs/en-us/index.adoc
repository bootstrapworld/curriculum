...
=== Overview
Students learn how to apply functions in @proglang, and are introduced to the notion of @vocab{Contracts}, which are a simple notation (based on function notation in Algebra!) for keeping track of the set of all possible inputs and outputs for a function.
...
In programming languages, values are like nouns. You've learned about values in our programming language, like Numbers (`42`, `-8.3`, etc), Strings (`"hello!"`), and Booleans (`true` and `false`). @ifproglang{pyret}{We already know _some_ verbs, like `+`, and `-` ... each of which _does something_ to Number values.}
...
What do you think this function does? Come up with a theory about what `string-length` does, and try using it with other inputs to test that theory.
...
Contracts are _binding agreements!_ An actor signs a Contract agreeing to perform in a film in exchange for a fee and royalties. A contractor makes an agreement with a homeowner about what they will build or repair in a set amount of time for a predetermined price. A parent agrees to pizza for dinner in exchange for the child completing their chores. A student agrees to certain behavioral expectations in order to have access to computers at school or participate in an overnight trip.
...
@lesson-instruction{
- Knowing all of this, how much can you figure out about a function just by reading its contract? With your partner, complete @printable-exercise{pages/practicing-contracts.adoc} and @printable-exercise{pages/matching-expressions.adoc}.
- _Note_: These pages use made-up functions that are not built into @proglang. Students who try to type them into the computer will get and error!
}
...
Now that we know what a contract is, let's see how error messages in @proglang can help us to figure out the contract for a function we've never seen before.
...
This means that the computer knows about something called `triangle`. It could very well be a function!
...
Suppose we want to see the `text` "Diego" written in yellow letters of size 150. The code would be @show{(code '(text "Diego" 150 "yellow"))}. What would the Circle of Evaluation look like?

@show{(coe '(text "Diego" 150 "yellow"))}

How can we use the `rotate` function with this Circle of Evaluation, to rotate the text 90 degrees?
...
Circles of Evaluation help us think about what we want to do, without worrying about syntax like quotation marks, paretheses, etc. They let us use all our brains for _thinking_, before we use them for _coding_.
