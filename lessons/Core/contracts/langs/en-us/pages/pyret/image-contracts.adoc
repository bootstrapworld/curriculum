...
[.linkInstructions]
Log into @starter-file{editor} and click "Run".@ifsoln{Experiment with each of the functions listed below, trying to find an expression that will build. Record the contract and example code for each function you are able to successfully build!

*Teachers:* Students are only seeing the function names and the contract and example for `triangle`.
}
@ifnotsoln{
