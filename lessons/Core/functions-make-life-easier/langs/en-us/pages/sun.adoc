...
@n *Let's define a function* `gr` *to build a solid, green rectangle of whatever height and width we give it!*
...
@show{(code '(define (gr (?ANS height) (?ANS width)) (?ANS (rectangle height width "solid" "green"))))}
