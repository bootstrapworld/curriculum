...
@teacher{Invite students to take a look at the expressions below:}
...
@QandA{
@Q{Why do you think we didn't need to define @show{(code 'pi)} in this file before using it in our definition of `c`?}
@A{@proglang has some built-in definitions!}

@Q{In question 14, you looked at different ways of writing the same definition. Each way broke the definition up into multiple lines. Which one did you like best, and why?}
...
@Q{Why might programmers decide to define values?}
@A{Answers will vary. For now, just solicit students' ideas. 
   *** We'll come back to this question and improve upon our answer!
   *** Sample response: They realize that it will save them time in the future. e.g., being able to use @show{(code 'pi)} instead of typing out the digits.
}
}
