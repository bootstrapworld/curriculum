...
[cols=".^8a,1,14a", frame="none", grid="none"]
|===
...
@n What image do you see repeated in the flag? +
@fitb{}{@ifsoln{small yellow stars}}

@n *Highlight or underline* every place in the code above that you see the repeated expression for that image. +
...
@star This file uses a function we haven’t seen before! _Hint: Focus on the last instance of the function._
@indented{
What is its name? @fitb{}{@ifsoln{`translate`}} +
How many inputs are in its domain? @fitb{}{@ifsoln{4}} +
What are the types of those inputs? @fitb{}{@ifsoln{Image, Number, Number, Image}}
}
