...
- This file defines the `shelter-sheet` spreadsheet, the `lizard-sample` table, and two functions named `cy` and `jo`.
- Can you find the link to the spreadsheet?
- Can you find where `lizard-sample`, `cy`, and `jo` are all defined?
...
@A{The red dots are the data from the dataset.}
...
*You can't calculate @math{S} if you only have two datapoints!* We use two points to define the line, and if there's no other data left over then there's nothing to fit! In order for us to calculate @math{S}, we need a table with _three or more points._
...
@A{The x and y coordinates of each point in the dataset, and the animals they are associated with.}
...
- Complete the first two sections of @printable-exercise{interpret-models.adoc} with your partner.
}

@teacher{
- The information needed for the first section is most easily gathered by looking at the table (in @proglang) and using the `dot-plot` and `box-plot` functions. 
...
@QandA{
@Q{What do our lines summarize about this dataset?}
@Q{What do they leave out?}
}
...
++++
<style>
div#body .strategy-box.algebra-2 {
  border-radius: 20px;
...
++++
[.algebra-2]
@strategy{What about Non-linear Models and Algebra 2?}{
