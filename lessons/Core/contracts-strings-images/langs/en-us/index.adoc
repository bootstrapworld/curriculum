...
@lesson-instruction{
Turn to number 4 on @printable-exercise{catching-bugs.adoc} with your partner and try to explain the difference between syntax and contract errors in your own words.
}
...
@lesson-instruction{
- Turn to the second section of @printable-exercise{catching-bugs.adoc}.
...
@teacher{
@opt{
@scrub{LEAVE THIS BLANK LINE so that bullets work}

- If time allows, both `star-polygon` and `radial-star` generate a wide range of really interesting shapes! You may want to print copies of @opt-printable-exercise{radial-star.adoc} and/or @opt-printable-exercise{star-polygon.adoc} to scaffold further investigation.
- If your students are ready to dig into more complex triangles, @opt-printable-exercise{triangle-contracts-cont.adoc} will be a good challenge. 
- An option for practice applying what they've already discovered is @opt-starter-file{images-code}.
}
}
...
@hspace{2em} @show{(contract 'rotate '((degrees Number) (image Image)) "Image")}
...
@A{90, 270 and other multiples of 90 that aren't also multiples of 180 would make it vertical}
