...
@span{.center}{
@show{ (sexp->code '(make-posn 100 200)) }
}

@n On the computer, try making a `Posn` or two. What expression will make a `Posn` representing the origin? @fitb{10em}{@showsoln{(sexp-code> '(make-posn 0 0))}}
