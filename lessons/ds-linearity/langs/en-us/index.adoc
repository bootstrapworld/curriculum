...
Look at the six "sideways" data tables below. You can think of the "x" column as counting the __order in which the y-values appear__ (1st value, 2nd value, etc). The "y" column, on the other hand, shows the values in the sequence:
...
When there is a @vocab{linear} relationship, the values in a table increase at a _constant rate_ and the graph of those values will all fall on a straight line.

The same linear relationship can be shown as a table or a graph. Can you see the same pattern in both representations? For any line on the graph, we can select a few points and represent them as x's and y's as rows in a table. 
