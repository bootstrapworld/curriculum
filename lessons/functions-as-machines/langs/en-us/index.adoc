...
Have students open @starter-file{gt}. And find the function definition.
...
[.lesson-instruction]
--
Turn to @printable-exercise{pages/match-examples-definitions.adoc}. You will see the definition of `gt` in the first row of the table. Circle the numbers that are changing and label them with the word `size`.

Then, using `gt` as a model, identify and label what is changing in each of the mystery example blocks and match them to their corresponding definitions.
--
...
We've successfully defined a function to make green triangles for me, but they're not everyone's favorite. So, let's take what we've learned to define some other functions.

=== Investigate

[.lesson-instruction]
- Open the @starter-file{gt} and try changing one of the examples to be incorrect.
...
Have students turn to @printable-exercise{defining-functions.adoc}. As they complete each definition, they should type the examples and definitions into the `Definitions Area` of the @starter-file{gt}, click run and test them out.
