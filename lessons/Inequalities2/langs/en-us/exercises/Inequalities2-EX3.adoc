@design-recipe-exercise{ "is-beach?" "Use the Design Recipe to write a function ’is-beach?’, which takes in a temperature in Fahrenheit and a forecast and determines if it is beach weather: hot and sunny" 
