...
- We're going to tell Soekia to generate a fairy tale and follow along and read the text until Soekia finishes writing.
- @right{@image{images/text-gen.png, 150}}To tell Soekia to start writing, click on the paper airplane on the right.
- When Soekia is done writing, explore what the other buttons on the screen will let you do. 
}
...
Just like a natural language, the tic-tac-toe text can be parsed into n-grams and then the likelihood of each n-gram's appearance can be determinec, so Soekia was able to apply the same algorithms used on our fairytale corpus to produce output.
