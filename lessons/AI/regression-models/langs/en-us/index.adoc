...
@lesson-prereqs{decision-tree-models-quantitative, data-driven-algorithms}
...
_Hint: Think back to the case study from @lesson-link{data-driven-algorithms} on Michelle's Spotify use. Recall that, at first, Michelle did not like Spotify's "Discover Weekly" playlist because the songs did not match her tastes._
}
...
- Open the @starter-file{premiums} starter file. Click "Run" and then type `premiums-table` into the Interactions area. Press "Enter".}
