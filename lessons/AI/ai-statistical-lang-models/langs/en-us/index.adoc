...
- Write down two other possible next words.
}

@teacher{
Record students word suggestions on the board. If a word is repeated, keep a tally of how many students suggest each particular word.
...
. When making a suggestion, it offers the words that had the highest probability of following "Close the" in the training dataset.

In this particular corpus, "door", "doors", and "house"  most commonly appeared after "Close the". As a result, those words are suggested.
...
@teacher{
Share the @handout{old-lady-lyrics.adoc, song lyrics"} with students,  have students and have them read them independently". If desired, you could also listen to a recorded version of the song.
}
