...
We're going to explore Soekia, a simplified text generation tool designed for student learning. As we explore, we are going to consider: What makes a _language_ in a statistical language model?
...
Think back to @printable-exercise{sampling.adoc}. Let's compare the process that you used to generate text with Soekia's algorithm. 
