...
A @vocab{decision tree} is a model produced by a machine learning algorithm that uses a tree-like model similar to the diagram we just made to show decisions and their possible consequences. Many softwares that make recommendations utilize decision trees.
...
@lesson-instruction{
- The diagram I made of our 20 questions game is a partial decision tree.
...
@Q{How are they different?}using `flat?` as the root node with `used-to-chop` as the first decision node for "yes" and `utensil?` as the first decision node for "no"
}

[cols="1a,1a"]
|===
|@image{images/tree4.png}
|@image{images/flat-tree-soln.jpg}
|===
