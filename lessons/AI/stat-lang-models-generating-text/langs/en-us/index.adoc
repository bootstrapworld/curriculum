...
When restricted to a single word (unigrams only), the n-gram language model stores the exact same information as the Bag of Words! Even though they store the same data, however, it's important to keep in mind that they come from two very different places:

- Bag-of-Words is a _representation_ of a corpus of documents, which turns them into coordinates of n-dimensions for the sake of classification or regression.
- Unigrams come from _Language Modeling_. They (and their siblings bigrams, trigrams, etc) are used to _calculate probabilities_.
