...
- A messaging app also includes a @vocab{model} of _the words that the owner uses_, to personalize suggestions to the way the user writes. The app can also account for previously-accepted word suggestions.
- Some predictive text features even account for the specific message being written! For instance, typing first letter of an unusual word that you used in the same message triggers the app to propose that unusual word.
...
@slidebreak{LaunchR}
@right{@image{images/bag-of-colors.png, 300}}
...
- We can use these contract to find all the possible completions for a starting string: `completions(c, "swallowed the")`

- Or to choose the next `n` words at random: `choose-completion(c, "swallowed the", 1)` chooses one word; increase the `1` to generate more at once
...
@QandA{
@A{Will _any_ line we draw be *perfect*? Why or why not?}
@A{When is a line useful?}
}
