...
While texting, emailing, or even conducting a web search, you've probably noticed that your thoughts and sentences are sometimes finished for you! Predictive text is now a common feature of many apps, and it is made possible by a certain form of AI: the @vocab{language model}.
...
- As a class, are there more than one “next word”?
...
1. The computer extracts each use of the phrase "Close the" from the corpus, along with the subsequent word.
...
@hspace{1em}It offers the words that had the highest probability of following "Close the" in the corpus.
...
@A{This question is a great opportunity to revisit previous concepts, in particular the process of training and sampling that students discussed when thinking about @lesson-link{decision-trees-evaluating} and @lesson-link{regression-models-simple}.}
}
...
- If there are multiple words with equal probability, the system will select the word at random. As a result, even the same phone won't necessarily propose the same word every time.
- Not all phones use the same corpus, the same prediction rules, or the same amount of randomness.
- A messaging app also includes a @vocab{model} of _the words that the owner uses_, to personalize suggestions to the way the user writes.
- Some predictive text features even account for the specific message being written! Typing first letter of an unusual word that you used in the same message triggers the app to propose that unusual word.
...
For our corpus, we will use the folk song @printable-exercise{old-lady-lyrics.adoc}, which tells the nonsensical story of an old lady who swallows a fly, and the unfortunate series of events that follow.

@lesson-instruction{
- Read the lyrics on @printable-exercise{old-lady-lyrics.adoc}
- Fill in the rest of the table at the bottom
}

@slidebreak{Launch}

You chopped up the first line of our corpus into differently sized chunks (one word at a time, two words at a time, etc.):
...
@slidebreak{LaunchR}
@lesson-point{The Table of Unigrams is effectively a Bag of Words}

@right{@image{images/bag-of-colors.png, 250}}
When restricted to single words (unigrams), an n-gram model stores the exact same data as a Bag of Words, though they originate from very different concepts:

- The @vocab{bag of words model} represents documents as @math{n}-dimensional coordinates, allowing them to be used for classification or regression.
- Unigrams come from Language Modeling and—along with bigrams and trigrams—are used to _calculate probabilities_.

Same data, very different purposes.

=== Investigate
@slidebreak{Investigate}

@lesson-instruction{
- Open the @starter-file{ai-ngrams}, Save a Copy, and click "Run"
- Evaluate `corpus` in the Interactions Area
- Evaluate `generate-ngrams(corpus, 1)`.
}
...
@QandA{
@Q{Referring to the corpus (the @printable-exercise{old-lady-lyrics.adoc, "lyrics"} *and the title*: how many times does the word "there" appear in the song?}
@A{5}
...
In the example you just worked through, you computed the _conditional probability_ that "was" appears after the unigram "there" by dividing the number of times *`"there"` is followed by `"was"`* (five times) by the number of times we see *`"there"` followed by anything* (five times)}

This fraction represents the  that "was" will come after "there":

@indented{
@math{p(was | there) = \frac
...
@teacher{
If you'd like to explore probability and sampling in greater depth, check out our lesson on @lesson-link{probability-inference, the subject}!
}

@slidebreak{Investigate}

@lesson-instruction{
- Return to @starter-file{ai-ngrams}
- *In the Definitions Area*, build your language model by evaluating `m = build-lang-model(corpus)`, then click "Run".
- Evaluate `m` to see what it looks like!
- To find the probability of one word following another, evaluate `next-word-probability(m, "swallowed a", "fly")`
}
...
@objective{ai-slm-hallucination}
...
We've created helper functions for this, too:

@show{(contract "completions" '((model Table) (start String)) "Table")}

@show{(contract "choose-completion" '((model Table) (start String) (n Number)) "String")}

- We can use these contract to find all the possible completions for a starting string: `completions(m, "swallowed the")`

- Or to choose the next `n` words at random:
@indented{
`choose-completion(m, "swallowed the", 1)`
`choose-completion(m, "swallowed the", 2)`
}

@slidebreak{Investigate-DN}


@teacher{Note that on @printable-exercise{sampling.adoc}, questions build on one another. In other words, a student who misses Question 2 will also get Question 3 incorrect. To ensure that class runs smoothly, we encourage you to require that students check their work with a partner before moving from one question to the next. Encouraging students to annotate @printable-exercise{old-lady-lyrics.adoc} will also result in more accurate counting and therefore more correct responses!}

@lesson-instruction{
- Complete the first section of @printable-exercise{sampling.adoc}.
}
...
@Q{What do you think would happen if you start with a word like "llama", which doesn't appear _at all_ in the original corpus?}
@A{The probability of following "llama" is the same for every word: zero. Maybe `choose-completion` would choose at random?}
}
...
- When you're finished, discuss: what would happen if we just kept evaluating `choose-completion`, over and over?
}
...
@Q{What would happen if we just kept evaluating `choose-completion`, over and over?}
@A{It would keep choosing words!}
}

@slidebreak{Investigate}
If you're curious to see what would happen if we just kept going, evaluate `generate-from(m, "there")`, or `generate-from(m, "cat")`!

@QandA{
@Q{Will we always get the same output if we always use the same input?}
@A{No - in the event of a tie, it will choose randomly!}
}
...
@QandA{
@Q{What does our n-grams table summarize about this dataset?}
...
@QandA{
@Q{Will _any_ language model be *perfect*? Why or why not?}
@Q{When is a language model *useful*?}
}
