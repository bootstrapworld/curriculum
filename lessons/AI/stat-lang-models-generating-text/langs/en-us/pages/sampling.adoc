...
@link-instructions{Use Pyret to answer the questions below.}

@indented{
@show{(contract "build-lang-model" '((corpus String)) "Table")} +
@show{(contract "generate-ngrams" '((model Table) (size Number)) "Table")} +
...
@n Use statistical language modeling to determine the fourth word for each of the sentences below, using each of the two words you computed for the third spot.
