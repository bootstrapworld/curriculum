...
@lesson-instruction{
- Before we start exploring the other panels, we'll want to find a smaller and shorter collection of documents. Go to the collections @image{images/collection-icon.png, 15} drop down menu and select {nbsp} @image{images/paw-icon.png, 13} *Intelligent Monkeys?*
- Once you have the collection open, take a look at the N-Grams panel. By default you should see the `3` tab selected, displaying all of the possible @vocab{trigrams} in the collection, sorted from most to least frequently occurring. 
- Turn to @printable-exercise{meet-soekia-2.adoc}, which will prompt you to interact with a few specific features of the other panels. 
- After you have a basic sense of each panel's purpose, take a moment to Notice and Wonder. Get curious, click around, and consider how each panel relates to the others that you have already explored.
