= Reflecting on Self-Driving Cars
...
@blanklines{3}{
@ifsoln{Although our car's training at this point enables accurate steering in a variety of conditions, it was not trained on busy multi-lane roads. The steering angles differ when the road is wider, and the appearance of unexpected, unfamiliar objects (like other cars) could result in very dangerous driving.
}
}
...
@n In addition to producing a steering angle for each image of the road, many models produces a _numeric_ measure of "confidence" in its response.

- What do you think causes a model's "confidence" to increase or decrease?
