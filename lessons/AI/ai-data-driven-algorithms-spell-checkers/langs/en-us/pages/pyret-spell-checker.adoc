...
@blanklines{3}{
@ifsoln{The first spell checker would only propose one adjustment, whereas the Pyret spell checker proposes two. Pyret can delete letters, while the first spell checker could not. Pyret also only proposes 5-letter words, while the first spell checker did not have a letter limit.}
}
