...
Now we have thirty words total, made up of _four_ unique words: doo, be, bop, and *ski*. Plotting all of our documents would require the use of a _four-dimensional_ space (!). Having trouble visualizing a four-dimensional space? You're not alone! Our brains work in three dimensions each and every day, so visualizing a four-dimensional space is counter-intuitive, to say the least. That's why we leave it to the computer!


=== Investigate

Computers, unlike humans, have no issue working with multi-dimensional spaces that have hundreds of thousands of dimensions. A teacher who wants to catch plagiarism will likely opt for a plagiarism detector that has trained on an _extremely_ large collection of documents, also known as a @vocab{training corpus}.

Throughout this lesson--for instructional purposes--we've used Documents a, b, c, and d as our "training corpus."

@QandA{

@Q{Imagine a plagiarism detector that compares student essays to short strings of jazz vocalizations. Does this comparison seem logical or useful? Explain.}
...
@Q{Let's say your teacher asks all 20 students in her class to write a 500-word essay. She plans to feed those 20 essays into a plagiarism detector to use as the @vocab{training corpus}, allowing her to detect if, perhaps, two students submitted essay that were a little too similar. About how many dimensions will there be in the model?}

@A{If students do not include any repeated words in their writing whatsoever, then the model would have 10,000 dimensions. Assuming that there is _some_ repetition of common words, we can confidently guess that there will be thousands of dimensions in the model.}
}
