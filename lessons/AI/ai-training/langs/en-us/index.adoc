...
- Your students might bring new complexity to the conversation by acknowledging the possibility that a programmer's changes to the algorithm could be the cause of Michelle's increased satisfaction with her play list.
...
One standard way to summarize a document is by creating a "bag of words" model. Let's look at two documents (below). (These two documents make up our @vocab{training corpus}, or a collection of data used to train AI/ML systems.) Each document is an example of jazz "scatting", when a vocalist improvises with nonsense syllables.
...
The bag-of-words summary for both documents is exactly the same! We can construct a plagiarism detector that takes stock of word frequency and ignores literally everything else. In this instance, the plagiarism detector would discover that the two models are a perfect match.
...
== Vectors, Data Normalization, and Dimensionality
...
Documents a and b were relatively simple. We used a total of two words, and those words had the same frequency in both documents. As a result, their bags of words were identical. Checking if two bags are _equal_, however, is not terribly useful. We need a way to check if bags are _similar_. To do this, we will need to represent our bags as @vocab{vectors}.
...
Let's look at some slightly more complicated documents to better understand exactly how we can put these @vocab{vectors} to use.
