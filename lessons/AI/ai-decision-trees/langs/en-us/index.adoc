...
@lesson-instruction{
- Let's complete @printable-exercise{part-1.adoc} together, starting with "age" as the root node.
- For now, let’s agree to create three groups: teenagers, twenties, and thirties.
- This page introduces a new tool that we will use to complete all the levels of our model: the @vocab{decision stump}.
}
...
** Our rule predicted that individuals in their teens would *not* purchase the game, so we place checkmarks by the Ns that represent Jan, Jose, and Jillian. Our rule was correct for them.
...
As we move down the tree, our job is to figure out _which questions_ we should ask and _when_ we should ask them... just like when we play 20 Questions! Decision stumps will help us decide which questions produce a greater @vocab{information gain}.
...
@lesson-instruction{
- As you complete @printable-exercise{part-2.adoc} you will create and compare different __decision stumps__.
