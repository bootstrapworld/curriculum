...
- @vocab{Splitting} is the process of dividing a node into sub-nodes (decision and/or leaf nodes)with branches. These branches will not necessarily be equal in size, and can contain one or many ITEMS (WHAT WORD DO WE ACTUALLY WANT HERE?)
...
We can use a different representation, called a @vocab{tree stump} for this than we did with the yes-no questions we saw in our previous decision trees.
