...
@right{@image{images/wordle-water.png, 250}}

@Q{What do you Notice?}
@A{There are three five-letter words.}
@A{Some letters are grey, one is yellow, some are green.}
@A{Once a green letter appears, its position remains the same.}
@A{It's a game of Wordle!}

@Q{What do you Wonder?}
@A{What is the solution?}
@A{What did the player choose "radio" as their first guess?}
@A{Should we guess "later" or "water" next?}

@Q{Have you seen this game before? What are the rules?}
@A{It's Wordle!}
@A{When playing, you have six attempts to guess the five-letter word.}
@A{Each guess must be a real word.}
@A{Color feedback includes *green* (the letter is in the word and in the correct position), *yellow* (the letter is in the word but in the wrong position), and *grey* (the letter is not in the word).}

}

@teacher{
Are you familiar with Wordle? If not, you can quickly learn the rules and play it @link{https://www.nytimes.com/games/wordle/index.html, "here"}. Before moving on with the lesson, be sure to check for students' familiarity with the game via a show of hands. If any of your students have _not_ played Wordle before, play one round as a class before proceeding. Later in the lesson, students will interact with a @starter-file{spell-checker} that draws its suggestions from a Wordle dictionary.
}
...
But how exactly is a spell checker data-driven? And how do spell checkers measure similarity, as we discussed during @lesson-link{ai-intro}?
...
[cols="7a,18a", grid="none", frame="none"]
|===
|@image{images/wordle.png, 200}
|

Let's consider a different partially-played Wordle game.
