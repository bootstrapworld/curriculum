...
_Note: In our game, there is a hidden starter-file that will randomly generate the y-coordinate for where the `TARGET` enters the screen. @ifproglang{pyret}{Our `update-target` function will take in that coordinate, but won't change it. For this role play, it will likely make the most sense to work with a y-coordinate of 0.}_ 
