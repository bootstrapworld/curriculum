...
@lesson-instruction{Let's complete @printable-exercise{slope-int-tables-basic.adoc} for more practice with this before we move on to more complicated tables.}
...
@teacher{
We discuss finding the y-intercept in greater detail in the @lesson-link{function-definition-linear} lesson.
}
