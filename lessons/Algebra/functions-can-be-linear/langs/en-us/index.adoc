...
- Tables only show us __some points__ on a line, representing a __sample__ of some larger trend,
...
=== Investigate
@QandA{
@Q{How do know whether a graph represents a function?}
...
How do we find the @vocab{slope} and @vocab{y-intercept} for these functions, __without__ having to sort or extend the table?

We can exploit the fact that all linear functions form __straight lines__, and a straight line can be defined with only __two points!__ 
...
When we are fluent at reading graphs and tables, they tell us a story. In this section students will have a chance to describe the stories they see.
