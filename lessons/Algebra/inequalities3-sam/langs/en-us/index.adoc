...
@lesson-instruction{
- Open the @starter-file{inequalities-samX} in a new tab and save a copy of your own.
...
@ifproglang{pyret}{Students ready to tackle this challenge can open the @opt-starter-file{inequalities-samXY} where @show{(code '(onscreen2? x y))} is defined to take in both x and y.} 

_Note: In the context of Sam the Butterfly, being able to control x and y coordinates is optimal, but, when we add the @show{(code 'onscreen?)} function to our Game Starter File, other (hidden) functions will control the characters' y-coordinates and we don't want to interfere with them, so it'll be important that the @show{(code 'onscreen?)} function we use only changes the x-coordinate._ 

@ifproglang{wescheme}{We recommend that students tackling this challenge define a new function @show{(code 'onscreen2?)} so that their initial @show{(code 'onscreen?)} code remains ready for them to use in the next section of this lesson.
}
}
