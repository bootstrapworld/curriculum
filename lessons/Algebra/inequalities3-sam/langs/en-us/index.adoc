...
|
Even though the function is tracking both coordinates (x and y), for now we're looking to write a function that only evaluates where Sam is on the left and where Sam is on the right, because that's all we will need the function to evaluate when we use it in our Game Starter File. In other words, @show{(code '(onscreen? x y))} takes in two variables, but only uses one of them.
|
This convention is not unique to code. We see it in algebra, too. In function notation, whatever is between the parentheses specifies the variables the function takes in -- whether they are used or not! For instance, @math{f(x) = 15} and @math{f(g,m) = 15g} are two functions that have unused variables.
|
