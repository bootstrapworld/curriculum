...
|
@preparation{
- Decide whether you want to have students interact directly with the @starter-file{interactive-functions} on their computers or whether it would be easier for your situation to project each graph from the peardeck on the white board and have students come up to add points that satisfy the rules or dictate points that satisfy the rules to a scribe who will add them to the graph.
- If you would like students to use Desmos interact with the @starter-file{interactive-functions}, decide how you will share it with students and, if you are using our Google Slides, add the appropriate link to the slide deck. _If you're a first-time Desmos user, fear not!_ @dist-link{pages/desmos-instructions.adoc, _Here's what you need to do._}
...
@teacher{If you want your students to interact directly with the peardeck, be sure to make your own copy before you share the link with students!}
@lesson-instruction{
- On each slide of the @starter-file{interactive-functions}, we'll see a coordinate grid and a rule.
...
@teacher{You might just want to do a few of these slides, or you might do lots of them.}
...
We can write functions that take in an x-value and produce a y-value. 

- Different inputs can yield the same outputs. 
- But each input can only be associated with one output!
...
- If the graph represents a function, none of the vertical lines will cross the graph at more than one point. 
- If there is any vertical line that can be drawn that would intersect more than one point on the graph, it means the same input can have more than one output and the relationship is not a function.
