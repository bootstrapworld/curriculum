...
@lesson-instruction{
- Open the @starter-file{interactive-functions} on your computer.
- On each slide, you'll see a coordinate grid and a rule.
- Choose an x-value that is within the domain of the graph, apply the rule to your x-value to get a y-value, and then place a dot on the graph whose coordinates are the @math{x} you put in and the @math{y} you got back.
