...
@A{The Order of Operations tells us to start by making groupings of whatever is in parentheses (or other Grouping symbols), giving us two options for how we fill in our circles:
...
@show{(coe '(- (/ (sqr 8) 4) (* 5(+ 3 9)) ))} }
@A{No. They can happen in parallel because they are inside of separate circles.}
@A{But we do have to start with one of those computations, since all of the other operations consume their outputs.}
