...
.oooCOE .circleevalsexp { vertical-align: middle; }
...
@Q{Do we _have_ to begin with one of those two computations?}
@A{*Yes!* We must start with one of those two computations, because all of the other operations consume and depend on their outputs.}
...
- @opt{Ready for a challenge? Turn to @printable-exercise{3-column-challenge.adoc}.}
}
