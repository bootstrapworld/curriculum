...
|Preparation
| Decide whether or not you want to print and cut out @handout{pages/cards-for-matching-coe-exp.adoc, a printable set of cards for physically matching arithmetic expressions with Circles of Evaluation}.
...
@lesson-instruction{
Turn to  @printable-exercise{pages/3-column-intro-w-parens.adoc} and draw Circles of Evaluation for each of the expressions. (Ignore the code column for now! We will come back to it later.)
}
...
@lesson-instruction{
- Now that we know how to translate Circles of Evaluation into Code, turn back to @printable-exercise{pages/3-column-intro-w-parens.adoc}. Translate the circles you drew into code!
...
Note: @printable-exercise{pages/3-column-intro-w-parens.adoc} offers students the scaffold of extra parentheses. Those scaffolds drop away on @printable-exercise{pages/3-column-outro.adoc}.

There is one page of more complex problems - @printable-exercise{pages/3-column-challenge.adoc} - so that you're ready to challenge students who fly. Make sure these students know that we use @show{(code 'sqrt)} as the name of the square root function, and @show{(code 'sqr)} as the function that squares its input.
