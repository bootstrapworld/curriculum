...
@teacher{
This first page of practice is scaffolded with unnecessary parentheses to make circle placement extra obvious. In our experience this is an important bridge activity. It is important to note that those training wheels will be removed after this page so students may need support with drawing circles on subsequent exercises, even if all of your students complete this page successfully. 
}
...
@A{But we can't do anything else until we've done at least one of those computations.}
...
@show{(coe '(* 0 (+ (?ANS 3) (/ (?ANS 124) (?ANS 3)))))}
}
}
