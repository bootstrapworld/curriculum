...
@ifproglang{pyret}{
The first thing the spider does is cross over a curved line (an open parenthesis!). For Circles of Evaluation with _operators_ (addition, subtraction, etc.), the spider visits the first number on the left, then she visits the top of the circle for the operation, then the number on the right. Finally, she has to leave the circle by crossing another curved line (a close parenthesis).
}
}
...
@A{ @show{(code expr1 #:parens true)}	@hspace{10em} @show{(code expr1 #:parens true)}
}
