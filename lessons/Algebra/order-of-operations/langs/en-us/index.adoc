...
.big .mathunicode {font-size: 4em !important; color: black;}
...
@right{@image{images/pemdas.png, 200}}The order of operations are often summarized by models like this pyramid or mnemonic like PEMDAS, GEMDAS, etc. This set of rules is brittle, and doesn't always make it clear what we need to do. 
...
@hspace{5em} @show{(coe '(/ (+ 7 8) 2))} @hspace{10em} @show{(coe '(- (sqrt (+ 11 5)) 2))}
...
Let's practice with some simpler problems so that you have the tools you need to solve the problem at the center of the internet debate!
}

@center{@show{(coe '(* (/ 15 3) 6))}}
