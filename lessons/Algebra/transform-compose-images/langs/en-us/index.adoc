...
The `translate` function works like `overlay`, but instead of centering the top image on the bottom image, it specifies where on the bottom image to place the center of the top image.
...
We're about to look at some code in the @starter-file{flags}. The programmer included a series of special lines in this file - called @vocab{comments} - that will never be read by the computer.
