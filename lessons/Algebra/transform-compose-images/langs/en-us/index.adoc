...
But not all shapes are so intuitive! Stars and triangles, for example, are not "grabbed" by their centers. Instead, they are "grabbed" by the center of their __bounding box__. A bounding box is the tightest possible rectangle that can be drawn around an image.
