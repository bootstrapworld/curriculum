...
@QandA{
@Q{If the x- and y-coordinates are each numbers, how many numbers does it take to represent a single frame of the video game?}
@A{Seven: 2 for the cat (moves left and right), 2 for the cloud (moves left at random y-positions), and 1 each for the dog, ruby, and score}
@Q{How are those numbers changing - or _varying_ - as the game plays? When do they increase? Decrease?}
@A{The cat's y-coordinate increases when they jump and decreases when they fall. x-coordinates increase when a character moves right, and decreases when they move left. The score increases when the cat gets a ruby or jumps on the dog.}
@Q{How many variables would we need if the dog could also move up and down?}
@A{Eight: we'd need to add the dog's y-coordinate.}
@Q{How many variables would we need to have a two-player game with two cats?}
@A{(Sample answer) Nine: we'd need to add a second set of x-y coordinates.}
@Q{How many variables would we need if the entire game was in 3d?}
@A{(Sample answer) Nine: we'd need to add z-coordinates to each character.}
