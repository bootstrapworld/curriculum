...
- Students can make-use of the image manipulation functions `rotate`, `flip-horizontal`, and `flip-vertical`. You'll find their contracts in the @pathway-link{resources/pages/contracts.pdf, Contracts pages}. @pathway-only{_If you're working with a printed workbook, the contracts pages are included in the back._}
