...
Hovering over a pie slice or bar reveals the label, as well as the count or percentage of the whole. We can also resize the window by dragging its borders. This allows us to experiment with the data before closing the window and generating the final, non-interactive image.
...
- Use `bar-chart` to make a display showing the numbers of male and female animals at the shelter.
