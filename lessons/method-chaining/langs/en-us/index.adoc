...
Describe for your students the following scenario: a journalist comes to the shelter. She wants to write a story about a successful pet adoption--but she has a very specific set of criteria. The reporter wants to report on the adoption of an animal that weighs no more than 9 kilograms. She wants to review the an updated copy of the dataset each week, which shows all eligible animals.

[.lesson-instruction]
- What formula would a  Transformer named `weight-in-kg` require? (To convert pounds to kilograms, divide by 2.205.)
- How about `filter-if-light`?
- In this scenario, which Transformer should we apply _first_ - `weight-in-kg` or `filter-if-light`? Why?

In CODAP, we can use the result of one Transformer as an input to another. In other words: Transformers can be chained, resulting in a sequence of datasets, each a transformed version of the previous. Any updates made to an input dataset will flow through and affect its outputs. If you change the original dataset (by adding or removing an animal, for example), the updates will flow through the chain.
