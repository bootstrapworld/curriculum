...
This code takes the `animals-table`, and builds a new column called `label`. `.build-column` produces a new Table...and we call that Table's `.filter` method. `.filter` produces _yet another Table_ (of all fixed animals) and we call _that_ Table’s `order-by` method. The Table produced by `.order-by` is our final result.
