...
* @link{pages/function-cards.html, "Function Cards - print and cut"}
@material-links
...
[.lesson-instruction]
- Log into @starter-file{editor}, open a new program, and save it as "Function Composition".
...
- Be sure to use the @vocab{Definitions Area} (left side) for code you want to keep and the @vocab{Interactions Area} (right side) to test code or try out new ideas.
...
When students have completed the worksheet, explain that there is a special function in @ifproglang{wescheme}{WeScheme} @ifproglang{pyret}{code.pyret.org (CPO)} that lets us test whether or not two images are equal:
