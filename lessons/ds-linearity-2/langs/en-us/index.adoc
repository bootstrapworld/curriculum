...
2) There is allso @vocab{y-intercept}: the y-value when @math{x = 0}. On a graph, this value is where the line "intercepts" the y-axis.
...
NOTE FOR EMMANUEL: Not sure I agree with this. The magical thing about the y-intecept is that when you put in 0 for x the x term falls away... that doesn't happen with any other number... so, unlike lots of other things, like why coordinate pairs are written (x, y) instead of (y, x) there is actually a mathematically important logic to this convention.

ALSO FOR E: I think rows is a confusing word here... I'm going to make some edits to eliminate the word. Revert if that language is important from a perspective I don't have.
|===
