...
The easiest way to do this is with the `try-self-guided` script, from the top level of the repo:

  ./try-self-guided lessonName

This builds the lesson (via `make`) and launches a dev server for its self-guided materials, printing a `localhost` URL you can open in a browser. If you've already run `make` recently and don't want to wait for another full build, pass `--nomake`:

  ./try-self-guided --nomake lessonName

For lessons with a WeScheme or CODAP variant, pass `--proglang wescheme` or `--proglang codap`.

Under the hood, `make` creates the raw files needed for self-guided lessons in `distribution/$NATLANG/lessons/lessonName/self-guided/`, and it's `npm run dev` from that directory that launches the dev server -- `try-self-guided` just automates finding that directory and running the command for you.
